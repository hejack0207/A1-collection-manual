...
this feature or another sfdisk activity races with **udevd**.  The recommended way
...
the partition addressed by _partition-number_.  The unspecified fields
...
* **--part-label device partition-number **[**label**]  
  Change the GPT partition name (label).  If _label_ is not specified,
...
* **--part-type device partition-number **[**type**]  
  ...
  current partition type.  The _type_ argument is hexadecimal for MBR,
...
* **--part-uuid device partition-number **[**uuid**]  
  Change the GPT partition UUID.  If _uuid_ is not specified,
...
* **--move-data**[**=path**]  
  ...
  The _path_ overrides the default log file name
...
* **-w**,** --wipe **when  
  ...
  **auto**, in which case signatures are wiped only when in interactive mode;
  ...
  a new partition-table if the argument _when_ is not **never**. In all
...
contain only raw data from the _device_.
...
The GPT header can later be restored by:
    ...
    "dd  if=~/sfdisk-sda-0x00000200.bak  of=/dev/sda  e"
