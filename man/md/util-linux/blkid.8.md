...
util-linux, March 2013

```
.IP blkid --label label | --uuid uuid
</synopsis>

<synopsis>
.IP blkid [--no-encoding --garbage-collect --list-one --cache-file file] [--output format] [--match-tag tag] [--match-token NAME=value] [device ...]
</synopsis>

<synopsis>
.IP blkid --probe [--offset offset] [--output format] [--size size] [--match-tag tag] [--match-types list] [--usages list] device ...
</synopsis>

<synopsis>
.IP blkid --info [--output format] [--match-tag tag] device ...
```
...
* **-L**, **--label** _label_  
  ...
  **--list-one --output device --match-token LABEL=_label_**.
  ...
  **blkid**(8)
  ...
  -t LABEL=_label_ and **-o list** in your scripts rather than the **-L** option.
...
* **-u**, **--usages** _list_  
  Restrict the probing functions to the specified (comma-separated) _list_ of "usage" types.
...
* **-U**, **--uuid** _uuid_  
  Look up the device that uses this filesystem _uuid_.  For more details see the
  **--label** option.
* **-V**, **--version**  
...
**--match-token**) was found and it's possible to gather any information about the
device, an exit code 0 is returned.  Note the option **--match-tag** filters output
