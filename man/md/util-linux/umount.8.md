...
* **-N**,** --namespace **ns  
  Perform umount in namespace specified by _ns_.
...
where _suffix_ is the filesystem type (or the value from a
...
the _/etc/mtab_ file when ordinary users need to be able to unmount
a mountpoint that is not defined in _/etc/fstab_
...
Note that _/etc/mtab_ is currently deprecated and helper= and another
