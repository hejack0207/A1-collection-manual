...
net\-tools, 2014\-02\-17

```
route [-CFvnNee] [-A family |-4|-6] .TP route  [-v] [-A family  |-4|-6] add  [-net|-host] target  [netmask Nm] [gw Gw] [metric N] [mss M] [window W] [irtt I] [reject] [mod] [dyn] [reinstate] [[dev] If] .TP route  [-v] [-A family |-4|-6] del  [-net|-host] target  [gw Gw] [netmask Nm] [metric M] [[dev] If] .TP route  [-V] [--version] [-h] [--help]
```
...
* _target_  
  ...
  network or host name. Optionally you can use **/prefixlen** notation
  instead of using the **netmask** option.
* **-net**  
  the
  _target_
  is a network.
...
* **mss M**  
  sets MTU (Maximum Transmission Unit) of the route to _M_ bytes.
  Note that the current implementation of the route command does not allow
  the option to set the Maximum Segment Size (MSS).
* **window W**  
  set the TCP window size for connections over this route to _W_
...
* **irtt I**  
  ...
  route to _I_ milliseconds (1-12000). This is typically only used on
...
_/proc/net/ipv6_route_  
_/proc/net/route_  
_/proc/net/rt_cache_
...
Currently maintained by Phil Blundell &lt;[Philip.Blundell@pobox.com](mailto:Philip.Blundell@pobox.com)&gt; and Bernd Eckenfels &lt;net-tools@lina.inka.de&gt;.
