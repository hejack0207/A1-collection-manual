...
treated as an escaped version of character C, so \\nbsp;is a nbsp;character and
...
# Global Definitions
    ...
    global_defs {
        ...
        lvs_sync_daemon <INTERFACE> <VRRP_INSTANCE> [id <SYNC_ID>] [maxlen <LEN>] e
...
allows more granular control of global definition **linkbeat\_use\_polling**.

This option is preferred over the deprecated use of **linkbeat\_use\_polling** in
a vrrp_instance block, since the latter only allows using linkbeat on the interface
of the vrrp_instance itself, whereas track\_interface and virtual_ipaddresses
...
<a name="static-routesaddressesrules"></a>
...
<a name="vrrp-synchronization-groups"></a>
...
<a name="vrrp-gratuitous-arp-and-unsolicited-neighbour-advert-delay-groups"></a>
...
<a name="virtual-server-groups"></a>
...
Any combination of IP addresses, IP address ranges and firewall marks can be used. Use
of this option is intended for very large LVSs.
...
<a name="conditional-configuration-and-configuration-id"></a>
...
<a name="pre-defined-definitions"></a>
...
**${_PWD} : **The directory of the current configuration file (this can be changed if using the include directive).  
**${_INSTANCE} : **The instance name (as defined by the -i option, defaults to hostname).  
**${_RANDOM [MIN [MAX]]} : **This is replaced by a random integer in the range [MIN, MAX],
...
omitted, in which case it defaults to 1 if **end** &gt; 0 or -1 if **end** &lt; 0. so, for example:
...
There can be multiple ~SEQ elements on a line, so for example:
        ...
        $VI4= e
        vrrp_track_file offset_instance_4.${IF}.${NUM}.${ID} { e
            file "${_PWD}/679/track_files/4.${IF}.${NUM}.${ID}" e
            weight -100 e
        } e
        vrrp_instance vrrp4.${IF}.${NUM}.${ID} { e
            interface bond${IF}.${NUM} e
            use_vmac vrrp4.${IF}.${NUM}.${ID} e
            virtual_router_id ${ID} e
            priority 130 e
            virtual_ipaddress { e
                10.${IF}.${NUM}.${ID}/24 e
            } e
