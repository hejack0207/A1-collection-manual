...
\f(CWrsync -av /src/foo /dest  
\f(CWrsync -av /src/foo/ /dest/foo  
...
\f(CWrsync -av host: /dest  
\f(CWrsync -av host::module /dest  
...
\f(CWrsync somehost.mydomain.com::
...
\f(CWrsync -av host:file1 :file2 host:file{3,4} /dest/  
\f(CWrsync -av host::modname/file{1,2} host::modname/file3 /dest/  
\f(CWrsync -av host::modname/file1 ::modname/file{3,4}


Older versions of rsync required using quoted spaces in the SRC, like these
examples:

\f(CWrsync -av host:'dir1/file1 dir2/file2' /dest  
\f(CWrsync host::'modname/dir1/file1 modname/dir2/file2' /dest
...
specify the **--protect-args** (**-s**) option, or you’ll need to escape
the whitespace in a way that the remote shell will understand.  For
instance:

\f(CWrsync -av host:'file\e name\e with\e spaces' /dest
...
* **--help**  
  ...
  versions of rsync, the help will also be output if you use the **-h**
...
This option increases the amount of information you  
  ...
  information at the end. More than two **-v** options should only be used if
...
* However, do keep in mind that a daemon’s "max verbosity" setting will limit how
  ...
  a higher value than what would be set by **-vv** will be downgraded to the
...
This option changes rsync to send all its output  
  ...
  **--remote-option** -- e.g. **-M--msgs2stderr**.
  ...
  debugging using this option, you should start up a daemon using **--no-detach**
...
* \f(CW   rsync alias -a -a@-1
      \f(CW   rsync alias -t -t@-1
  
* With that as the default, you’d need to specify **--modify-window=0** (aka
  **-@0**) to override it and ignore nanoseconds, e.g. if you’re copying between
...
* Note that rsync always verifies that each _transferred_ file was
...
* For example: if you want to use **-a** (**--archive**) but don’t want
  **-o** (**--owner**), instead of converting **-a** into **-rlptgD**, you
...
* \f(CW   rsync -av /foo/bar/baz.c remote:/tmp/
  
* ... this would create a file named baz.c in /tmp/ on the remote
  machine. If instead you used
* \f(CW   rsync -avR /foo/bar/baz.c remote:/tmp/
...
* \f(CW   (cd /foo; rsync -avR bar/baz.c remote:/tmp/) 
...
* \f(CW   rsync -avR --rsync-path="cd /foo; rsync" \e   
      \f(CW       remote:bar/baz.c /tmp/
...
* For instance, if a command-line arg or a files-from entry told rsync to
  ...
  are implied when **--relative** is used.  If "path/foo" is a symlink to
  ...
  the new directory.  With **--no-implied-dirs**, the receiving rsync updates
  ...
  preservation is to use the **--keep-dirlinks** option (which will also
...
* Note that if you don’t specify **--backup-dir**, (1) the
  **--omit-dir-times** option will be forced on, and (2) if **--delete** is
  also in effect (without **--delete-excluded**), rsync will add a "protect"
...
*     * Hard links are not broken.  This means the new data will be visible
    ...
    * The efficiency of rsync’s delta-transfer algorithm may be reduced if
      ...
      a position later in the file.  This does not apply if you use **--backup**,
...
This causes rsync to update a file by appending data onto  
  ...
  Implies **--inplace**.
* The use of **--append** can be dangerous if you aren’t 100% sure that the files
...
* * **--append-verify**  
This works just like the **--append** option, but  
...
* The **--dirs** option is implied by the **--files-from** option
  ...
  **--list-only** usage) if **--recursive** wasn’t specified (so that
  directories are seen in the listing).  Specify **--no-dirs** (or **--no-d**)
...
* The option only affects the client side of the transfer, so if you need it to
  affect the server, specify it via **--remote-option**.  (Note that in a local
...
* \f(CWrsync -r --relative src/./ src/./follow-me/ dest/
...
*     * If the destination contains extraneous hard-links (more linking than
    ...
    * If you specify a **--link-dest** directory that contains hard links,
      the linking of the destination files against the **--link-dest** files can
...
* In summary: to give destination files (both old and new) the source
  ...
  **--perms** option is off and use **--chmod=ugo=rwX** (which ensures that
  ...
  putting this line in the file ~/.popt (the following defines the **-Z** option,
  and includes --no-g to use the default group of the destination dir):
* \f(CW   rsync alias -Z --no-p --no-g --chmod=ugo=rwX
  
* You could then use this new option in a command such as this one:
* \f(CW   rsync -avZ src/ dest/
...
This option causes rsync to preserve the  
  executability (or non-executability) of regular files when **--perms** is
...
* For systems that support extended-attribute namespaces, a copy being done by a
  ...
  a normal user, see the **--fake-super** option.
* The above name filtering can be overridden by using one or more filter options
  with the **x** modifier. When you specify an xattr-affecting filter rule, rsync
...
* This option also has the side-effect of avoiding early creation of directories
  in incremental recursion copies.  The default **--inc-recursive** copying
...
This tells rsync to omit symlinks when  
  it is preserving modification times (see **--times**).
* * **--super**  
This tells the receiving side to attempt super-user  
  activities even if the receiving rsync wasn’t run by the super-user.  These
  activities include: preserving users via the **--owner** option, preserving
  all groups (not just the current user’s groups) via the **--groups**
  option, and copying devices via the **--devices** option.  This is useful
...
When this option is enabled, rsync simulates  
  ...
  This option also handles ACLs (if **--acls** was specified) and non-user
  extended attributes (if **--xattrs** was specified).
...
* \f(CW  rsync -av -M--fake-super /src/ host:/dest/
...
Try to handle sparse files efficiently so they take  
  up less space on the destination.  If combined with **--inplace** the
  file created might not end up with sparse blocks with some combinations
  of kernel version and/or filesystem type.  If **--whole-file** is in
...
This tells the receiver to allocate each destination  
  ...
  **fallocate**(2) system call or Cygwin’s **posix\_fallocate**(3), not the slow
...
This makes rsync perform a trial run that doesn’t  
  ...
  **-i, --itemize-changes** options to see what an rsync command is going
...
* The output of **--itemize-changes** is supposed to be exactly the same on a
  ...
  send the actual data for file transfers, so **--progress** has no effect,
...
This option overrides the checksum algoriths.  
  ...
  and (assuming **--checksum** is specifed) the pre-transfer checksumming. If two
...
* The algorithm choices are "auto", "md4", "md5", and "none".  If "none" is
  specified for the first name, the **--whole-file** option is forced on and no
  checksum verification is performed on the transferred data.  If "none" is
  specified for the second name, the **--checksum** option cannot be used. The
...
This tells rsync to skip  
  ...
  combined with the **--ignore-existing** option, no files will be updated
...
This tells rsync to skip updating files that  
  already exist on the destination (this does _not_ ignore existing
...
* The **--delete** option may be combined with one of the --delete-WHEN options
  ...
  **--delete-during** algorithm when talking to rsync 3.0.0 or newer, and
  the **--delete-before** algorithm when talking to an older rsync.  See also
...
Request that the file-deletions on the  
  ...
  for updates, so it behaves like a more efficient **--delete-before**,
...
* * **--delete-delay**  
Request that the file-deletions on the receiving  
  side be computed during the transfer (like **--delete-during**), and then
  ...
  **--delete-after** (but can behave differently, since **--delete-after**
  ...
  using **--delete-after** (which it cannot do if **--recursive** is doing an
  incremental scan).
  See **--delete** (which is implied) for more details on file-deletion.
* * **--delete-after**  
Request that the file-deletions on the receiving  
...
This option takes the behavior of (the implied)  
  **--ignore-missing-args** option a step farther:  each missing arg will become
...
This tells rsync to avoid transferring any  
  ...
  See the **--max-size** option for a description of SIZE and other information.
...
* If this option is used with **[user@]host::module/path**, then the
  remote shell _COMMAND_ will be used to run an rsync daemon on the
...
* \f(CW    -e 'ssh -p 2234'  
      \f(CW    -e 'ssh -o "ProxyCommand nohup ssh firewall nc -w1 %h %p"'  
...
* \f(CW    rsync -avR --rsync-path="cd /a/b && rsync" host:c/d /e/
...
This option is used for more advanced  
  ...
  transfer only.  For instance, if you want to pass **--log-file=FILE** and
  **--fake-super** to the remote system, specify it like this:
* \f(CW    rsync -av -M --log-file=foo -M--fake-super src/ dest/
  
* If you want to have an option affect only the local side of a transfer when
  it normally affects both sides, send its negation to the remote side.  Like
  this:
* \f(CW    rsync -av -x -M--no-x src/ dest/
...
* Note some versions of the popt option-parsing library have a bug in them that
  ...
  option letter (e.g. \f(CW-M--log-file=/tmp/foo).  If this bug affects your
...
* If you’re combining **-C** with your own **--filter** rules, you should
  ...
  should omit the **-C** as a command-line option and use a combination of
...
* * **-F**  
The **-F** option is a shorthand for adding two **--filter** rules to  
  your command.  The first time it is used is a shorthand for this rule:
* \f(CW   --filter='dir-merge /.rsync-filter'
  
* This tells rsync to look for per-directory .rsync-filter files that have
  been sprinkled through the hierarchy and use their rules to filter the
  files in the transfer.  If **-F** is repeated, it is a shorthand for this
  rule:
* \f(CW   --filter='exclude .rsync-filter'
...
*     * The **--relative** (**-R**) option is implied, which preserves the path
    ...
    * These side-effects change the default state of rsync, so the position
      of the **--files-from** option on the command-line has no bearing on how
...
* The filenames that are read from the FILE are all relative to the
  ...
  command:
* \f(CW   rsync -a --files-from=/tmp/foo /usr remote:/backup
...
* If /tmp/foo contains the string "bin" (or even "/bin"), the /usr/bin
  ...
  if the **-r** option was enabled, that dir’s entire hierarchy would
  also be transferred (keep in mind that **-r** needs to be specified
...
* \f(CW   rsync -a --files-from=:/path/file-list src:/ /tmp/copy
...
* You may also control this option via the RSYNC_PROTECT_ARGS environment
  ...
  (note that **--no-s** and **--no-protect-args** are the negative versions).
...
* Note that the use of the **--delete** option might get rid of any potential
  fuzzy-match files, so either use **--delete-after** or specify some
  filename exclusions if you need to prevent this.
* * **--compare-dest=DIR**  
This option instructs rsync to use _DIR_ on  
  the destination machine as an additional hierarchy to compare destination
  files against doing transfers (if the files are missing in the destination
  directory).  If a file is found in _DIR_ that is identical to the
...
* Beginning in version 2.6.4, multiple **--compare-dest** directories may be
  ...
  If a match is not found, a basis file from one of the _DIR_s will be
...
This option behaves like **--compare-dest**, but  
  rsync will also copy unchanged files found in _DIR_ to the destination
...
This option behaves like **--copy-dest**, but  
  unchanged files are hard linked from _DIR_ to the destination directory.
  The files must be identical in all preserved attributes (e.g. permissions,
  possibly ownership) in order for the files to be linked together.
  An example:
* \f(CW  rsync -av --link-dest=$PWD/prior_dir host:src_dir/ new\_dir/
...
* Note that this option typically achieves better compression ratios than can
  ...
  the **-z** option, but only if both sides are at least version 3.1.1.
...
* The client rsync requests new-style compression on the server via the
  **--new-compress** option, so if you see that option rejected it means that
  the server is not new enough to support **-zz**.  Rsync also accepts the
  **--old-compress** option for a future time when new-style compression
  becomes the default.
...
Explicitly set the compression level to use  
  (see **--compress**) instead of letting it default.  If NUM is non-zero,
  the **--compress** option is implied.
* * **--skip-compress=LIST**  
Override the list of file suffixes that will  
  not be compressed.  The **LIST** should be one or more file suffixes
...
These options allow you to  
  ...
  values separated by commas.  Any matching **FROM** value from the sender is
  replaced with a **TO** value from the receiver.  You may specify usernames
...
* For the **--usermap** option to have any effect, the **-o** (**--owner**)
  ...
  as a super-user (see also the **--fake-super** option).  For the **--groupmap**
  option to have any effect, the **-g** (**--groups**) option must be used
  (or implied), and the receiver will need to have permissions to set that
  group.
* * **--chown=USER:GROUP**  
This option forces all files to be owned by USER  
  with group GROUP.  This is a simpler interface than using **--usermap** and
  **--groupmap** directly, but it is implemented using those options internally,
...
By default rsync will bind to the wildcard address when  
  connecting to an rsync daemon.  The **--address** option allows you to
...
* The "%i" escape has a cryptic output that is 11 letters long.  The general
  format is like the string **YXcstpoguax**, where **Y** is replaced by the
  type of update being done, **X** is replaced by the file-type, and the
...
*     * A **c** means either that a regular file has a different checksum
      (requires **--checksum**) or that a symlink, device, or special file has
      a changed value.
    ...
    * A **t** means the modification time is different and is being updated
      to the sender’s value (requires **--times**).  An alternate value of **T**
      means that the modification time will be set to the transfer time, which happens
      when a file/symlink/device is updated without **--times** and when a
      symlink is changed and the receiver can’t set its time.
      (Note: when using an rsync 3.0.0 client, you might see the **s** flag combined
...
This allows you to specify exactly what the  
  ...
  either **--info=name** or **-v** is specified (this tells you just the name
...
* Rsync will output the out-format string prior to a file’s transfer unless
  ...
  is in effect and **--progress** is also specified, rsync will also output
...
This option causes rsync to log what it is doing  
  ...
  enabled with a default format of "%i %n%L".  See the **--log-file-format**
...
This allows you to specify exactly what  
  per-update logging is put into the file specified by the **--log-file** option
...
This tells rsync to print a verbose set of statistics  
  ...
  algorithm is for your data.  This option is equivalent to **--info=stats2**
  if combined with 0 or 1 **-v** options, or **--info=stats3** if combined
...
Output numbers in a more human-readable format.  
  ...
  units of 1024.
* The default is human-readable level 1.  Each **-h** option increases the level
  by one.  You can take the level down to 0 (to output numbers as pure digits) by
  specifing the **--no-human-readable** (**--no-h**) option.
...
* Backward compatibility note:  versions of rsync prior to 3.1.0 do not support
  ...
  two **-h** options will behave in a comparable manner in old and new versions
  as long as you didn’t specify a **--no-h** option prior to one or more **-h**
...
A better way to keep partial files than the  
  **--partial** option is to specify a _DIR_ that will be used to hold the
...
* Rsync will create the _DIR_ if it is missing (just the last dir -- not
...
* If the partial-dir value is not an absolute path, rsync will add an exclude
  ...
  receiving side.  An example: the above **--partial-dir** option would add
  the equivalent of "**-f '-p .rsync-partial/'**" at the end of any other
  filter rules.
...
* You can also set the partial-dir value the RSYNC_PARTIAL_DIR environment
  ...
  option does not look for this environment value are (1) when **--inplace** was
  specified (since **--inplace** conflicts with **--partial-dir**), and (2) when
  **--delay-updates** was specified (see below).
* For the purposes of the daemon-config’s "refuse options" setting,
  **--partial-dir** does _not_ imply **--partial**.  This is so that a
  refusal of the **--partial** option can be used to disallow the overwriting
...
This option puts the temporary file from each  
  ...
  **--partial-dir** option, that directory will be used instead.  See the
  comments in the **--partial-dir** section for a discussion of how this
...
This option tells rsync to print information  
  ...
  **--info=flist2,name,progress**, but any user-supplied settings for those
...
* There is also a **--info=progress2** option that outputs statistics based
  ...
  lot of names.  (You don’t need to specify the **--progress** option in
  order to use **--info=progress2**.)
* * **--password-file=FILE**  
This option allows you to provide a password for  
  accessing an rsync daemon via a file or via standard input if **FILE** is
  **-**.  The file should contain just the password on the first line (all other
  lines are ignored).  Rsync will exit with an error if **FILE** is world
...
* Starting with rsync 3.1.0, the sizes output by **--list-only** are affected
  ...
  **--no-h** if you want just digits in the sizes, and the old column width
  of 11 characters.
* Compatibility note:  when requesting a remote listing of files from an rsync
  ...
  the content of subdirectories: **-r --exclude='/*/*'**.
* * **--bwlimit=RATE**  
This option allows you to specify the maximum transfer  
  ...
  be a fractional value (e.g.  "**--bwlimit=1.5m**").  If no suffix is specified,
...
* Due to the internal buffering of data, the **--progress** option may not be an
...
Works like **--write-batch**, except that  
...
Apply all of the changes stored in FILE, a  
  ...
  If _FILE_ is **-**, the batch data will be read from standard input.
  See the "BATCH MODE" section for details.
* * **--protocol=NUM**  
Force an older protocol version to be used.  This  
  is useful for creating a batch file that is compatible with an older
  version of rsync.  For instance, if rsync 2.6.4 is being used with the
  **--write-batch** option, but rsync 2.6.3 is what will be used to run the
  **--read-batch** option, you should use "--protocol=28" when creating the
...
Rsync can convert filenames between character  
  ...
  **--iconv=utf8,iso88591**.  This order ensures that the option
  ...
  Finally, you can specify either **--no-iconv** or a CONVERT_SPEC of "-"
...
* If you specify the **--protect-args** option (**-s**), rsync will translate
  ...
  remote host.  See also the **--files-from** option.
...
* * **--address**  
By default rsync will bind to the wildcard address when  
  run as a daemon with the **--daemon** option.  The **--address** option
...
This option allows you to specify the maximum transfer  
  ...
  specify a smaller **--bwlimit** value, but no larger value will be allowed.
...
\f(CWRULE [PATTERN\_OR\_FILENAME]  
\f(CWRULE,MODIFIERS [PATTERN\_OR\_FILENAME]  
...
<a name="includeexclude-pattern-rules"></a>
...
\f(CW+ /some/path/this-file-will-not-be-found  
\f(CW+ /file-is-included  
\f(CW- *  
...
\f(CW+ /some/  
\f(CW+ /some/path/  
\f(CW+ /some/path/this-file-is-found  
\f(CW+ /file-also-included  
\f(CW- *  
...
* A **p** indicates that a rule is perishable, meaning that it is
  ignored in directories that are being deleted.  For instance, the **-C**
...
\f(CWmerge /etc/rsync/default.rules  
\f(CW. /etc/rsync/default.rules  
\f(CWdir-merge .per-dir-filter  
\f(CWdir-merge,n- .non-inherited-per-dir-excludes  
\f(CW:n- .non-inherited-per-dir-excludes  
...
\f(CWmerge /home/user/.global-filter  
\f(CW- *.gz  
\f(CWdir-merge .rules  
\f(CW+ *.[ch]  
\f(CW- *.o  
...
\f(CWrsync -avF /src/path/ /dest/dir  
\f(CWrsync -av --filter=': ../../.rsync-filter' /src/path/ /dest/dir  
\f(CWrsync -av --filter=': .rsync-filter' /src/path/ /dest/dir  
...
\f(CWcat &lt;&lt;EOT | rsync -avC --filter='. -' a/ b  
\f(CW+ foo.o  
\f(CW:C  
\f(CW- *.old  
\f(CWEOT  
\f(CWrsync -avC --include=foo.o -f :C --exclude='*.old' a/ b  
...
omit the **-C** command-line option and instead insert a "-C" rule into
...
<a name="anchoring-includeexclude-patterns"></a>
...
\f(CWrsync -av --filter=': .excl' --exclude=.excl host:src/dir /dest  
\f(CWrsync -av --filter=':e .excl' host:src/dir /dest  
...
\f(CWrsync -avF --delete-after host:src/dir /dest
...
\f(CW$ rsync --write-batch=foo -a host:/source/dir/ /adest/dir/  
\f(CW$ scp foo* remote:  
\f(CW$ ssh remote ./foo.sh /bdest/dir/  


\f(CW$ rsync --write-batch=foo -a /source/dir/ /adest/dir/  
\f(CW$ ssh remote rsync --read-batch=- -a /bdest/dir/ &lt;foo  
...
\f(CWssh remotehost /bin/true &gt; out.dat
...
* **RSYNC\_PROTECT\_ARGS**  
  ...
  **--protect-args** option to be enabled by default, or a zero value to make
