...
_tar_
or
_zip_. If this option is not given, and the output file is specified, the format is inferred from the filename if possible (e.g. writing to "foo.zip" makes the output to be in the zip format). Otherwise the output format is
**tar**.
...
tar.umask
This variable can be used to restrict the permission bits of tar archive entries. The default is 0002, which turns off the world write bit. The special value "user" indicates that the archiving user’s umask will be used instead. See umask(2) for details. If
**--remote**
...
**git-upload-archive**(1). Defaults to false for user-defined formats, but true for the "tar.gz" and "tgz" formats.
...
_git-1.4.0-docs.zip_, with the prefix
_git-docs/_.
