...
You can make interesting things happen to a repository every time you push into it, by setting up _hooks_ there. See documentation for **git-receive-pack**(1).
...
**refs/**
(e.g.
**refs/heads/master**) we will try to infer where in
**refs/***
...
**refs/{tags,heads}/***. In the case of tags and commits, these will be treated as if they were the commits inside
**refs/heads/***
...
**refs/tags/***, any update of them will be rejected.
...
**+**
in the refspec (or
**--force**), except when forbidden by configuration or hooks. See
**receive.denyDeletes**
in
**git-config**(1)
and
...
**tmp**
will be removed if a local branch with the same name doesn’t exist any more. This also respects refspecs, e.g.
...
**git fetch origin-push**. This method is of course entirely defeated by something that runs
**git fetch --all**, in that case you’d need to either disable it or do something more tedious like:
...
I.e. create a
**base**
tag for versions of the upstream code that you’ve seen and are willing to overwrite, then rewrite history, and finally force push changes to
**master**
...
**githooks**(5)). The default is --verify, giving the hook a chance to prevent the push. With --no-verify, the hook is bypassed completely.
