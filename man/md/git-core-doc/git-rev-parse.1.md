...
<a name="options-for-filtering"></a>
...
**-S**
with
_git diff-*_). In contrast to the
**--sq-quote**
...
--absolute-git-dir
Like
**--git-dir**, but its output is always the canonicalized absolute path.
...
_refs/heads/master_. If you happen to have both
_heads/master_
and
_tags/master_, you can explicitly say
_heads/master_
...
_{yesterday}_,
_{1 month 2 weeks 3 days 1 hour 1 second ago}_
or
_{1979-02-26 18:30:00}_) specifies the value of the ref at a prior point in time. This suffix may only be used immediately following a ref name and the ref must have an existing log (_$GIT\_DIR/logs/&lt;ref&gt;_). Note that this looks up the state of your
**local**
...
_{1}_,
_{15}_) specifies the n-th prior value of that ref. For example
_master@{1}_
...
_master_. This suffix may only be used immediately following a ref name and the ref must have an existing log (_$GIT\_DIR/logs/&lt;refname&gt;_).
...
_&lt;branchname&gt;@{u}_) refers to the branch that the branch specified by branchname is set to build on top of (configured with
**branch.&lt;name&gt;.remote**
and
**branch.&lt;name&gt;.merge**). A missing branchname defaults to the current one. These suffixes are also accepted when spelled in uppercase, and they mean the same thing no matter the case.
...
_@{push}_
is the same as
_@{upstream}_, and there is no need for it.
...
_&lt;rev&gt;^1^1^1_. See below for an illustration of the usage of this form.
...
_rev_; because a tag is already an object, it does not have to be dereferenced even once to get to an object.
...
_foo_. Any other sequence beginning with
_:/!_
...
<a name="dotted-range-notations"></a>
...
_r2_
and is defined as
_r1 r2 --not $(git merge-base --all r1 r2)_. It is the set of commits that are reachable from either one of
_r1_
(left side) or
_r2_
...
<a name="other-ltrevgt-parent-shorthand-notations"></a>
...
**&lt;arg-hint&gt;**
**&lt;arg-hint&gt;**, if specified, is used as a name of the argument in the help output, for options that take arguments.
**&lt;arg-hint&gt;**
