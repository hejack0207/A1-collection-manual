...
Use **-r** together with **-d** to delete remote-tracking branches. Note, that it only makes sense to delete remote-tracking branches if they no longer exist in the remote repository or if _git fetch_ was configured not to fetch them again. See also the _prune_ subcommand of **git-remote**(1) for a way to clean up all obsolete remote-tracking branches.
...
**-d**
(or
**--delete**), allow deleting the branch irrespective of its merged status. In combination with
**-m**
(or
...
**&lt;pattern&gt;...**, e.g.
**git branch --list 'maint-*'**, list only the branches that match the pattern(s).
...
**git-check-ref-format**(1), **git-fetch**(1), **git-remote**(1), \m[blue]**“Understanding history: What is a branch?”**\m[]\s-2\u[1]\d\s+2 in the Git User’s Manual.
