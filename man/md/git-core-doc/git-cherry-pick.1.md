...
**gitrevisions**(7). Sets of commits can be passed but no traversal is done by default, as if the
**--no-walk**
option was specified, see
**git-rev-list**(1). Note that specifying a range will feed all &lt;commit&gt;... arguments to a single revision walk (see a later example that uses
_maint master..next_).
...
**1. **apply the change that would be shown by
...
**4. **try to apply the change introduced by
**topic^**
