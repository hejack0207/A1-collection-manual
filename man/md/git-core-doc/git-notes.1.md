...
To change which notes are shown by _git log_, see the "notes.displayRef" configuration in **git-log**(1).
...
_git notes merge --commit_, or abort the merge with
_git notes merge --abort_.
...
_git notes merge_. Use this option when you have resolved the conflicts that
_git notes merge_
...
_git notes merge_, i.e. a notes merge with conflicts. This simply removes all files related to the notes merge.
...
The default notes merge strategy is "manual", which checks out conflicting notes in a special work tree for resolving notes conflicts (**.git/NOTES\_MERGE\_WORKTREE**), and instructs the user to resolve the conflicts in that work tree. When done, the user can either finalize the merge with _git notes merge --commit_, or abort the merge with _git notes merge --abort_.
...
**core.notesRef**
or
**GIT\_NOTES\_REF**, to read notes from when showing commit messages with the
_git log_
...
**core.notesRef**
or
**GIT\_NOTES\_REF**, to read notes from when showing commit messages. This overrides the
**notes.displayRef**
setting.
...
*  1.  
  Permitted pathnames have the form _ab_**/**_cd_**/**_ef_**/**_..._**/**_abcdef..._: a sequence of directory names of two hexadecimal digits each followed by a filename with the rest of the object ID.
