...
**wpa\_supplicant** is an implementation of
...
After **wpa\_supplicant** has configured the
...
* ·  
  **wpa\_supplicant** requests the kernel
  driver to scan neighboring BSSes
* ·  
  **wpa\_supplicant** selects a BSS based on
  its configuration
* ·  
  **wpa\_supplicant** requests the kernel
...
* ·  
  ...
  Supplicant
* ·  
  If WPA-PSK: **wpa\_supplicant** uses PSK
  as the master session key
* ·  
  **wpa\_supplicant** completes WPA 4-Way
  Handshake and Group Key Handshake with the Authenticator
  (AP)
* ·  
  **wpa\_supplicant** configures encryption
...
interface, and are only valid if at least one **-i** option
...
In most common cases, **wpa\_supplicant** is
started with:
...
bug reports, is to start **wpa\_supplicant** on
...
line. **wpa\_supplicant** will use the first driver
...
**wpa\_supplicant** can control multiple
...
The
**wpa\_supplicant** system consists of the following
components:
...
* **wpa\_passphrase**  
  ...
  _wpa\_supplicant.conf_ files that include
  encrypted passwords.
...
_/etc/wpa\_supplicant.conf_, that describes the networks
you are interested in. See **wpa\_supplicant.conf**(5)
for details.

Once the configuration is ready, you can test whether the
configuration works by running **wpa\_supplicant**
...
command to start **wpa\_supplicant** on background
without debugging:
...
<a name="interface-to-pcmcia-cscardmrg"></a>
...
action handler in _/etc/pcmcia/wireless_:
...
This will make **cardmgr** start
**wpa\_supplicant** when the card is plugged
in.
