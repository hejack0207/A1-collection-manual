...
_readfds_, _writefds_, and _exceptfds_.
...
* _readfds_  
  ...
  After
  **select**()
  has returned, _readfds_ will be
...
* _writefds_  
  ...
  After
  **select**()
  has returned, _writefds_ will be
  cleared of all file descriptors except for those that
...
* _exceptfds_  
  ...
  the availability of _out-of-band_ (OOB) data for reading
  from a TCP socket.
  See
  **recv**(2),
  **send**(2),
  ...
  **select**()
  has returned,
  _exceptfds_ will be cleared of all file descriptors except for those
...
* _nfds_  
  ...
  then increment this value by one, and then pass this as _nfds_.
* _utimeout_  
...
<a name="combining-signal-and-data-events"></a>
...
(or
**pselect**())
call to return with _errno_ set to **EINTR**.
...
* 2.  
  The value _nfds_ must be properly calculated for efficiency as
  explained above.
* 3.  
...
* 5.  
  ...
  **write**(2),
  and
  **send**(2)
  do _not_ necessarily read/write the full amount of data
...
* 9.  
  ...
  **send**(2)
  ...
  then you should _not_ pass that file descriptor to
  **select**()
  again.
...
A return value of -1 indicates an error, with _errno_ being
set appropriately.
In the case of an error, the contents of the returned sets and
the _struct timeout_ contents are undefined and should not be used.
**pselect**()
however never modifies _ntimeout_.
...
#define SHUT_FD1 do {                                &nbsp;                    if (fd1 &gt;= 0) {                 &nbsp;                        shutdown(fd1, SHUT_RDWR);   &nbsp;                        close(fd1);                 &nbsp;                        fd1 = -1;                   &nbsp;                    }                               &nbsp;                } while (0)

#define SHUT_FD2 do {                                &nbsp;                    if (fd2 &gt;= 0) {                 &nbsp;                        shutdown(fd2, SHUT_RDWR);   &nbsp;                        close(fd2);                 &nbsp;                        fd2 = -1;                   &nbsp;                    }                               &nbsp;                } while (0)
