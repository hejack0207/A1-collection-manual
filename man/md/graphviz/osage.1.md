...
the graph attributes **pack** and **packmode** control the layout.
...
**-Thpgl** (HP pen plotters), and **-Tpcl** (Laserjet printers),
...
**size="_x,y**"_ sets bounding box of drawing in inches.

**page="_x,y**"_ sets the PostScript pagination unit.
...
overlay diagrams.  The PostScript array variable _layercolorseq_
...
**splines**. If set to _true_, edges are
drawn as splines.
If set to _polyline_, edges are 
drawn as polylines.
If set to _ortho_, edges are 
...
**shape=record polygon epsf builtin\_polygon**  
_builtin\_polygon_ is one of: plaintext ellipse oval circle egg 
...
**labeldistance** and port\_label\_distance set distance; also
...
**layer=_id** or id:id_ or "all" sets the edge's active layers.
The empty string means no layers (invisible).


**(neato-specific attributes)**  
**w=f** sets the weight (spring constant) of an edge
