...
**lnstat** is a generalized and more feature-complete replacement for the old
...
These names are used by **lnstat** as keys for selecting which statistics to
print. For every CPU present in the system, a line follows which lists the
actual values for each column of the file. **lnstat** sums these values up
...
* **-k, --keys k,k,k,...**  
  Display only keys specified. Each key **k** is of the form **[file:]key**. If **&lt;file&gt;**
...
* **/proc/net/stat/arp_cache, /proc/net/stat/ndisc_cache**  
  Statistics around neighbor cache and ARP. **arp\_cache** is for IPv4, **ndisc\_cache** is the same for IPv6.
...
**hits**
How many **lookups** were successful.
...
**forced\_gc\_runs**) has failed.


* **/proc/net/stat/ip_conntrack, /proc/net/stat/nf_conntrack**  
  Conntrack related counters. **ip\_conntrack** is for backwards compatibility
  with older userspace only and shows the same data as **nf\_conntrack**.
...
lnstat was written by Harald Welte &lt;[laforge@gnumonks.org](mailto:laforge@gnumonks.org)&gt;.

This manual page was written by Michael Prokop &lt;[mika@grml.org](mailto:mika@grml.org)&gt; for the Debian project (but may be used by others).
