...
<synopsis>
.ti -8 TYPE := [  bridge |  bond |  can |  dummy |  hsr |  ifb |  ipoib | macvlan |  macvtap |  vcan |  vxcan |  veth |  vlan |  vxlan | ip6tnl | ipip | sit | gre | gretap | erspan | ip6gre | ip6gretap | ip6erspan | vti | nlmon | ipvlan | ipvtap | lowpan | geneve | bareudp | vrf | macsec | netdevsim | rmnet | xfrm ]
</synopsis>
...
<synopsis>
.ti -8 VFVLAN :=  [ vlan VLANID [  qos VLAN-QOS ] [ proto VLAN-PROTO ] ] .in -8
</synopsis>

<synopsis>
.ti -8 ip link property add [ altname NAME .. ]
</synopsis>

<synopsis>
.ti -8 ip link property del [ altname NAME .. ]
```
...
* Bareudp Type Support  
  ...
  **ip link add **_DEVICE_
  **type**_ bareudp _**dstport**_ PORT _**ethertype**_ ETHERTYPE_
  [
  **srcportmin**_ SRCPORTMIN _
  ] [
  [**no**]**multiproto**
  ]
...
* BRIDGE Type Support  
  ...
  **vlan_stats_per_port**_ VLAN_STATS_PER_PORT _
  ] [
...
**vlan_stats_per_port**_ VLAN_STATS_PER_PORT _
- enable
(_VLAN_STATS_PER_PORT_ == 1) 
or disable
(_VLAN_STATS_PER_PORT_ == 0) 
...
* XFRM Type Support  
  ...
  **ip link add **_DEVICE_** type xfrm dev **_PHYS_DEV_** [ if_id **_IF_ID_** ]**
