...
iproute2, 19 Apr 2012

```

 .in +8 .ti -8 ip [ OPTIONS ] l2tp  { COMMAND |  help } 
 .ti -8 ip l2tp add tunnel
...
[ udp6_csum_tx { on | off } ]
[ udp6_csum_rx { on | off } ]
.ti -8 ip l2tp add session [ name NAME  ]
tunnel_id ID session_id ID peer_session_id ID
[ cookie HEXSTR  ] [ peer_cookie HEXSTR  ]
[ l2spec_type { none | default } ]
[ seq { none | send | recv | both } ]
.ti -8 ip l2tp del tunnel tunnel_id ID
.ti -8 ip l2tp del session tunnel_id ID session_id ID
.ti -8 ip l2tp show tunnel [ tunnel_id ID ]
.ti -8 ip l2tp show session [ tunnel_id ID.B ] [ session_id ID ]
.ti -8 NAME :=  STRING .ti -8 ADDR := { IP_ADDRESS | any } .ti -8 PORT := { NUMBER } .ti -8 ID := { NUMBER } .ti -8 .ti -8 HEXSTR := { 8 or 16 hex digits (4 / 8 bytes) }
```
...
<a name="ip-l2tp-add-tunnel-add-a-new-tunnel"></a>
...
<a name="ip-l2tp-del-tunnel-destroy-a-tunnel"></a>
...
<a name="ip-l2tp-show-tunnel-show-information-about-tunnels"></a>
...
<a name="ip-l2tp-add-session-add-a-new-session-to-a-tunnel"></a>
...
<a name="ip-l2tp-del-session-destroy-a-session"></a>
...
<a name="ip-l2tp-show-session-show-information-about-sessions"></a>
...
<a name="setup-l2tp-tunnels-and-sessions"></a>
...
<a name="configure-as-ip-interfaces"></a>
...
<a name="configure-as-bridged-interfaces"></a>
...
James Chapman &lt;[jchapman@katalix.com](mailto:jchapman@katalix.com)&gt;
