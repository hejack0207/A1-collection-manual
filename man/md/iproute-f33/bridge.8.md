...
```
.in +8 .ti -8 bridge [ OPTIONS ] OBJECT { COMMAND |  help } 

</synopsis>

<synopsis>
.ti -8 OBJECT := {  link | fdb | mdb | vlan | monitor } 

</synopsis>

<synopsis>
.ti -8 OPTIONS := {  -V[ersion] | -s[tatistics] | -n[etns] name | -b[atch] filename | -c[\folor] | -p[retty] | -j[son] | -o[neline\fr] }
</synopsis>

<synopsis>
.ti -8 "bridge link set" dev DEV [  cost COST ] [  priority PRIO ] [  state STATE ] [  guard { on | off } ] [  hairpin { on | off } ] [  fastleave { on | off } ] [  root_block { on | off } ] [  learning { on | off } ] [  learning_sync { on | off } ] [  flood { on | off } ] [  hwmode { vepa | veb } ] [  mcast_flood { on | off } ] [  mcast_to_unicast { on | off } ] [  neigh_suppress { on | off } ] [  vlan_tunnel { on | off } ] [  isolated { on | off } ] [  backup_port DEVICE ] [ nobackup_port ] [  self ] [ master ]
</synopsis>

<synopsis>
.ti -8 bridge link [ show ] [  dev DEV ]
</synopsis>

<synopsis>
.ti -8 bridge fdb { add | append | del | replace }  LLADDR dev DEV {  local | static | dynamic } [  self ] [ master ] [ router ] [ use ] [ extern_learn ] [ sticky ] [  dst IPADDR ] [  src_vni VNI ] [ vni VNI ] [ port PORT ] [ via DEVICE ] [ src_vni VNI ]
</synopsis>

<synopsis>
.ti -8 bridge fdb [ [ show ] [  br BRDEV ] [  brport DEV ] [  vlan VID ] [  state STATE ] [ dynamic ] ]
</synopsis>

<synopsis>
.ti -8 bridge fdb get [ to ] LLADDR "[ " br BRDEV ] { brport | dev } DEV [  vlan VID ] [  vni VNI ] [ self ] [ master ] [ dynamic ]
</synopsis>

<synopsis>
.ti -8 bridge mdb { add | del }  dev DEV port PORT grp GROUP [  permanent | temp ] [  vid VID ] 
</synopsis>

<synopsis>
.ti -8 bridge mdb show [ dev DEV ]
</synopsis>

<synopsis>
.ti -8 bridge vlan { add | del }  dev DEV vid VID [  tunnel_info TUNNEL_ID ] [  pvid ] [ untagged ] [  self ] [ master ] 
</synopsis>

<synopsis>
.ti -8 bridge vlan [ show | tunnelshow ] [  dev DEV ]
</synopsis>

<synopsis>
.ti -8 bridge monitor [ all | neigh | link | mdb ]
```
...
* **-n**,** -net**,** -netns **&lt;NETNS&gt;  
  ...
  **ip netns exec**
  _NETNS_
  **bridge**
  [ _OPTIONS_ ] _OBJECT_ { _COMMAND_ | 
  **help** }
  
  to
  
  **bridge**
  -n[etns] _NETNS_ [ _OPTIONS_ ] _OBJECT_ { _COMMAND_ | 
  **help** }
...
<a name="bridge-link-set-set-bridge-specific-attributes-on-a-port"></a>
...
<a name="bridge-link-show-list-ports-configuration-for-all-bridges"></a>
...
<a name="bridge-fdb-add-add-a-new-fdb-entry"></a>
...
<a name="bridge-fdb-append-append-a-forwarding-database-entry"></a>
...
<a name="bridge-fdb-delete-delete-a-forwarding-database-entry"></a>
...
<a name="bridge-fdb-replace-replace-a-forwarding-database-entry"></a>
...
<a name="bridge-fdb-show-list-forwarding-entries"></a>
...
<a name="bridge-fdb-get-get-bridge-forwarding-entry"></a>
...
<a name="bridge-mdb-add-add-a-new-multicast-group-database-entry"></a>
...
<a name="bridge-mdb-delete-delete-a-multicast-group-database-entry"></a>
...
<a name="bridge-mdb-show-list-multicast-group-database-entries"></a>
...
<a name="bridge-vlan-add-add-a-new-vlan-filter-entry"></a>
...
<a name="bridge-vlan-delete-delete-a-vlan-filter-entry"></a>
...
<a name="bridge-vlan-show-list-vlan-configuration"></a>
...
<a name="bridge-vlan-tunnelshow-list-vlan-tunnel-mapping"></a>
