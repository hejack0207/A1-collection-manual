...
* Host Access  
  ...
  the _xhost_ command.   The list is stored in the server by network
  ...
  the _Xserver_ man page for details on how this list is initialized at
  server startup.
* MIT-MAGIC-COOKIE-1  
  ...
  _xdm_ generates such cookies automatically when this form of
  access control is used.
  The user's copy of
  the cookie is usually stored in the _.Xauthority_ file in the home
  directory, although the environment variable **XAUTHORITY** can be used
  to specify an alternate location.
  _Xdm_ automatically passes a cookie to the server for each new
...
* XDM-AUTHORIZATION-1  
  ...
  stored in the _.Xauthority_ file and is shared with the X server.
  However,
...
* SUN-DES-1  
  ...
  X server is needed for this; this identity is stored in the _.Xauthority_
...
* To allow access by a new user, use _xhost_.  For example,
  ...
  their _.Xauthority_ file.  For example:
  ...
  _keylogin_ on the remote host first.
* Server Interpreted  
  ...
  _xhost_.  For example to add a Server Interpreted entry of type
...
the _.Xauthority_ file to generate the correct authorization information
...
(unix._hostname_@_domain_ when the server is started by _xdm_),
and so it is not useful to anyone not authorized to connect to the server.

Each entry in the _.Xauthority_ file matches a certain connection family
...
The _xauth_ program manipulates the _.Xauthority_ file format.
...
information from a file name passed on the command line with the _-auth_
option (see the _Xserver_ manual page).  The authorization entries in
...
used by both processes.  This is especially useful when _xinit_ is used.

* MIT-MAGIC-COOKIE-1  
  This system uses 128 bits of data shared between the user and the X server.
  Any collection of bits can be used.  _Xdm_ generates these keys using a
...
* XDM-AUTHORIZATION-1  
  ...
  in 8 bytes, the last byte of which is ignored.  _Xdm_ generates these keys
...
* SUN-DES-1  
  ...
  When _xdm_ starts the X server, it uses the root
  principal for the machine on which it is running
  (unix._hostname_@_domain_, e.g.,
  "unix.expire.lcs.mit.edu@our.domain.edu").  Putting the correct principal
  name in the _.Xauthority_ file causes Xlib to generate the appropriate
