...
```
.HP \w'named&nbsp;'u named [[-4] | [-6]] [-c&nbsp;config-file] [-d&nbsp;debug-level] [-D&nbsp;string] [-E&nbsp;engine-name] [-f] [-g] [-L&nbsp;logfile] [-M&nbsp;option] [-m&nbsp;flag] [-n&nbsp;#cpus] [-p&nbsp;port] [-s] [-S&nbsp;#max-socks] [-t&nbsp;directory] [-U&nbsp;#listeners] [-u&nbsp;user] [-v] [-V] [-X&nbsp;lock-file] [-x&nbsp;cache-file]
```
...
_external_, which causes the internal memory manager to be bypassed in favor of system-provided memory allocation functions.
...
_port_. If not specified, the default is port 53.
...
On Linux,
**named**
uses the kernels capability mechanism to drop all root privileges except the ability to
**bind(2)**
...
<a name="red-hat-selinux-bind-security-profile"></a>
...
By default, the SELinux policy allows named to write any master zone database files. Only the root user may create files in the $ROOTDIR/var/named zone database file directory (the options { "directory" } option), where $ROOTDIR is set in /etc/sysconfig/named.
...
Copyright © 2000, 2001, 2003-2009, 2011, 2013-2021 Internet Systems Consortium, Inc. ("ISC")  
