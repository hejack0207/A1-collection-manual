...
polynomial used for CRC error checking in the ISO/IEC&nbsp;8802-3:\|1996 standard (Ethernet).

The encoding for the CRC checksum is defined by the generating
polynomial:

    
    G(x)=xus-332s+3d+xus-326s+3d+xus-323s+3d+xus-322s+3d+xus-316s+3d+xus-312s+3d+xus-311s+3d+xus-310s+3d+xus-38s+3d+xus-37s+3d+xus-35s+3d+xus-34s+3d+xus-32s+3d+x+1
...
utility shall write in the following format:
    ...
    "%u %d %sen", <checksum>, <# of octets>, <pathname>
...
The algorithm selected is based on that used by the ISO/IEC&nbsp;8802-3:\|1996 standard (Ethernet)
