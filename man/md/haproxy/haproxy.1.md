...
* **-C &lt;dir&gt;**  
  Change directory to &lt;_dir_&gt; before loading any files.
...
* **-dM[&lt;byte&gt;]**  
  Initializes all allocated memory areas with the given &lt;_byte_&gt;. This makes
  it easier to detect bugs resulting from uninitialized memory accesses, at the
  expense of touching all allocated memory once. If &lt;_byte_&gt; is not
...
package have UDP disabled by default. The **-r** option must be passed
...
* - **SIGPIPE**  
  This signal is intercepted and ignored on systems without **MSG\_NOSIGNAL**.
