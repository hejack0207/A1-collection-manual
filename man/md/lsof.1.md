...
_Lsof_
revision  lists on its standard output file information about files
...
specifying the
**-i**
option without an address and the **-u**foo option produces a
...
**-a**,
**-U**,
and **-u**foo produces a listing of only UNIX socket files that
...
**-a**,
and **-u**fff,ggg will select the listing of files that belong to
...
* **CAUTION:**
  ...
  **+|-m**_ m_
  ...
  Use this option with great care and fully specify the path name of the
  file system to be exempted.
...
* Pseudoterminal
  ...
  \`\`-&gt;/dev/pts_min_&nbsp;_PID,cmd,FDmode_'' or \`\`_PID,cmd,FDmode_''.
...
* UNIX socket file endpoint information is displayed in the NAME column
  in the form  
  \`\`type=_TYPE_&nbsp;-&gt;INO=_INODE_&nbsp;_PID,cmd,FDmode_'', where
  _TYPE_
  is the socket type;
  _INODE_
...
* **-i**_ [i]_  
  ...
  matches the address specified in _i_.
...
* .ie !\n(.g \{
  [_46_][_protocol_][@_hostname_\||\|_hostaddr_][:_service_\||\|_port_]
  \}
  .el \{
  [_46_][_protocol_][@_hostname_\||\|_hostaddr_][:_service_\||\|_port_]
  \}
...
* When
  ...
  **-L**.)
  ...
  have been unlinked.
  A specification of the form \`\`**+aL1&nbsp;&lt;file\_system&gt;**'' will select
...
* **-o**_ o_  
  defines the number of decimal digits (_o_) to be
...
* Case is unimportant in the protocol or state names, but there may
  ...
  name (_p_) and the state name list (_s_) is required.
...
* or \`\`clone _n_:/dev/event'' for SCO OpenServer file clones of the
  _/dev/event_
  device, where
  _n_
...
<a name="output-for-other-programs"></a>
...
<a name="avoiding-kernel-blocks"></a>
...
<a name="alternate-device-numbers"></a>
...
<a name="lsof-permissions-that-affect-device-cache-file-access"></a>
...
<a name="device-cache-file-path-from-the-d-option"></a>
...
built in a specific location (**b**[_path_]);
read but not rebuilt (**r**[_path_]);
or read and rebuilt (**u**[_path_]).
The
**b**,
**r**,
and
...
<a name="device-cache-path-from-an-environment-variable"></a>
...
<a name="system-wide-device-cache-path"></a>
...
<a name="personal-device-cache-path-default"></a>
...
<a name="modified-personal-device-cache-path"></a>
...
_/dev/sr0_).
(_Lsof_ reports the directory numbers.)
...
_lsof.itap.purdue.edu_
at
_pub/tools/unix/lsof_FAQ_._
The URL is:
...
_Lsof_
was written by Victor A.Abell &lt;[abe@purdue.edu](mailto:abe@purdue.edu)&gt; of Purdue University.
