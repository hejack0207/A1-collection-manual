...
For NVMe namespaces see two paragraphs down. When the _--hosts_ option
is given for SCSI devices the tuple is reduced to one element: the host
number.

If a _H:C:T:L_ tuple is given as an argument on the command line then
...
_--hosts_ in which case only the _H_ is active and may be either
...
that use the SCSI subsystem. To filter using a _H:C:T:L_ argument for
...
Linux would produce by default, so the _--kname_ option is provided.
...
long options can also take underscore, and vice versa (e.g. _--scsi\_id_
or _--scsi-id_ are acceptable).
...
* **-b**, **--brief**  
  ...
  _--generic_ option it will show on each line the tuple (from which
  ...
  pass-through).  
  When the _--pdt_ option is used together with this option the SCSI
...
* **-g**, **--generic**  
  ...
  when the _--hosts_ option is used.  
  To unclutter the single line per device mode the _--brief_ option
  combined with this option should help.
* **-h**, **--help**  
  Output the usage message and exit.
* **-H**, **--hosts**  
  List the SCSI hosts and NVMe controllers currently attached to
  the system. If this option is not given (and the _--controllers_
...
* **-U**, **--long-unit**  
  ...
  _--long\_unit_ is also an acceptable form when invoking this option.
* **-x**, **--lunhex**  
...
* **-N**, **--no-nvme**  
  ...
  output like that produced when the _--classic_ option is used.  
...
* **-t**, **--transport**  
  ...
  if _--hosts_ is given, initiator related information. When used without
  _--list_, a name or identifier (or both) are output on a single line,
  ...
  _--list_ option is also given then additionally multiple lines
...
* **-w**, **--wwn**  
  ...
  superseded by the _--unit_ and _--long-unit_ options.
...
when the _--hosts_ option is given. A host is called an initiator in
...
When the _--transport_ option is given for devices (i.e.
_--hosts_ not given) then most of the information produced by lsscsi
...
Significantly more information can be obtained by adding the _--list_
option. When used together with the _--transport_ option, after
...
EUI-64 based target port name is output when _--transport_ is given,
in the absence of the _--hosts_ option. When the _--hosts_
...
when _--transport_ is given. In the absence of the _--hosts_
option these ids will be for the target port associated with the
device (logical unit) being listed. When the _--hosts_ option is
...
For iSCSI the target port name is output when _--transport_ is given,
in the absence of the _--hosts_ option. This is made up of the
iSCSI name and the target portal group tag. Since the iSCSI name starts
with "iqn" no further prefix is used. When the _--hosts_ option
...
port if _--hosts_ option is also given) is output. This will be a naa-5
...
a number between 0 and 15 inclusive) is output when _--transport_ is
given, in the absence of the _--hosts_ option. When the _--hosts_
...
When the _--transport_ option is given, after "pcie" the NVMe
...
a Linux LUN as a decimal integer. When the _--lunhex_ option is given
...
If the _--lunhex_ option is given twice then the full 64 bits (i.e. 16
hexadecimal digits) are shown.

If the _--lunhex_ option is not given on the command line then the
...
_--lunhex_ is set internally. So, for
example, 'LSSCSI_LUNHEX_OPT=2  lsscsi' and 'lsscsi -xx' are equivalent.

<a name="order-of-scsi-identifiers"></a>

# Order of Scsi Identifiers

This section only applies to the _--scsi\_id_ option and only for
...
There is a more general _--wwn_ option that should apply to almost all
...
Plus there the _--unit_ and the _--long-unit_ options that may
...
form '/dev/bsg/_H:C:T:L_'. So, for example, the SCSI device shown by this
