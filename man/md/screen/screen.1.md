...
screen -r sessionowner/[[pid.]tty[.host]] .ta .5i 1.8i
```
...
Screen runs them as sub-shells, unless told otherwise (See shell\*U .screenrc command).
...
_Screen_
does not understand the prefix C-\*U to mean control, although this notation is 
used in this manual for readability. 
Please use the caret notation (^A\*U instead of \*QC-a\*U) as arguments
to e.g. the 
_escape_
command or the _-e_ option.
_Screen_
will also print out control characters in caret notation.

The standard way to create a new window is to type C-a c\*U.
...
C-a :\*U command line) and
then using it just like the C-a c\*U command.
...
If /etc/utmp\*U is writable by
_screen_,
an appropriate record will be written to this file for each window, and
removed when the window is terminated.
This is useful for working with talk\*U, \*Qscript\*U, \*Qshutdown\*U,
rsend\*U, \*Qsccs\*U and other similar programs that use the utmp
file to determine who you are. As long as
_screen_
is active on your terminal,
the terminal's own record is removed from the utmp file. See also C-a L\*U.
...
you should remember this one command:  C-a ?\*U.
...
the section DEFAULT KEY BINDINGS\*U. The manual section \*QCUSTOMIZATION\*U
deals with the contents of your .screenrc.

If your terminal is a true\*U auto-margin terminal (it doesn't allow
...
nowadays have magic\*U margins (automatic margins plus usable last
column). This is the VT100 style type and perfectly suited for
_screen_.
If all you've got is a true\*U auto-margin terminal 
_screen_
...
* **-A**  
  ...
  (those with WS\*U in its description, e.g. suncmd or some xterm).
* **-c **_file_  
  override the default configuration file from $HOME/.screenrc\*U
  to _file_.
* **-d**|**-D** [**_pid.tty.host_**]  
  does not start
  _screen_,
  ...
  _screen_
  session. It has the same effect as typing C-a d\*U from
  _screen_'s
...
* ""  
  ...
  screen -list\*U.
* **-e **_xy_  
  specifies the command character to be _x_ and the character generating a
  literal command character to _y_ (when typed after the command character).
  The default is C-a\*U and \`a', which can be specified as \*Q-e^Aa\*U.
  When creating a
  _screen_
  ...
  This option is equivalent to either the commands defescape\*U or
  escape\*U respectively.
* **-f**, **-fn**, and **-fa**  
  turns flow-control on, off, or automatic switching mode\*U.
  This can also be defined through the defflow\*U .screenrc command.
* **-h **_num_  
  Specifies the history scrollback buffer to be _num_ lines high.
* **-i**  
  will cause the interrupt key (usually C-c) to interrupt the display
  immediately when flow-control is on.
  See the defflow\*U .screenrc command for details.
  The use of this option is discouraged.
* **-l** and **-ln**  
  turns login mode on or off (for /etc/utmp updating).
  This can also be defined through the deflogin\*U .screenrc command.
* **-ls** [**match**]  
* **-list** [**match**]  
  does not start
  _screen_,
  ...
  _screen_
  sessions.
  Sessions marked \`detached' can be resumed with screen -r\*U. Those marked
...
* **-Logfile **_file_  
  By default logfile name is screenlog.0\*Q. You can set new logfile name
  with the -Logfile\*Q option.
* **-m**  
  causes
  _screen_
  to ignore the $STY environment variable. With screen -m\*U creation of
...
* **-d -m**  
  Start
  _screen_
  in detached\*U mode. This creates a new session but doesn't
  attach to it. This is useful for system startup scripts.
* **-D -m**  
  This also starts screen in detached\*U mode, but doesn't fork
...
* **-O**  
  ...
  This can also be set in your .screenrc by specifying \`OP' in a termcap\*U
  command.
* **-p **_number_or_name|-|=|+_  
  Preselect a window. This is useful when you want to reattach to a
  specific window or you want to send a command via the -X\*U
  option to a specific window. As with screen's select command, -\*U
  selects the blank window. As a special case for reattach, =\*U
  brings up the windowlist on the blank window, while a +\*U
  will create a new window. The command will not be
  executed if the specified window could not be found.
* **-q**  
  Suppress printing of error messages. In combination with -ls\*U the exit 
  ...
  In combination with -r\*U the exit value is as follows: 10 indicates that 
  ...
  In all other cases -q\*U has no effect.
* **-Q**  
  Some commands now can be queried from a remote session using this
  flag, e.g. screen -Q windows\*U. The commands will send the
...
* **-r** sessionowner/[**pid.tty.host**]  
  resumes a detached
  _screen_
  session.  No other options (except combinations with **-d**/**-D**) may
  be specified, though an optional prefix of [_pid._]_tty.host_
...
* **-R**  
  ...
  **-R**
  ...
  _screen_
  is run as a login-shell (actually screen uses -xRR\*U in that case).
  For combinations with the **-d**/**-D** option see there.
* **-s **_program_  
  sets the default shell to the program specified, instead of the value
  in the environment variable $SHELL (or /bin/sh\*U if not defined).
  This can also be defined through the shell\*U .screenrc command.
  See also there.
* **-S **_sessionname_  
  When creating a new session, this option can be used to specify a
  meaningful name for the session. This name identifies the session for
  screen -list\*U and \*Qscreen -r\*U actions. It substitutes the
  default [_tty.host_] suffix.
* **-t **_name_  
  sets the title (a.\|k.\|a.) for the default shell or specified program.
  See also the shelltitle\*U .screenrc command.
* **-T **_term_  
...
* **-wipe** [**match**]  
  does the same as screen -ls\*U, but removes destroyed sessions instead of
...
* **-X**  
  ...
  several screen sessions running. You can use the **-d** or
...
C-a\*U followed by one other character.
...
of C-a a\*U; see below), thus, \*QC-a c\*U as well as \*QC-a C-c\*U can
be used to create a window. See section CUSTOMIZATION\*U for a description
of the command.
...
See also _split, remove, only_.
T}
_
C-a C-a;(other);T{
Toggle to the window displayed previously.
Note that this binding defaults to the command character typed twice,
unless overridden.  For instance, if you use the option **-e]x**\*U,
this command becomes ]]\*U.
T}
_
C-a a  ;(meta);T{
Send the command character (C-a) to window. See _escape_ command.
T}
_
C-a A;(title);T{
...
C-a C-f
T};(flow);T{
Toggle flow _on_, _off_ or _auto_.
T}
_
C-a F;(fit);T{
...
_
C-a h;(hardcopy);T{
Write a hardcopy of the current window to the file hardcopy._n_\*U.
T}
_
C-a H;(log);T{
Begins/ends logging of the current window to the file screenlog._n_\*U.
T}
_
T{
C-a i,  
...
T}
_
C-a x or C-a C-x;(lockscreen);T{
Lock this terminal.
T}
_
C-a X ;(remove);T{
Kill the current region.
See also _split, only, focus_.
T}
_
T{
C-a z,  
...
_
C-a Z;(reset);T{
Reset the virtual terminal to its power-on\*U values.
T}
_
C-a .;(dumptermcap);T{
Write out a .termcap\*U file.
T}
_
C-a ?;(help);T{
Show key bindings.
...
The socket directory\*U defaults either to $HOME/.screen or simply to
...
/etc/screenrc\*U and
.screenrc\*U in the user's home directory. These are the \*Qprogrammer's
defaults that can be overridden in the following ways: for the
...
The command line option **-c** takes
...
etc/screenrc\*U and \*Qetc/etcscreenrc\*U. They contain a number of
useful examples for various commands.

Customization can also be done 'on-line'. To enter the command mode type
\`C-a :'. Note that commands starting with def\*U change default values,
while others change current settings.

The following commands are available:

* **acladd**_ usernames _**[**_crypted-pw_**]**  
* **addacl**_ usernames_  

Enable users to fully access this screen session. _Usernames_ can be one
user or a comma separated list of users. This command enables to attach to the
_screen_
session and performs the equivalent of \`aclchg _usernames_ +rwx "#?"'.
...
* **aclchg**_ usernames permbits list_  
* **chacl**_ usernames permbits list_  
...
all windows, \`?' to all commands. if _usernames_ consists of a single \`*',
...
\`aclchg _username_ -w+w 2'.
To allow read-only access to the session: \`aclchg _username_
...
**nobody**
cannot be changed (see the su\*U command).
...
* **aclgrp**_ username _**[**_groupname_**]**  
...
A user is removed from all groups the special value none\*U is used for
_groupname_.
If the second parameter is omitted all groups the user is in are listed.

* **aclumask** [[ _users_ ] _+bits_ | [ _users_ ] _-bits..._ ]  
* **umask** [[ _users_ ] _+bits_ | [ _users_ ] _-bits..._ ]  
...
aclchg\*U command. The special username \*Q?\*U predefines the access
that not yet known users will be granted to any window initially.
The special username ??\*U predefines the access that not yet known 
...
The notification message can be re-defined by means of the activity\*U
command.
Each occurrence of \`%' in _message_ is replaced by
...
by use of the monitor\*U command (C-a M).
...
with allpartial off\*U.  This is a global flag that immediately takes effect
on all windows overriding the partial\*U settings. It does not change the
...
* **at **[_identifier_][**#|*|%] **command **[**_args_** ... ]**  

Execute a command at other displays or windows as if it had been entered there.
At\*U changes the context (the \`current window' or \`current display'
...
parameter is of the form \`_identifier_*' then identifier is matched against
user names.  The command is executed once for each display of the selected 
user(s). If the first parameter is of the form \`_identifier_%' identifier 
...
If _identifier_ has a \`#' or nothing appended it is matched against 
...
what happened. Permission is checked for initiator of the at\*U command,
...
Permission is checked for the initiator of the at\*U command, not for the
...
(like other\*U) may be called again. In shared windows the command will
be repeated for each attached display. Beware, when issuing toggle commands
like login\*U!
Some commands (e.g. process\*U) require that 
a display is associated with the target windows.  These commands may not work
correctly under at\*U looping over windows.

* **attrcolor **_attrib _**[**_attribute/color-modifier_**]**  
...
modifier is given, the current one is deleted. See the STRING ESCAPES\*U
chapter for the syntax of the modifier. Screen understands two
pseudo-attributes, i\*U stands for high-intensity foreground
color and I\*U for high-intensity background color.
...
* **backtick **_id lifespan autorefresh cmd args..._  
* **backtick **_id_  

Program the backtick command with the numerical id _id_.
The output of such a command is used for substitution of the
%\`\*U string escape. The specified _lifespan_ is the number
...
The _autorefresh_ parameter triggers an
...
If both the _lifespan_ and the _autorefresh_ parameters
...
with the numerical id _id_.

* **bce **[**on**|**off**]  

Change background-color-erase setting. If bce\*U is set to on, all
...
An empty message can be supplied to the bell_msg\*U command to suppress
output of a message line (bell_msg "").
Without parameter, the current message is shown.

* **bind **[**_class_**]** _key_ **[**_command_** [**_args_**]]  

Bind a command to a key.
By default, most of the commands provided by
_screen_
are bound to one or more keys as indicated in the DEFAULT KEY BINDINGS\*U
section, e.\|g. the
command to create a new window is bound to C-c\*U and \*Qc\*U.
The bind\*U command can be used to redefine the key bindings and to
define new bindings.
The _key_ argument is either a single character, a two-character sequence
of the form ^x\*U (meaning \*QC-x\*U), a backslash followed by an octal
number (specifying the ASCII code of the character), or a backslash followed
by a second character, such as \e^\*U or \*Q\e\e\*U.
...
The _command_ argument can be any command listed in this section.

If a command class is specified via the -c\*U option, the key
is bound for the specified class. Use the command\*U command
...
of windows (so that the command usually invoked by C-a C-w\*U
would also be available as C-a space\*U). The next three lines
remove the default kill binding from C-a C-k\*U and \*QC-a k\*U. 
C-a K\*U is then bound to the kill command. Then it
binds C-f\*U to the command \*Qcreate a window with a TELNET
connection to foobar, and bind \*Qescape\*U to the command
that creates an non-login window with a.\|k.\|a. root\*U in slot #9, with
...
makes C-b 0\*U select window 10, \*QC-b 1\*U window 11, etc.
...
makes C-a - 0\*U select window 10, \*QC-a - 1\*U window 11, etc.

* **bindkey **[**-d**] [**-m**] [**-a**] [[**-k**|**-t**]** _string_ **[**_cmd-args_**]]  
...
INPUT TRANSLATION\*U for a list of default key bindings.
...
This key-binding makes ^T\*U an escape character for key-bindings. If
you did the above stuff barfoo\*U binding, you can enter the word
foo\*U by typing \*Q^Tfoo\*U. If you want to insert a \*Q^T\*U
...
Send a break signal for _duration_*0.25 seconds to this window.
For non-Posix systems the time interval may be rounded up to full seconds.
Most useful if a character device is attached to the window rather than 
a shell process (See also chapter WINDOW TYPES\*U). The maximum duration of
...
This command is normally used together with the idle\*U command.

* _**blankerprg** _[_program-args_]  
...
* _**breaktype** _[_tcsendbreak_|_TIOCSBRK_|_TCSBRK_]  

Choose one of the available methods of generating a break signal for
terminal devices. This command should affect the current window only.
But it still behaves identical to defbreaktype\*U. This will be changed in
the future.
Calling breaktype\*U with no parameter displays the break method for the
current window.

* _**bufferfile** _[_exchange-file_]  

Change the filename used for reading and writing with the paste buffer.
If the optional argument to the bufferfile\*U command is omitted, 
the default setting (/tmp/screen-exchange\*U) is reactivated.
...
Change c1 code processing. C1 on\*U tells screen to treat
...
codes and can be changed with the defc1\*U command. 
...
all escapes from the STRING ESCAPES\*U chapter. Screen uses
a default of \`%3n %t'.
...
(_set_ is padded to six characters internally by appending '.'
chars). New windows have "BBBB02" as default charset, unless a
encoding\*U command is active.  
The current setting can be viewed with the info\*U command.

* _**chdir** _[_directory_]  

Change the _current directory_ of
_screen_
...
All windows that are created by means of the screen\*U command
from within .screenrc\*U or by means of \*QC-a : screen ...\*U
or C-a c\*U use this as their default directory.
Without a chdir command, this would be the directory from which
_screen_
was invoked.

Hardcopy and log files are always written to the _window's_ default
directory, _not_ the current directory of the process running in the
window.
...
Allows you to enter .screenrc\*U command lines. Useful 
for on-the-fly modification of key bindings, 
specific window creation and changing settings. Note that the set\*U
...
_screen_,
you may regard C-a esc\*U (copy mode) as its \`Vi command mode'.

* **command **[**-c** _class_**]**  

This command has the same effect as typing the screen escape
character (^A). It is probably only useful for key bindings.
If the -c\*U option is given, select the specified command
class.  See also bind\*U and \*Qbindkey\*U.
...
* **space** or **enter** to set the first or second mark
  respectively. If **mousetrack** is set to \`on', marks can also be set using
  left mouse click.
...
Example: C-a C-[ H 10 j 5 Y\*U will copy lines
11 to 15 into the paste buffer.

The folllowing search keys are defined:

* **/** _Vi_-like search forward.
* **?** _Vi_-like search backward.
* **C-a s** _Emacs_ style incremental search forward.
* **C-r** _Emacs_ style reverse i-search.
...
_screen_
does. Press: **c** or **C** to set the left or right margin
respectively. If no repeat count is given, both default to the current
cursor position.

Example: Try this on a rather full text screen: 

C-a [ M 20 l SPACE c 10 l 5 j C SPACE\*U.
...
C-a [ M 20 l SPACE 10 l 5 j SPACE\*U
...
character with a carriage return character, by issuing a crlf on\*U.

**v** or **V** is for all the
_vi_
users with :set numbers\*U - it toggles the left margin between column 9
and 1. Press 
...
No longer exists, use readreg\*U instead.
...
SCREEN\*U process correctly. Debug output from attacher processes can only
...
Same as the **autonuke** command except that the default setting for new displays is changed. Initial setting is \`off'.
...
* _**defbreaktype** _[_tcsendbreak_|_TIOCSBRK_|_TCSBRK_]  

Choose one of the available methods of generating a break signal for
terminal devices. The preferred methods are 
_tcsendbreak_ and _TIOCSBRK_.
The third, 
_TCSBRK_,
blocks the complete 
_screen_
...
Calling defbreaktype\*U with no parameter displays the current setting.
...
escape\*U except that it is useful multiuser sessions only. In a
multiuser session escape\*U changes the command character of the
calling user, where defescape\*U changes the default command
...
Specifying defflow auto interrupt\*U is the same as the command-line options
**-fa**
and
**-i**.
...
The mode of each newly allocated pseudo-tty is set to _mode_.
_Mode_ is an octal number.
When no defmode\*U command is given, mode 0622 is used.
...
* **defnonblock on**|**off**|**_numsecs_**  

Same as the **nonblock** command except that the default setting for
displays is changed. Initial setting is \`off'.

* **defobuflimit **_limit_  

Same as the **obuflimit** command except that the default setting for new displays is changed. Initial setting is 256 bytes.
...
wrap\*U command (\*QC-a r\*U) or by means of "C-a : wrap on|off".
...
with the
**-r**
option (see also section COMMAND-LINE OPTIONS\*U). The
**-h**
...
when mousetrack\*U is set to on.
T}
_
...
The following is an example of what displays\*U could look like:
    fC
...
displays\*U needs a region size of at least 10 characters wide and 5 characters high in
order to display.

* _**digraph** _[_preset_[_unicode-value_]]  
...
_preset_
is treated as user input, thus one can create an umlaut\*U key.
...
active window to the file .termcap\*U in the user's 
$HOME/.screen\*U directory (or wherever 
_screen_
stores its sockets. See the FILES\*U section below).
...
The option -n\*U may be used to suppress the line feed.
See also sleep\*U.
Echo is also useful for online checking of environment variables.

* **encoding **_enc _**[**_enc_**]**  
...
the terminal type by using the KJ\*U termcap entry.
...
See also defencoding\*U, which changes the default setting of a new
window.

* **escape **_xy_  

Set the command character to _x_ and the character generating a literal
command character (by triggering the meta\*U command) to _y_ (similar
to the -e option).
...
* **eval** _command1_[_command2 ..._]  

Parses and executes each argument as separate command.

* _**exec** _[[_fdpat_]_newcommand _[_args ..._]]  

Run a unix subprocess (specified by an executable path _newcommand_ and its 
...
See the discussion on FLOW-CONTROL\*U later on in this document for full 
...
\`Prev' cycles in the opposite order. See split\*U for more
...
Useful bindings are (h, j, k, and l as in vi)
        ...
        bind t focus top
        bind b focus bottom
Note that **k** is traditionally bound to the _kill_ command.

* **focusminsize [ ( **_width_**|max|_ ) ( **_height_**|max|_ ) ]**  

This forces any currently selected region to be automatically
resized at least a certain _width_ and _height_. All
other surrounding regions will be resized in order to accommodate.
This constraint follows everytime the focus\*U command is
used. The resize\*U command can be used to increase either
dimension of a region, but never below what is set with
focusminsize\*U. The underscore \`_' is a synonym for
**max**. Setting a _width_ and _height_ of \`0 0'
...
default (see also defgr\*U) is not to process GR switching because
otherwise the ISO88591 charset would not work.

* _**group** _[_grouptitle_]  
...
* _**hardcopy** _[_-h_] [_file_]  

Writes out the currently displayed image to the file _file_,
or, if no filename is specified, to _hardcopy.n_ in the
default directory, where _n_ is the number of the current window. 
...
If set to "on", 
_screen_
will append to the "hardcopy.n" files created by the command C-a h\*U, 
otherwise these files are overwritten each time.
Default is \`off'.

* **hardcopydir **_directory_  
...
When firstline/lastline\*U is used, 
_screen_
will reserve the first/last line of the display for
the hardstatus. message\*U uses 
_screen's_
message mechanism and
ignore\*U tells 
_screen_
never to display the hardstatus.
If you prepend the word always\*U to the type (e.g., \*Qalwayslastline\*U), 
_screen_
...
window (settable via ESC]0;&lt;string&gt;^G\*U or \*QESC_&lt;string&gt;ESC\e\*U)
is displayed.  You can customize this to any string you like including
the escapes from the STRING ESCAPES\*U chapter. If you leave out
the argument
_string_,
...
All other characters are ignored. If the -c\*U option is given,
display all bound commands for the specified command class.
See also DEFAULT KEY BINDINGS\*U section.

* **history**  

Usually users work with a shell that allows easy access to previous commands.
For example csh has the command !!\*U to repeat the last command executed. 
_Screen_
allows you to have a primitive way of re-calling the command that
started ...: You just type the first letter of that command, then hit
\`C-a {' and
_screen_
...
Change the window's hardstatus line to the string _status_.

* _**idle** _[_timeout_[_cmd-args_]]  

Sets a command that is run after the specified number of seconds
inactivity is reached. This command will normally be the blanker\*U
...
zero (or the special timeout **off**) disables the timer.
...
the cursor position in the form (column,row)\*U starting with \*Q(1,1)\*U,
the terminal width and height plus the size of the scrollback buffer in lines, 
like in (80,24)+50\*U, the current state of window XON/XOFF flow control
...
The currently active character set (_G0_, _G1_, _G2_,
or _G3_) and in square brackets the terminal character sets that are
currently designated as _G0_ through _G3_ is shown. If the window
is in UTF-8 mode, the string UTF-8\*U is shown instead.

Additional modes depending on the type of the window are displayed at the end of the status line (See also chapter WINDOW TYPES\*U).
...
For system information use the time\*U command.

* **ins_reg** [**key**]  

No longer exists, use paste\*U instead.
...
It is recommended not to use C-a\*U as the
_screen_
escape key or to rebind kill to C-a K\*U.
...
Refer to the commands msgwait\*U and \*Qmsgminwait\*U for fine tuning.
...
* **layout select **[_n|title_]  

Select the desired layout. Either the number or the title can
be specified. Without either specification, _screen_ will
...
* **layout attach **[_title_|**:last**]  

Change or display which layout to reattach back to. The default is
**:last**, which tells _screen_ to reattach back to the last
used layout just before detachment. By supplying a title, You can
instruct _screen_ to reattach to a particular layout regardless
...
* **layout save **[_n|title_]  

Remember the current arrangement of regions. When used, _screen_
will remember the arrangement of vertically and horizontally split
regions. This arrangement is restored when a _screen_ session
is reattached or switched back from a different layout. If the
session ends or the _screen_ process dies, the layout
arrangements are lost. The **layout dump** command should help
in this siutation. If a number
or title is supplied, _screen_ will remember the arrangement of
that particular layout. Without any options, _screen_ will
...
default is **on**, meaning when _screen_ is detached or
...
If autosave is set to **off**, that arrangement will only be
...
a single window. Without either an **on** or **off**, the
current status is displayed on the message line.

* **layout dump **[_filename_]  
...
the default is _layout-dump_, saved in the directory that the
_screen_ process was started in. If the file already exists,
**layout dump** will append to that file. As an example:

    	C-a : layout dump /home/user/.screenrc

will save or append the layout to the user's _.screenrc_ file.
...
screenlog._n_\*U in the window's default directory, where _n_ 
...
* **logfile **_filename_  
* **logfile flush **_secs_  

Defines the name the log files will get. The default is
screenlog.%n\*U. The second form changes the number of seconds
_screen_
...
The default setting (in config.h.in) should be on\*U for a 
_screen_
that runs under suid-root.
Use the deflogin\*U command to change the default login state for new 
windows. Both commands are only present when 
_screen_
has been compiled with utmp support.

* **logtstamp **[**on**|**off**]  
* _**logtstamp after** _[_secs_]  
* **logtstamp string**  
      [_string_]
...
in the default bindkey table. See also bindkey\*U.

* **mapnotnext**  

Like mapdefault, but don't even look in the default bindkey table.

* __maptimeout_ _[_timeout_]  
...
The string is made up of _oldchar_=_newchar_ pairs which are
separated by \`:'. Example: The string B=^B:F=^F\*U will change the 
keys \`C-b' and \`C-f' to the vi style binding (scroll up/down fill page).
This happens to be the default binding for \`B' and \`F'.
The command markkeys h=^B:l=^F:$=^E\*U would set the mode for an emacs-style
binding.
...
The no-op character is \`@' and is used like this: markkeys
@=L=H if you do not want to use the \`H' or \`L' commands any longer.
...
is determined by the defmousetrack\*U command.
...
_screen_.
When you are familiar with the game nethack\*U, you may enjoy the
...
it blocked\*U and stop sending characters to it. If at
some time it restarts to accept characters, screen will unblock
the display and redisplay the updated window contents.

* **number **[[+|-]**n**]  

Change the current window's number. If the given number _n_ is already 
...
_other_ has the same effect as _next_.

* **partial on**|**off**  

Defines whether the display should be refreshed (as with _redisplay_) after
switching to the current window. This command only affects the current window.
To immediately affect all windows use the _allpartial_ command.
Default is \`off', of course.  This default is fixed, as there is currently no 
_defpartial_ command.

* **password **[**crypted_pw**]  

Present a crypted password in your .screenrc\*U file and
_screen_
will ask
...
* _**paste** _[_registers_ [_dest_reg_]]  
...
register to paste.
The paste buffer can be filled with the _copy_, _history_ and 
_readbuf_ commands. 
Other registers can be filled with the _register_, _readreg_ and 
_paste_ commands.
If _paste_ is called with a second argument, the contents of the specified
...
the destination.
Note, that paste\*U uses a wide variety of resources: Whenever a second 
...
Power detach. 
Mainly the same as _detach_, but also sends a HANGUP signal to
...
* _**pow\_detach\_msg** _[_message_]  

The _message_ specified here is output whenever a \`Power detach' was
...
po/pf\*U if it detects an ansi print sequence
**ESC [ 5 i**,
but pipe the output into
_cmd_.
This should normally be a command like lpr\*U or
'cat &gt; /tmp/scrprint'\*U.
**printcmd**
...
Use the empty bind command (as in bind '^\e'\*U) to remove a key binding.

* _**readbuf** _[_encoding_] [_filename_]  
...
See also bufferfile\*U command.

* _**readreg** _[_encoding_] [_register_ [_filename_]]  
...
file into the register, just as _readbuf_ reads the screen-exchange file
...
* **register** _[_**-e**_encoding_]_key-string_  

Save the specified _string_ to the register _key_.
The encoding of the string can be specified via the **-e** option.
See also the paste\*U command.
...
Unlinks the screen-exchange file used by the commands writebuf\*U and 
readbuf\*U. 

* **rendition bell | monitor | silence | so  _attr_  [ _color_ ]**  

Change the way
_screen_
renders the titles of windows that have monitor or bell flags set in caption or hardstatus or windowlist. See the STRING ESCAPES\*U chapter for the syntax of the modifiers.
The default for monitor is currently =b \*U (bold, active colors), for bell \*Q=ub \*U (underline, bold and active colors), and \*Q=u \*U for silence.

* **reset**  

Reset the virtual terminal to its power-on\*U values. Useful when strange
...
ways. By specifying a number _n_ by itself will resize the region by
that absolute amount. You can specify a relative amount by prefixing a
plus \`+' or minus \`-' to the amount, such as adding +_n_ lines
or removing -_n_ lines. Resizing can also be expressed as an absolute
...
See focusminsize\*U if you want to restrict the minimun size a region
can have.

* **screen [_-opts_] [_n_] [_cmd_ [_args_]|**//group**]**  

Establish a new window.
The flow-control options (**-f**, **-fn** and **-fa**),
title (a.\|k.\|a.) option (**-t**), login options (**-l** and **-ln**)
, terminal type option (**-T** &lt;term&gt;), the all-capability-flag (**-a**)
and scrollback option (**-h** &lt;num&gt;) may be specified with each command.
The option (**-M**) turns monitoring on for this window.
The option (**-L**) turns output logging on for this window.
If an optional number _n_ in the range 0..MAXWIN-1 is given,
the window number _n_ is assigned to the newly created window
(or, if this number is already in-use, the next available number).
If a command is specified after screen\*U, this command (with the given
arguments) is started in the window; otherwise, a shell is created.
If **//group** is supplied, a container-type window is created in
which other windows may be created inside it. 

Thus, if your .screenrc\*U contains the lines
...
to the machine foobar (with no flow-control using the title foobar\*U
in window #2) and will write a logfile (screenlog.2\*U) of the telnet 
session.
Note, that unlike previous versions of
_screen_
no additional default window is created when screen\*U commands are 
included in your .screenrc\*U file. When the initialization is completed,
_screen_
switches to the last window specified in your .screenrc file or, if none,
opens a default window #0.

Screen has built in some functionality of cu\*U and \*Qtelnet\*U.
See also chapter WINDOW TYPES\*U.

* **scrollback _num_**  

Set the size of the scrollback buffer for the current windows to _num_ 
lines. The default scrollback is 100 lines.
See also the defscrollback\*U command and use \*Qinfo\*U to view the
current setting. To access and use the contents in the scrollback buffer,
use the copy\*U command.

* **select **[**WindowID**]  

Switch to the window identified by _WindowID_.
...
Thus, the first window can be activated by select 0\*U.
The number of windows is limited at compile-time by the MAXWIN
configuration parameter (which defaults to 40).
There are two special WindowIDs, -\*U selects the
internal blank window and .\*U selects the current window. The
latter is useful if used with screen's -X\*U option.

* **sessionname **[**name**]  

Rename the current session. Note, that for screen -list\*U the
name shows up with the process-id prepended. If the argument name\*U
...
discouraged. Use the -S\*U command-line option if you want to
name a new session.
The default is constructed from the tty and host names.

* _**setenv** _[_var_ [_string_]]  

Set the environment variable _var_ to value _string_.
If only _var_ is specified, the user will be prompted to enter a value.
...
the windows. If setsid is turned _off_, this is not done
...
The default is _on_, of course. This command is probably useful
...
E.g. Bash will not read your ~/.bashrc\*U unless it is a login-shell.
...
entitled TITLES (naming windows)\*U.

* **silence **[**on**|**off**|_sec_]  
...
This command will pause the execution of a .screenrc file for _num_ seconds.
Keyboard activity will end the sleep.
It may be used to give users a chance to read the messages output by echo\*U.
...
_screen_
will make a pause of _msec_ milliseconds after each single character write 
...
Read and execute commands from file _file_. Source commands may
...
* _**sorendition** _[_attr_[_color_]]  
...
Use the remove\*U or the \*Qonly\*U command to delete regions.
Use focus\*U to toggle between regions.
...
some commands (focus\*U and \*Qresize\*U) behave.
...
to use system commands such as cat\*U or \*Qtail -f\*U.
...
status messages to any corner of the screen. **top** is the same as **up**,
...
This is like the paste\*U command but with much less overhead.
...
bindings. See also bindkey\*U.

* _**su **_[_username_ [_password_ [_password2_]]]  
...
_screen_
password as set with the commands acladd\*U or \*Qpassword\*U.
Su\*U may be useful for the 
_screen_
...
for user
**nobody**.
These are detach\*U, \*Qlicense\*U, \*Qversion\*U, \*Qhelp\*U and
displays\*U.
...
_screen_
opens, the $TERM variable is set to screen\*U by default. 
But when no description for screen\*U is installed in the local termcap
or terminfo data base, you set $TERM to - say -
vt100\*U. This won't do much harm, as 
_screen_
is VT100/ANSI compatible.
The use of the term\*U command is discouraged for non-default purpose.
That is, one may want to specify special $TERM settings (e.g. vt100) for the
next screen rlogin othermachine\*U command. Use the command \*Qscreen -T vt100
rlogin othermachine rather than setting and resetting the default.

* **termcap**_ term terminal-tweaks_**[**__window-tweaks__**_]_**  
* **terminfo**_ term terminal-tweaks_**[**__window-tweaks__**_]_**  
* **termcapinfo**_ term terminal-tweaks_**[**__window-tweaks__**_]_**  
...
Each _tweak_ argument contains one or more termcap defines (separated
...
_screen_
understands (see the VIRTUAL TERMINAL\*U
section).
...
Note that we assume \`LP' for all terminal names that start with vt\*U,
...
_screen_.
See the discussion on the VIRTUAL TERMINAL\*U in this manual, and the termcap(5)
...
For window specific information, use info\*U.

If a string is specified, it changes the format of the time report like it is
described in the STRING ESCAPES\*U chapter. Screen uses a default of
"%c:%s %M %d %H%? %l%?".

* **title **[**windowtitle**]  

Set the name of the current window to _windowtitle_. If no name is 
specified,
_screen_
...
encoding is also changed (this should rather be done with screen's -U\*U
option).
See also defutf8\*U, which changes the default setting of a new
window.
...
* _**vbell\_msg** _[_message_]  

Sets the visual bell message. _message_ is printed to the status line if
the window receives a bell character (^G), vbell is set to on\*U, but the 
terminal does not support a visual bell.
The default message is Wuff, Wuff!!\*U.
...
* **width **[**-w**|**-d**] [**_cols_ **[**_lines_**]]  

Toggle the window width between 80 and 132 columns or set it to _cols_ 
columns if an argument is specified. 
This requires a capable terminal and the termcap entries Z0\*U and \*QZ1\*U.
See the termcap\*U command for more information. You can also specify
...
* **windowlist **[**-b**] [**-m**] [**-g**]  
* _**windowlist string** _[_string_]  
* _**windowlist title** _[_title_]  
...
The following keys are used to navigate in windowlist\*U:
...
The table format can be changed with the **string** and
**title** option, the title is displayed as table heading, while
the lines are made by using the string setting. The default
setting is Num Name%=Flags\*U for the title and \*Q%3n %t%=%f\*U
for the lines.
See the STRING ESCAPES\*U chapter for more codes (e.g. color
settings).

Windowlist\*U needs a region size of at least 10 characters wide
...
all the windows that are logged in\*U are marked with a \`$';
...
The optional string parameter follows the STRING ESCAPES\*U format.
...
* **writebuf **[**-e** _encoding_**""**] [**_filename_**]  
...
The filename can be set with the _bufferfile_
command and defaults to /tmp/screen-exchange\*U.
...
by the command writelock off\*U. If the user issues the command 
writelock on\*U he keeps the exclusive write permission while switching
to other windows.
...
* _**zmodem sendcmd** _[_string_]  
* _**zmodem recvcmd** _[_string_]  

Define zmodem support for screen. Screen understands two different
modes when it detects a zmodem request: pass\*U and \*Qcatch\*U.
If the mode is set to pass\*U, screen will relay all data
to the attacher until the end of the transmission is reached.
In catch\*U mode screen acts as a zmodem endpoint and starts
the corresponding rz/sz commands. If the mode is set to auto\*U,
screen will use catch\*U if the window is a tty (e.g. a serial line),
otherwise it will use pass\*U.

You can define the templates screen uses in catch\*U mode
via the second and the third form.

Note also that this is an experimental feature.

* **zombie **[_keys_**[**onerror**]**]  
...
The **kill** command may be used to remove such a window. Pressing the 
...
Optionally you can put the word onerror\*U after the keys. This will cause screen
...
<a name="the-message-line"></a>

# The Message Line

_Screen_
displays informational messages and other diagnostics in a _message line_.
...
the current window by means of the ANSI _Privacy message_
control sequence.
For instance, from within the shell, try something like:

* echo '&lt;esc&gt;^Hello world from window '$WINDOW'&lt;esc&gt;\e\e'

where '&lt;esc&gt;' is an _escape_, '^' is a literal up-arrow,
and '\e\e' turns into a single backslash.


<a name="window-types"></a>
...
_screen_'s
**screen**
command (see also the entry in chapter CUSTOMIZATION\*U). The first
parameter to the 
**screen**
...
* ·  
  If a tty (character special device) name (e.g. /dev/ttya\*U)
  is specified as the first parameter, then the window is directly connected to
  this device. 
  This window type is similar to screen cu -l /dev/ttya\*U.
...
* ·  
  If the first parameter is //telnet\*U, the second parameter is expected to
...
* t  
  ...
  Screen sends the name screen\*U unless instructed otherwise (see also
  the command \`term').
* w  
...
* f  
  flags of the window, see windows\*U for meanings of the various flags
* F  
...
* &gt;  
  ...
  parts with '...'.
* {  
  attribute/color modifier string terminated by the next }\*U
* \`  
...
is [attribute modifier] [color description]\*U. The attribute modifier
...
If you don't like this, prefix the color with a .\*U. If you want
the same behavior for two-letter color descriptions, also prefix them
with a .\*U.  
As a special case, %{-}\*U restores the attributes and colors that
...
* %-Lw%{= BW}%50&gt;%n%f* %t%{-}%+Lw%&lt;  
  ...
  This can be used with hardstatus alwayslastline\*U.
* %?%F%{.R.}%?%3n %t%? [%h]%?  
...
them (useful for the _emacs_ editor, for instance).
The trade-off is that it will take longer for output from a normal\*U
...
_screen_
commands (typically C-a q\*U (xon)
and C-a s\*U (xoff)).
...
**-f**
option or the defflow\*U .screenrc command. Per default the windows
are set to automatic flow-switching.
It can then be toggled between the three states 'fixed on', 'fixed off'
and 'automatic' interactively with the flow\*U command bound to "C-a f".

The automatic flow-switching mode deals with
flow control using the TIOCPKT mode (like rlogin\*U does). If
...
_screen_
with the interrupt\*U
option (add the interrupt\*U flag to the \*Qflow\*U command in
...
with C-a l\*U you would see the version of the output you would
have gotten without interrupt\*U being on.
...
The default name for all shell windows can be set with the shelltitle\*U
command in the .screenrc file, while all other windows are created with
a screen\*U command and thus can have their name set with the
**-t**
option.
Interactively, there is the title-string escape-sequence
(&lt;esc&gt;k_name_&lt;esc&gt;\e) and the title\*U command (C-a A).
...
You can also bind pre-defined names to keys with the title\*U command
...
to _search|name_\*U and arranging to have a null title escape-sequence
output as a part of your prompt.
The _search_ portion specifies an end-of-prompt search string, while
the _name_ portion specifies the default shell name for the window.
If the _name_ ends in a \`:'
_screen_
will add what it believes to be the current command running in the window
to the end of the window's shell name (e.\|g. _name:cmd_\*U).
...
for the _search_ portion of the title.
Once this is set up,
_screen_
...
top\*U command in window 2 named \*Qtop\*U rather than \*Qnice\*U.
...
The window status would show the name trn\*U while the command was
running, and revert to csh\*U upon completion.

* bind R screen -t '% |root:' su

Having this command in your .screenrc would bind the key
sequence C-a R\*U to the \*Qsu\*U command and give it an
auto-title name of root:\*U.
...
Here the user typed the csh history command !em\*U which ran the
previously entered emacs\*U command.
The window status would show root:emacs\*U during the execution
of the command, and revert to simply root:\*U at its completion.
...
for a title when you type C-a o\*U.
The second binding would clear an auto-title's current setting (C-a E).
The third binding would set the current window's title to (unknown)\*U
(C-a u).
...
The escape-sequence &lt;esc&gt;[0000m\*U not only normalizes the character
...
(I used \e134\*U to output a \`\e' because of a bug in bash v1.04).



<a name="the-virtual-terminal"></a>
...
it first looks
for an entry named screen.&lt;term&gt;\*U, where &lt;term&gt; is
the contents of your $TERM variable.
If no such entry exists,
_screen_
tries screen\*U (or \*Qscreen-w\*U if the terminal is wide
(132 cols or more)).
If even this entry cannot be found, vt100\*U is used as a
substitute.
...
_screen_
(named screen.&lt;dumbterm&gt;\*U) in which this capability
...
_screen_
by using the termcap\*U .screenrc command, or
...
terminal screen\*U (and/or \*Qscreen-w\*U) is defined.

Note that 
_screen_
honors the terminfo\*U .screenrc command if the system uses the
...
_lock shift G0_ (_SI_), _lock shift G1_ (_SO_),
_lock shift G2_, _lock shift G3_, _single shift G2_,
and _single shift G3_.
When a virtual terminal is created or reset, the ASCII character
set is designated as _G0_ through _G3_.
When the \`G0' capability is present,
_screen_
evaluates the capabilities
\`S0', \`E0', and \`C0' if present. \`S0' is the sequence the terminal uses
to enable and start the graphics character set rather than _SI_. 
\`E0' is the corresponding replacement for _SO_. \`C0' gives a character
...
Program Command (APC): ESC_&lt;string&gt;ESC\e\*U. As a convenience
for xterm users the sequence ESC]0..2;&lt;string&gt;^G\*U is
also accepted.
...
_screen_.
(V)\*U and \*Q(A)\*U indicate VT100-specific and ANSI- or
...
* **ESC ] 83 ; cmd ^G**	(A)  
  ...
  compiled into screen. The pseudo-user :window:\*U is used to
  check the access control list. Use addacl :window: -rwx #?\*U to
...
* **ESC [ Ps** ;**...**; **Ps m**  
  Select Graphic Rendition
    ...
    * Ps = **2**	(A)  
      Faint
    * Ps = **3**	(A)  
      _Standout_ Mode (ANSI: Italicized)
    * Ps = **4**  
      Underlined
    * Ps = **5**  
      Blinking
    ...
    * Ps = **22**	(A)  
      Normal Intensity
    * Ps = **23**	(A)  
      _Standout_ Mode off (ANSI: Italicized off)
    * Ps = **24**	(A)  
      Not Underlined
    * Ps = **25**	(A)  
      Not Blinking
...
style escape sequence. _Screen_ has a very flexible way of doing
...
(see also command **stuff** in the command table).
...
_Screen_ will insert the correct binding after each
reattach. See the **bindkey** command for further details on the
syntax and examples.
...
<a name="special-terminal-capabilities"></a>
...
* **WS**_	(str)_  
  ...
  arguments. _SunView(tm)_ example: '\eE[8;%d;%dt'.
* **NF**_	(bool)_  
...
_Screen_ has a powerful mechanism to translate characters to arbitrary
...
A _&lt;charset-mapping&gt;_ tells 
_screen_
how to map characters
in font _&lt;designator&gt;_ ('B': Ascii, 'A': UK, 'K': German, etc.)
to strings. Every _&lt;mapping&gt;_ describes to what string a single
...
of '%' in _&lt;template&gt;_ gets substituted with the _&lt;template-arg&gt;_
specified together with the character. If your strings are not
similar at all, then use '%' as a template and place the full
string in _&lt;template-arg&gt;_. A quoting mechanism was added to make
...
whenever 
_screen_
switches to the corresponding _&lt;designator&gt;_. In this
...
* SHELL  
  Default shell program for opening windows (default /bin/sh\*U).
  See also shell\*U .screenrc command.
* STY  
...
by Amadeusz Slawinski &lt;[amade@asmblr.net](mailto:amade@asmblr.net)&gt;
and Alexander Naumov &lt;[alexander_naumov@opensuse.org](mailto:alexander_naumov@opensuse.org)&gt;.
...
* ·  
  ...
  each window.
  Special permission may also be required to write the file /etc/utmp\*U.
* ·  
  Entries in /etc/utmp\*U are not removed when
  _screen_
...
* ·  
  Both breaktype\*U and \*Qdefbreaktype\*U change the break generating
