...
**conntrackd.conf** is the main configuration file for the
**conntrackd(8)** daemon. It is loaded by calling \`conntrackd -C
conntrackd.conf'.
...
_http://conntrack-tools.netfilter.org/manual.html_.
...
This top-level section defines how **conntrackd(8)** should handle
synchronization with other cluster nodes.

There are 3 main synchronization modes or protocols: **NOTRACK**, **ALARM**
and **FTFW**.

There are 3 transport protocols as well: **TCP**, **Multicast**
and **UDP**.
...
In this synchronization mode you may configure **ResendQueueSize**,
**CommitTimeout**, **PurgeTimeout**, **ACKWindowSize** ,
**DisableExternalCache** and **StartupResync**.
...
* **PurgeTimeout &lt;seconds&gt;**  
  ...
  \`_conntrackd -t command_' is invoked in the script. This command schedules
...
* **ACKWindowSize &lt;value&gt;**  
  ...
  **conntrackd(8)** has to handle more control messages. On the other hand, if
...
* **DisableExternalCache &lt;on|off&gt;**  
  ...
  consumption. You need a **Linux kernel &gt;= 2.6.29** to use this feature.
  
  If you are installing **conntrackd(8)** for first time, please read the user
...
In this synchronization mode you may configure **RefreshTime**,
**CacheTimeout**, **CommitTimeout** and **PurgeTimeout**.
...
In this synchronization mode you may configure **DisableInternalCache**,
**DisableExternalCache**, **CommitTimeout**, **PurgeTimeout** and
**StartupResync**.
...
This section indicates to **conntrackd(8)** to use multicast as transport
...
The _Default_ keyword indicates that this interface will be selected as the
...
* **SndSocketBuffer &lt;number&gt;**  
  ...
  available at **/proc/sys/net/core/wmem\_default**.
  ...
  Note: The **NOTRACK** protocol is best effort, it is really recommended
...
* **RcvSocketBuffer &lt;number&gt;**  
  ...
  buffer is available at **/proc/sys/net/core/rmem\_default**.
...
This section indicates to **conntrackd(8)** to use UDP as transport
mechanism between nodes of the firewall cluster.

As in the **Multicast** configuration, you may especify several fail-over
dedicated links using the _Default_ keyword.
...
* **Systemd &lt;on|off&gt;**  
  Enable **systemd(1)** runtime support if **conntrackd(8)** is compiled
...
* **Nice &lt;value&gt;**  
  ...
  future. Please note that you can run **nice(1)** and **renice(1)**
  externally. Also note that **conntrackd(8)** now uses by default a RT
  scheduler.
  
* **HashSize &lt;value&gt;**  
  Number of buckets in the cache hashtable. The bigger it is, the closer it gets
  to _O(1)_ at the cost of consuming more memory. Read some documents about
...
* **HashLimit &lt;value&gt;**  
  ...
  **/proc/sys/net/netfilter/nf\_conntrack\_max** since the daemon may keep some
...
* **Syslog &lt;on|off|facility&gt;**  
  ...
  in the **Stats** section, otherwise you'll get a warning message.
...
* **NetlinkBufferSize &lt;value&gt;**  
  ...
  default buffer size value in **/proc/net/core/rmem\_default** is used. This
  default value is usually around **100 Kbytes** which is fairly small for
...
* **NetlinkEventsReliable &lt;on|off&gt;**  
  ...
  set on this clause, it is a good idea to set off **NetlinkOverrunResync**.
...
* **PollSecs &lt;seconds&gt;**  
  ...
  This clause tells **conntrackd(8)** to dump the states in the kernel every N
...
Unix socket configuration. This socket is used by **conntrackd(8)** to listen
to external commands like \`_conntrackd -k_' or \`_conntrackd -n_'.
...
You can select if **conntrackd(8)** filters the event messages from
...
* **Protocol &lt;policy&gt; { &lt;protocols list&gt; }**  
  ...
  Protocols are: **TCP**, **SCTP**, **DCCP**, **UDP**, **ICMP** and
  **IPv6-ICMP**.
  
  The **ICMP** and **IPv6-ICMP** protocols require a
  **Linux kernel &gt;= 2.6.31**.
...
* **Address &lt;policy&gt; { &lt;addresses list&gt; }**  
  ...
  You may specify several **IPv4\_address** and/or **IPv6\_address**
...
* **State &lt;policy&gt; { &lt;states list&gt; }**  
  ...
  The existing TCP states are: **SYN\_SENT**, **SYN\_RECV**, **ESTABLISHED**,
  **FIN\_WAIT**, **CLOSE\_WAIT**, **LAST\_ACK**, **TIME\_WAIT**,
  **CLOSED** and **LISTEN**.
...
Select a different scheduler for the daemon, you can select between **RR**
and **FIFO** and the process priority.

Using a RT scheduler reduces the chances to overrun the Netlink buffer and
**conntrackd(8)** uses by default **RR** unless **FIFO** is selected.
See **sched\_setscheduler(2)** for more information.
...
* **Priority &lt;value&gt;**  
  ...
  Default: 99 (as returned by **sched\_get\_priority\_max(2)** for **SCHED\_RR**)
...
This top-level section indicates **conntrackd(8)** to work as a statistic
...
* **NetlinkEventsReliable &lt;on|off&gt;**  
  ...
  **NetlinkOverrunResync**. This requires **Linux kernel &gt;= 2.6.31**.
  
  Default is off.
  
* **Syslog &lt;on|off|facility&gt;**  
  Enable connection logging via Syslog.
  If you set the facility, use the same as in the **General** section,
...
This top-level section indicates **conntrackd(8)** to inject user-space
...
* **Policy &lt;name&gt; { }**  
  ...
  **ExpectMax &lt;number&gt;** (maximum number of simultaneous expectations)
  and **ExpecTimeout &lt;seconds&gt;** (maximum living time for one expectation).
...
<a name="stats-example"></a>

### STATS EXAMPLE

This configuration example tells **conntrackd(8)** to work as a stats
collector.
...
<a name="sync-example-1"></a>

### SYNC EXAMPLE 1

This example configures synchronization in **FTFW** mode with **Multicast**
transport.

It includes common general configuration as well.

Note: this is one of the recommended setups for **conntrackd(8)** in a
...
<a name="sync-example-2"></a>

### SYNC EXAMPLE 2


This example configures synchronization in **NOTRACK** mode with **TCP**
transport.
...
This manual page was written by Arturo Borrero Gonzalez &lt;[arturo@debian.org](mailto:arturo@debian.org)&gt;
based on the conntrackd tarball config examples.

Please send bug reports to &lt;[netfilter-devel@lists.netfilter](mailto:netfilter-devel@lists.netfilter).org&gt;. Subscription is required.
