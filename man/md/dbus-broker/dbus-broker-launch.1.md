# dbus-broker-launch(1) - Launcher for D-Bus Message Brokers
...
the command-line, or the default is picked from _/usr/share/dbus-1/_. Nearly
...
* **--config-file=**_PATH_  
  ...
  _/usr/share/dbus-1/{system,session}.conf_)
* **--scope=**_SCOPE_  
  select scope to run in (one of: _system_,
  _user_; **Default**: _system_)
  .UNINDENT
...
to activate services. In case of _system_, the launcher will use the system
instance of systemd. In case of _user_, the user instance is used instead.
...
Additional _&lt;listen&gt;%path%&lt;/listen&gt;_ attributes in the configuration are
ignored.
...
privileges are needed. If the _&lt;user&gt;%user%&lt;/user&gt;_ configuration attribute is
