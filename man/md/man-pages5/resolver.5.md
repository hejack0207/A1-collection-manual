...
* **nameserver** Name server IP address  
  ...
  Up to
  **MAXNS**
  (currently 3, see _&lt;resolv.h&gt;_) name servers may be listed,
  one per keyword.
...
* **search** Search list for host-name lookup.  
  ...
  following the _search_ keyword with spaces or tabs separating
  the names.
...
* **ndots:**_n_  
  ...
  **res_query**(3)
  (see
  **resolver**(3))
  before an _initial absolute query_ will be made.
  The default for
  _n_ is 1, meaning that if there are any dots in a name, the name
  will be tried first as an absolute name before any _search list_
...
The _domain_ and _search_ keywords are mutually exclusive.
...
The _options_ keyword of a system's _resolv.conf_ file can be
