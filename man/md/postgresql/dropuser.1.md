...
```
.HP \w'dropuser&nbsp;'u dropuser [connection-option...] [option...] [username]
```
...
SQL
command
DROP ROLE (**DROP\_ROLE**(7)). There is no effective difference between dropping users via this utility and via other methods for accessing the server.
...
**createuser**(1), DROP ROLE (**DROP\_ROLE**(7))
