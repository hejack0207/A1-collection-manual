...
NO SCROLL
SCROLL
specifies that the cursor can be used to retrieve rows in a nonsequential fashion (e.g., backward). Depending upon the complexity of the querys execution plan, specifying
SCROLL
might impose a performance penalty on the querys execution time.
NO SCROLL
...
Section&nbsp;37.7). When a previously fetched row is re-fetched, the functions might be re-executed, perhaps leading to results different from the first time. One workaround for such cases is to declare the cursor
WITH HOLD
