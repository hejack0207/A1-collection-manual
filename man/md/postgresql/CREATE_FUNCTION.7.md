...
_table\_name_._column\_name_%TYPE. Using this feature can sometimes help make a function independent of changes to the definition of a table.
...
The type of a column is referenced by writing
_table\_name_._column\_name_%TYPE.
...
STRICT
CALLED ON NULL INPUT
(the default) indicates that the function will be called normally when some of its arguments are null. It is then the function authors responsibility to check for null values if necessary and respond appropriately.
...
PARALLEL
PARALLEL UNSAFE
indicates that the function cant be executed in parallel mode and the presence of such a function in an SQL statement forces a serial execution plan. This is the default.
PARALLEL RESTRICTED
...
**SET LOCAL**
command executed inside the function for the same variable are restricted to the function: the configuration parameters prior value is still restored at function exit. However, an ordinary
**SET**
command (without
LOCAL) overrides the
SET
...
command. The string
_link\_symbol_
is the functions link symbol, that is, the name of the function in the C language source code. If the link symbol is omitted, it is assumed to be the same as the name of the SQL function being defined. The C names of all functions must be different, so you must give overloaded C functions different C names (for example, use the argument types as part of the C names).
...
The full
SQL
type syntax is allowed for declaring a functions arguments and return value. However, parenthesized type modifiers (e.g., the precision field for type
...
.\}
    CREATE FUNCTION dup(in int, out f1 int, out f2 text)
        AS $$ SELECT $1, CAST($1 AS text) ||  is text*(Aq $$
        LANGUAGE SQL;
...
_search\_path_. This function illustrates safe usage:
...
.\}
    ...
    $$  LANGUAGE plpgsql
        SECURITY DEFINER
        -- Set a secure search_path: trusted schema(s), then pg_temp*(Aq.
        SET search_path = admin, pg_temp;
.if n \{.RE
.\}

This functions intention is to access a table
...
_argname_. But only the first way is standard-compliant.
...
ALTER FUNCTION (**ALTER\_FUNCTION**(7)), DROP FUNCTION (**DROP\_FUNCTION**(7)), **GRANT**(7), **LOAD**(7), **REVOKE**(7)
