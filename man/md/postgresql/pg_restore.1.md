...
```
.HP \w'pg_restore&nbsp;'u pg_restore [connection-option...] [option...] [filename]
```
...
**--create**,
pg_restore
also restores the databases comment if any, and any configuration variable settings that are specific to this database, that is, any
**ALTER DATABASE ... SET ...**
and
...
pg_restore
— those that load data, create indexes, or create constraints — concurrently, using up to
_number-of-jobs_
...
_list-file_, and restore them in the order they appear in the file. Note that if filtering switches such as
**-n**
or
**-t**
are used with
...
Restore the named function only. Be careful to spell the function name and arguments exactly as they appear in the dump files table of contents. Multiple functions may be specified with multiple
**-P**
switches.
...
**BEGIN**/**COMMIT**). This ensures that either all the commands complete successfully, or no changes are applied. This option implies
**--exit-on-error**.
...
**pre-data**,
**data**, or
**post-data**. This option can be specified more than once to select multiple sections. The default is to restore all sections.
...
Notice we dont use
**-C**, and instead connect directly to the database to be restored into. Also note that we clone the new database from
template0
not
...
**pg\_dump**(1), **pg\_dumpall**(1), **psql**(1)
