...
clause causes
**UPDATE**
to compute and return value(s) based on each row actually updated. Any expression using the tables columns, and/or columns of other tables mentioned in
FROM, can be computed. The new (post-update) values of the tables columns are used. The syntax of the
RETURNING
...
_table\_name_. The column name can be qualified with a subfield name or array subscript, if needed. Do not include the tables name in the specification of a target column — for example,
...
**UPDATE**s target table. Note that
WHERE CURRENT OF
...
.if n \{.RS 4
.\}
    UPDATE films SET kind = Dramatic*(Aq WHERE kind = *(AqDrama*(Aq;
.if n \{.RE
.\}
...
.\}
    UPDATE weather SET temp_lo = temp_lo+1, temp_hi = temp_lo+15, prcp = DEFAULT
      WHERE city = San Francisco*(Aq AND date = *(Aq2003-07-03*(Aq;
.if n \{.RE
.\}
...
.\}
    UPDATE employees SET sales_count = sales_count + 1 FROM accounts
      WHERE accounts.name = Acme Corporation*(Aq
...
.\}
    ...
    -- other operations
    SAVEPOINT sp1;
    INSERT INTO wines VALUES(Chateau Lafite 2003*(Aq, *(Aq24*(Aq);
    -- Assume the above fails because of a unique key violation,
    -- so now we issue these commands:
    ROLLBACK TO sp1;
    UPDATE wines SET stock = stock + 24 WHERE winename = Chateau Lafite 2003*(Aq;
...
.if n \{.RS 4
.\}
    UPDATE films SET kind = Dramatic*(Aq WHERE CURRENT OF c_films;
.if n \{.RE
.\}
...
row constructor
or a sub-SELECT. An individual columns updated value can be specified as
DEFAULT
