...
```
    COPY table_name [ ( column_name [, ...] ) ]
        FROM { filename*(Aq | PROGRAM *(Aqcommand*(Aq | STDIN }
        [ [ WITH ] ( option [, ...] ) ]
        [ WHERE condition ]
    
    COPY { table_name [ ( column_name [, ...] ) ] | ( query ) }
        TO { filename*(Aq | PROGRAM *(Aqcommand*(Aq | STDOUT }
        [ [ WITH ] ( option [, ...] ) ]
    ...
    where option can be one of:
        ...
        FORMAT format_name
        FREEZE [ boolean ]
        DELIMITER delimiter_character*(Aq
        NULL null_string*(Aq
        HEADER [ boolean ]
        QUOTE quote_character*(Aq
        ESCAPE escape_character*(Aq
        FORCE_QUOTE { ( column_name [, ...] ) | * }
        FORCE_NOT_NULL ( column_name [, ...] )
        FORCE_NULL ( column_name [, ...] )
        ENCODING encoding_name*(Aq
...
**COPY FROM**, each field in the file is inserted, in order, into the specified column. Table columns not specified in the
**COPY FROM**
...
**COPY FROM**, the input is read from standard output of the command, and in
...
CSV
format. You might prefer an empty string even in text format for cases where you dont want to distinguish nulls from empty strings. This option is not allowed when using
binary
format.
.if n \{.sp
.\}
...
_encoding\_name_. If this option is omitted, the current client encoding is used. See the Notes below for more details.
...
SELECT * FROM ONLY _table_. The syntax
COPY (SELECT * FROM _table_) TO ...
...
**COPY TO**, but for
**COPY FROM**
you do have the option of reading from a file specified by a relative path. The path will be interpreted relative to the working directory of the server process (normally the clusters data directory), not the client\*(Aqs working directory.

Executing a command with
PROGRAM
might be restricted by the operating systems access control mechanisms, such as SELinux.
...
_DateStyle_. To ensure portability to other
PostgreSQL
...
When the
text
format is used, the data read or written is a text file with one line per table row. Columns in a row are separated by the delimiter character. The column values themselves are strings generated by the output function, or acceptable to the input function, of each attributes data type. The specified null string is used in place of columns that are null.
**COPY FROM**
...
**COPY TO**
will terminate each row with a Unix-style newline (“\en”). Servers running on Microsoft Windows instead output carriage return/newline (“\er\en”), but only for
**COPY**
...
PostgreSQLs standard text format, it produces and recognizes the common CSV escaping mechanism.
...
and
***recv**
functions for each columns data type (typically these functions are found in the
src/backend/utils/adt/
directory of the source distribution).

If OIDs are included in the file, the OID field immediately follows the field-count word. It is a normal field except that its not included in the field-count. Note that oid system columns are not supported in current versions of
PostgreSQL.
...
The file trailer consists of a 16-bit integer word containing -1. This is easily distinguished from a tuples field-count word.
...
.if n \{.RS 4
.\}
    COPY country TO STDOUT (DELIMITER |*(Aq);
.if n \{.RE
.\}
...
.if n \{.RS 4
.\}
    COPY country FROM /usr1/proj/bray/sql/country_data*(Aq;
.if n \{.RE
.\}

To copy into a file just the countries whose names start with A\*(Aq:

.if n \{.RS 4
.\}
    COPY (SELECT * FROM country WHERE country_name LIKE A%*(Aq) TO *(Aq/usr1/proj/bray/sql/a_list_countries.copy*(Aq;
.if n \{.RE
.\}
...
.if n \{.RS 4
.\}
    COPY country TO PROGRAM gzip > /usr1/proj/bray/sql/country_data.gz*(Aq;
.if n \{.RE
.\}
...
.\}
    COPY table_name [ ( column_name [, ...] ) ]
        FROM { filename*(Aq | STDIN }
        [ [ WITH ]
              [ BINARY ]
              [ DELIMITER [ AS ] delimiter_character*(Aq ]
              [ NULL [ AS ] null_string*(Aq ]
              [ CSV [ HEADER ]
                    [ QUOTE [ AS ] quote_character*(Aq ]
                    [ ESCAPE [ AS ] escape_character*(Aq ]
                    [ FORCE NOT NULL column_name [, ...] ] ] ]
    
    COPY { table_name [ ( column_name [, ...] ) ] | ( query ) }
        TO { filename*(Aq | STDOUT }
        [ [ WITH ]
              [ BINARY ]
              [ DELIMITER [ AS ] delimiter_character*(Aq ]
              [ NULL [ AS ] null_string*(Aq ]
...
.\}
    COPY [ BINARY ] table_name
        FROM { filename*(Aq | STDIN }
        [ [USING] DELIMITERS delimiter_character*(Aq ]
        [ WITH NULL AS null_string*(Aq ]
