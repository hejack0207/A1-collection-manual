...
IN SCHEMA
is not allowed when setting privileges for schemas, since schemas cant be nested.

_role\_name_
The name of an existing role to grant or revoke privileges for. This parameter, and all the other parameters in
_abbreviated\_grant\_or\_revoke_, act as described under
**GRANT**(7)
or
**REVOKE**(7), except that one is setting permissions for a whole class of objects rather than specific named objects.
