...
PID
(supplied in the notification event message) is the same as ones own session\*(Aqs
PID
(available from
...
**pg\_notify**(text, text). The function takes the channel name as the first argument and the payload as the second. The function is much easier to use than the
**NOTIFY**
...
.\}
    ...
    NOTIFY virtual, This is the payload*(Aq;
    Asynchronous notification "virtual" with payload "This is the payload" received from server process with PID 8448.
    
    LISTEN foo;
    SELECT pg_notify(fo*(Aq || *(Aqo*(Aq, *(Aqpay*(Aq || *(Aqload*(Aq);
