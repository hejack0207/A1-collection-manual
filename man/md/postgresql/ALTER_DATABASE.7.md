...
_value_
Set this databases session default for the specified configuration parameter to the given value. If
_value_
is
DEFAULT
or, equivalently,
...
SET FROM CURRENT
saves the sessions current value of the parameter as the database-specific value.
...
ALTER ROLE (**ALTER\_ROLE**(7)). Role-specific settings override database-specific ones if there is a conflict.
...
CREATE DATABASE (**CREATE\_DATABASE**(7)), DROP DATABASE (**DROP\_DATABASE**(7)), **SET**(7), CREATE TABLESPACE (**CREATE\_TABLESPACE**(7))
