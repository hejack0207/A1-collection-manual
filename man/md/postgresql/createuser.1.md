...
```
.HP \w'createuser&nbsp;'u createuser [connection-option...] [option...] [username]
```
...
SQL
command
CREATE ROLE (**CREATE\_ROLE**(7)). There is no effective difference between creating users via this utility and via other methods for accessing the server.
...
.\}
    ...
    CREATE ROLE joe PASSWORD md5b5f5ba1a423792b526f799ae4eb3d59e*(Aq SUPERUSER CREATEDB CREATEROLE INHERIT LOGIN;
.if n \{.RE
.\}

In the above example, the new password isnt actually echoed when typed, but we show what was typed for clarity. As you see, the password is encrypted before it is sent to the client.

<a name="see-also"></a>

# See Also

**dropuser**(1), CREATE ROLE (**CREATE\_ROLE**(7))
