...
_name_. The generator will be owned by the user issuing the command.
...
last_value
field of the sequence shows the last value allocated by any session. (Of course, this value might be obsolete by the time its printed, if other sessions are actively doing
**nextval**
calls.)
...
OWNED BY _table\_name_._column\_name_  
OWNED BY NONE
The
OWNED BY
...
_cache_
setting greater than one is used for a sequence object that will be used concurrently by multiple sessions. Each session will allocate and cache successive sequence values during one access to the sequence object and increase the sequence objects
last_value
...
**nextval**=11 before session A has generated
...
.if n \{.RS 4
.\}
    INSERT INTO distributors VALUES (nextval(serial*(Aq), *(Aqnothing*(Aq);
.if n \{.RE
.\}
...
.\}
    BEGIN;
    COPY distributors FROM input_file*(Aq;
    SELECT setval(serial*(Aq, max(id)) FROM distributors;
    END;
.if n \{.RE
.\}
...
* ·  
  ...
  **nextval()**
  function instead of the standards
  **NEXT VALUE FOR**
  expression.
...
ALTER SEQUENCE (**ALTER\_SEQUENCE**(7)), DROP SEQUENCE (**DROP\_SEQUENCE**(7))
