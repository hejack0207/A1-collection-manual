...
“extensions”, and should therefore be installed with
CREATE EXTENSION (**CREATE\_EXTENSION**(7))
not
...
CREATE EXTENSION _langname_ FROM unpackaged.
...
pg_pltemplate, but the languages ownership and permissions settings do not change, and any existing functions written in the language are assumed to still be valid. In addition to the normal privilege requirements for creating a language, the user must be superuser or owner of the existing language. The
REPLACE
...
_call\_handler_
is the name of a previously registered function that will be called to execute the procedural languages functions. The call handler for a procedural language must be written in a compiled language such as C with version 1 call convention and registered with
PostgreSQL
...
_inline\_handler_
is the name of a previously registered function that will be called to execute an anonymous code block (**DO**(7)
...
**DO**
commands internal representation, and it will typically return
...
Use
DROP LANGUAGE (**DROP\_LANGUAGE**(7))
...
ALTER LANGUAGE (**ALTER\_LANGUAGE**(7)), CREATE FUNCTION (**CREATE\_FUNCTION**(7)), DROP LANGUAGE (**DROP\_LANGUAGE**(7)), **GRANT**(7), **REVOKE**(7)
