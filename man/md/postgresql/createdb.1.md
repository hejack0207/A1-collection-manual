...
```
.HP \w'createdb&nbsp;'u createdb [connection-option...] [option...] [dbname&nbsp;[description]]
```
...
SQL
command
CREATE DATABASE (**CREATE\_DATABASE**(7)). There is no effective difference between creating databases via this utility and via other methods for accessing the server.
...
CREATE DATABASE (**CREATE\_DATABASE**(7)); see there for more information about them.
...
**dropdb**(1), CREATE DATABASE (**CREATE\_DATABASE**(7))
