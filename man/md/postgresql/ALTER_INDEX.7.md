...
ALTER [ COLUMN ] _column\_number_ SET STATISTICS _integer_
...
**ALTER INDEX OWNER**
variant, but this is now ignored (with a warning). An index cannot have an owner different from its tables owner. Changing the table\*(Aqs owner automatically changes the index as well.
...
CREATE INDEX (**CREATE\_INDEX**(7)), **REINDEX**(7)
