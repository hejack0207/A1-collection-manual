...
<a name="grant-on-database-objects"></a>
...
ALL PRIVILEGES
Grant all of the privileges available for the objects type. The
PRIVILEGES
...
A user may perform
**SELECT**,
**INSERT**, etc. on a column if they hold that privilege for either the specific column or its whole table. Granting the privilege at the table level and then revoking it for one column will not do what one might wish: the table-level grant is unaffected by a column-level operation.
...
See
Section&nbsp;5.7
for more information about specific privilege types, as well as how to inspect objects privileges.
...
PostgreSQL
treats the owners privileges as having been granted by the owner to themselves; therefore they can revoke them too. In the SQL standard, the owner\*(Aqs privileges are granted by an assumed entity
“_SYSTEM”. Not being
...
**GRANT**. PostgreSQL only supports it when granting role membership, and even then only superusers may use it in nontrivial ways.
...
**REVOKE**(7), ALTER DEFAULT PRIVILEGES (**ALTER\_DEFAULT\_PRIVILEGES**(7))
