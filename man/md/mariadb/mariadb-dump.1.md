...
```
.HP \w'mysqldump&nbsp;[options]&nbsp;[db_name&nbsp;[tbl_name&nbsp;...]]&nbsp;'u mysqldump [options] [db_name [tbl_name ...]]
```
...
_debug\_options_
string is
\'d:t:o,_file\_name_\'. The default value is
...
Also read groups with a suffix of _str_. For example, since mysqldump normally reads the [client] and [mysqldump] groups, --defaults-group-suffix=x would cause it to also read the groups [mysqldump_x] and [client_x].
...
INSERT
statements with
/*!40000 ALTER TABLE _tbl\_name_ DISABLE KEYS */;
and
/*!40000 ALTER TABLE _tbl\_name_ ENABLE KEYS */;
...
* ·  
  ...
  **--ignore-table=****db\_name.tbl\_name**
...
the **--dump-slave** option for a slave dump.
...
**--lock-tables**
**--quick**
**--set-charset**. It should give you a fast dump operation and produce a dump file that can be reloaded into a MariaDB server quickly.

_The __**--opt** option is enabled by default. Use __**--skip-opt** to disable it._
...
Add
SET NAMES _default\_character\_set_
...
* ·  
  .\}
  stats - statistics tables, InnoDB and Engine Independent Table Statistics (EITS), are dumped as **REPLACE INTO** (or **INSERT IGNORE** if **--insert-into** is specified) statements without (re)creating tables.
.ie n \{\h'-04'·\h'+03'\c
.\}
.el \{.sp -1
...
* ·  
  .\}
  timezones - timezone related system tables dumped as **REPLACE INTO** (or **INSERT IGNORE** if **--insert-into** is specified) statements without (re)creating tables.

The format of the output is affected by **--replace** and **--insert-into**. The **--replace** option will output **CREATE OR REPLACE**
forms of SQL, and also **DROP IF EXISTS** prior to **CREATE**, if a **CREATE OR REPLACE** option isn't available.
...
For stats, and timezones, **--replace** and **--insert-into** have the usual effects.
...
T}:T{
&lt;field name="_column\_name_" xsi:nil="true" /&gt;
T}
T{
\'\' (_empty string_)
T}:T{
&lt;field name="_column\_name_"&gt;&lt;/field&gt;
T}
T{
\'NULL\' (_string value_)
T}:T{
&lt;field name="_column\_name_"&gt;NULL&lt;/field&gt;
T}
.TE
