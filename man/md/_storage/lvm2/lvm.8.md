...
**lvmchange**, **lvmsadc**, **lvmsar**, **pvdata**.
For performance metrics, use **dmstats**(8) or to manipulate the kernel
device-mapper driver used by LVM2 directly, use **dmsetup**(8).
...
configuration setting **metadata/record\_lvs\_history** is enabled then an LV
...
_/dev/_ at the time of creation, nor can it be called '.' or '..'.
...
configuration parameter **allocation/cling\_tag\_list** is defined, then two
...
single **lvcreate**(8) command.  The linear and striped logical
volume types are an example of this.  Other logical volume types may
require more than one command to create.  The cache (**lvmcache**(7))
...
* **HOME**  
  Directory containing _.lvm\_history_ if the internal readline
  shell is invoked.
* **LVM_OUT_FD**  
...
* **LVM_COMMAND_PROFILE**  
  ...
  is overriden by direct use of **--commandprofile** command line option.
* **LVM_RUN_BY_DMEVENTD**  
