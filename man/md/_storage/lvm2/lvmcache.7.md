...
An **lvm**(8) **cache** Logical Volume (LV) uses a small and
...
LVM refers to the small fast LV as a **cache pool LV**.  The large
slow LV is called the **origin LV**.  Due to requirements from dm-cache
(the kernel driver), LVM further splits the cache pool LV into two
devices - the **cache data LV** and **cache metadata LV**.  The cache
...
<a name="split-a-cache-pool-lv-off-of-a-cache-lv"></a>
...
<a name="removing-a-cache-pool-lv-without-removing-its-linked-origin-lv"></a>
...
<a name="removing-a-cache-lv-both-origin-lv-and-the-cache-pool-lv"></a>
...
<a name="tolerate-device-failures-in-a-cache-pool-lv"></a>
...
<a name="create-a-new-cache-lv-without-an-existing-origin-lv"></a>
...
<a name="single-step-cache-pool-lv-creation"></a>
...
<a name="convert-existing-lvs-to-cache-types"></a>
...
<a name="cache-metadata-formats"></a>
...
Format **2** has better performance; it is more compact, and stores
dirty bits in a separate btree, which improves the speed of shutting down
the cache.
With **auto**, lvm selects the best option provided by the current
