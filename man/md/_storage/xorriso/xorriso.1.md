...
.SS
**Session model:**  
Unlike other filesystems, **ISO 9660** (aka **ECMA-119**)
...
**modifying** which produces a completely new ISO image from the old
...
The first session of a **xorriso** run begins by the definition of
...
**xorriso** will write onto them only if command -close is set to "on".  
...
-outdev are both set to different drives, then **blind growing** is
...
employed if a strict distinction between ISO formatter **xorriso**
...
**xorriso** is able to create or maintain an El Torito object which
...
hard-disk-like media. Several options install a **MBR**
...
An **APM** (Apple Partition Map) marks the HFS+ partition.
...
For now, only **xorriso** is able to retrieve those ACLs.
...
**-xattr**.  
As with ACL, currently only **xorriso** is able to retrieve xattr
...
terminated by either the **list delimiter**, occur at the end of
...
differs from the usual shell parsers. In **xorriso**, a quotation mark
...
type can enclose the marks of the other type. A trailing backslash nbsp;outside
...
the bash shell. Whether it is available in **xorriso** depends on the
availability
of package readline-dev at the time when **xorriso** was built from
its sourcecode.  
...
* **-drive\_class** "harmless"|"banned"|"caution"|"clear_list" disk_pattern  
  ...
  accepted by **xorriso** but rather lead to a FAILURE event.
...
* **-drive\_access** "exclusive"|"shared":"unrestricted"|"readonly"  
...
* **-scsi\_dev\_family** "default"|"sr"|"scd"|"sg"  
  GNU/Linux specific:  
  ...
  /etc/opt/xorriso/rc.
* **-grow\_blindly** "off"|predicted_nwa  
...
* **-read\_speed** code|number[k|m|c|d|b]  
...
* **-read\_fs** "any"|"norock"|"nojoliet"|"ecma119"   
...
* **-assert\_volid** pattern severity  
...
* **-in\_charset** character_set_name  
...
* **-auto\_charset** "on"|"off"  
  ...
  terminal where **xorriso** runs. Before attributing this local
  character set
...
* **-md5** "on"|"all"|"off"|"load_check_off"  
  ...
  Therefore load_check_off is in effect when **xorriso** -as mkisofs
...
* **-ecma119\_map** "stripped"|"unmapped"|"lowercase"|"uppercase"  
...
* **-iso\_nowtime** "dynamic"|timestring  
  ...
  of timestrings see command **-alter\_date**.
* **-disk\_dev\_ino** "on"|"ino_only"|"off"  
...
* **-rom\_toc\_scan** "on"|"force"|"off"[:"emul_off"][:"emul_wide"]  
...
* **-calm\_drive** "in"|"out"|"all"|"revoke"|"on"|"off"  
...
* **-early\_stdio\_test** "on"|"appendable_wo"|"off"  
...
* **-data\_cache\_size** number_of_tiles blocks_per_tile  
...
* **-map\_single** disk_path iso_rr_path  
  Like -map, but if disk_path is a directory then its sub tree is not inserted.
* **-map\_l** disk_prefix iso_rr_prefix disk_path [***]  
...
* **-update** disk_path iso_rr_path  
  ...
  updated.
* **-update\_r** disk_path iso_rr_path  
...
* **-update\_l** disk_prefix iso_rr_prefix disk_path [***]  
  Perform -update_r with each of the disk_path parameters. iso_rr_path will be
  composed from disk_path by replacing disk_prefix by iso_rr_prefix.
* **-update\_li** iso_rr_prefix disk_prefix iso_rr_path [***]  
...
* **-update\_lxi** disk_prefix iso_rr_prefix disk_path [***]  
...
* **-cut\_out** disk_path byte_offset byte_count iso_rr_path  
  ...
  E.g:  
   -cut_out /my/disk/file 0 2047m \.br
   /file/part_1_of_3_at_0_with_2047m_of_5753194821 \.br
   -cut_out /my/disk/file 2047m 2047m \.br
...
* **-cp\_clone** iso_rr_path_original [***] iso_rr_path_dest  
...
* **-file\_size\_limit** value [value [...]] --  
  ...
  size is not limited by **xorriso**.
...
* **-follow** occasion[:occasion[...]]  
  ...
  **link** is the hop from a symbolic link to its target file object for the
  ...
  **concat** is the hop from a symbolic link to its target file object for
...
* **-alter\_date** type timestring iso_rr_path [***]  
  ...
  **xorriso**'s own timestamps:  
  ...
  value.
* **-alter\_date\_r** type timestring iso_rr_path [***]  
...
* **-find** iso_rr_path [test [op] [test ...]] [-exec action [params]] --  
  ...
  **-lba\_range** start_lba block_count :
  ...
  **xorriso** commands which get performed on the found files.
  These commands
  ...
  **alter\_date** and **alter\_date\_r**
...
* **-external\_filter** name option[:option] program_path [arguments] --  
  ...
  Examples:  
   -external_filter bzip2 suffix=.bz2:if_block_reduction \.br
                    /usr/bin/bzip2 --  
   -external_filter bunzip2 suffix=.bz2:remove_suffix \.br
                    /usr/bin/bunzip2 --
* **-unregister\_filter** name  
...
* **-set\_filter\_r** name iso_rr_path [***]  
...
* **-changes\_pending** "no"|"yes"|"mkisofs_printed"|"show_status"  
...
* **-blank** mode  
  ...
  If feature 21h is missing, then **xorriso**
...
* **-list\_speeds**  
  ...
  "52xCD", "24xDVD", "20xBD".
* **-close\_damaged** "as_needed"|"force"  
...
* **-preparer\_id**  
  ...
  which shall be recorded. Normally this should be the ID of **xorriso**
  and not of the person or program which operates **xorriso**.
...
* **-application\_use** character|0xXY|disk_path  
...
* **-out\_charset** character_set_name  
...
* **-speed** code|number[k|m|c|d|b]  
  ...
  for their own decision.
* **-stream\_recording** "on"|"off"|"full"|"data"|number  
...
* **-modesty\_on\_drive** parameter[:parameters]  
...
* **-use\_immed\_bit** "on"|"off"|"default"  
...
* **-padding** number["k"|"m"]|"included"|"appended"  
  ...
  **xorriso** adds the traditional 300k of padding by default to
  all images.  
...
* **-boot\_image** "any"|"isolinux"|"grub"    
  ...
  **show\_status** will print what is known about the loaded boot images
  and their designated fate.  
  
  A **bootspec** is a word of the form name=value. It is used to describe
  ...
  In that case it suffices to use as bootspec the text "**dir=/isolinux**"
  ...
  **cat\_path=** with the first -boot_image "any" "next" or at -commit time.
  ...
  directory tree at all. One may hide it in all trees by **cat\_hidden=on**.
  ...
  **bin\_path=** depicts an El Torito boot image file, a binary program
  which is to be started by the hardware boot facility (e.g. the BIOS)
  at boot time.  
  **efi\_path=** depicts an El Torito boot image file that is ready for
  ...
  **emul\_type=** can be one of "no_emulation", "hard_disk", "diskette".
  ...
  **load\_size=** is a value which depends on the boot image.
  ...
  **boot\_info\_table=on** causes address patching to bytes 8 to 63
  of the boot image which is given by "any" "bin_path=".
  "boot_info_table=off" disables this patching.  
  **grub2\_boot\_info=on** causes address patching to byte 2548
  of the boot image which is given by "any" "bin_path=".
  ...
  **platform\_id=** defines by a hexadecimal or decimal number
  the Platform ID of the boot image. "0x00" is 80x86 PC-BIOS, "0x01" is PowerPC,
  "0x02" is Mac, "0xef" is EFI (decimal "239").  
  **id\_string=**text|56_hexdigits defines the ID string of the boot catalog
  ...
  **sel\_crit=**hexdigits defines the Selection Criteria of the boot image.
  ...
  **system\_area=**disk_path copies at most 32768 bytes from the given
  ...
  to the ISO image.  
  **-boot_image isolinux system\_area=** implies "partition_table=on".
  ...
  may be augmented by
  **isolinux partition\_entry=gpt\_basdat**
  or **isolinux partition\_entry=gpt\_hfsplus**,
  and by **isolinux partition\_entry=apm\_hfsplus**.
  ...
  commands.  
  **-boot_image any gpt\_disk\_guid=**value controls whether an emerging GPT
  ...
  **-boot_image any part\_like\_isohybrid=on** enables
  ...
  **-boot_image any iso\_mbr\_part\_type=**number sets the partition type
  ...
  **grub2\_mbr=**disk_path works like "any" system_area= with additional
  ...
  **partition\_table=on** causes a simple partition table to be written
  ...
  **appended\_part\_as=gpt** marks partitions from -append_partition
  in GPT rather than in MBR. In this case the MBR shows a single partition
  of type 0xee which covers the whole output data.  
  **appended\_part\_as=mbr** is the default. Appended partitions get
  ...
  may be added to "mbr".  
  **appended\_part\_as=apm** marks partitions from -append_partition in APM
  ...
  **chrp\_boot\_part=on** causes a single partition in MBR which covers
  ...
  **prep\_boot\_part=**disk_path inserts the content of a data file into
  the image and
  ...
  **efi\_boot\_part=**disk_path inserts the content of a data file into
  the image and
  ...
  **partition\_offset=**2kb_block_adr causes a partition table with a single
  ...
  **partition\_hd\_cyl=**number gives the number of heads per cylinder for
  the partition table. 0 chooses a default value. Maximum is 255.  
  **partition\_sec\_hd=**number gives the number of sectors per head for
  ...
  **partition\_cyl\_align=**mode controls image size alignment to an integer
  ...
  **mbr\_force\_bootable=**mode enforces an MBR partition with
  ...
  **mips\_path=**iso_rr_path declares a data file in the image to be a
  ...
  **mipsel\_path=**iso_rr_path declares a data file in the image to be the
  ...
  **sparc\_label=**text causes the production of a SUN Disk Label with the
  ...
  **grub2\_sparc\_core=**iso_rr_path causes the content address and size
  ...
  **hppa\_cmdline=**text sets the PALO command line for HP-PA. Up to 1023
  ...
  **hppa\_bootloader=**iso_rr_path designates the given path as HP-PA
  bootloader file.  
  **hppa\_kernel\_32=**iso_rr_path designates the given path as HP-PA
  32 bit kernel file.  
  **hppa\_kernel\_64=**iso_rr_path designates the given path as HP-PA
  64 bit kernel file.  
  **hppa\_ramdisk=**iso_rr_path designates the given path as HP-PA
  RAM disk file.  
  **hppa\_hdrversion=**number chooses between PALO header version 5 (default)
  and version 4.
  For the appropriate value see in PALO source code: PALOHDRVERSION.  
  **alpha\_boot=**iso_rr_path declares a data file in the image to be the
  ...
  **mips\_discard**, **sparc\_discard**, **hppa\_discard**,
  **alpha\_discard**
...
* **-append\_partition** partition_number type_code disk_path  
  ...
  the unclaimed space before partition 1. So together with most **xorriso**
...
* **-jigdo** parameter_name value  
  ...
  emulation command.  
  Parameter **clear** with any value empties the whole list.
  No .jigdo and .template file will be produced.  
  **template\_path** sets the disk_path for the .template file with the
  holed and compressed ISO image copy.  
  Alias: -jigdo-template  
  **jigdo\_path** sets the disk_path for the .jigdo file with the checksums
  and download addresses for filling the holes in .template.  
  Alias: -jigdo-jigdo  
  **md5\_path** sets the disk_path where to find the .md5 input file.  
  Alias: -md5-list  
  **min\_size** sets the minimum size for a data file to be listed
  ...
  **exclude** adds a regular expression pattern which will get compared
  ...
  Alias: -jigdo-exclude  
  **demand\_md5** adds a regular expression pattern which will get compared
  ...
  **mapping** adds a string pair of the form To=From to the parameter list.
  ...
  **checksum\_iso** chooses one or more of "md5", "sha1", "sha256", "sha512"
  ...
  **checksum\_template** is like checksum_iso but for "# Template Hex".  
...
The meanings of byte codes are defined in **character sets** which have
names. Shell command iconv -l lists them.   
The file names on hard disk are assumed to be encoded by the
**local character set** which is also used for the communication
with the user.
...
* **-local\_charset** character_set_name  
...
* **-abort\_on** severity  
  ...
  **xorriso** begin. Only "-abort_on" with dash "-" is recognized that way.
* **-return\_with** severity exit_value  
  Set the threshold and exit_value to be returned at program end if no abort
  has happened. This is to allow **xorriso** to go on after problems
  ...
  **xorriso** if it decides to abort the program run:  
...
* **-report\_about** severity  
  ...
  of **xorriso** begin. Only "-report_about" with dash "-" is
  recognized that way.
* **-signal\_handling** mode  
  ...
  own signal precautions of **xorriso**. Inherited signal
  ...
  the risk that **xorriso** refuses to end until externally kill -9
  if performed.
  ...
  among the start arguments is in effect already when the first operations
  of **xorriso** begin. Only "-signal_handling" with dash "-" is
  recognized that way.
* **-error\_behavior** occasion behavior  
...
* **-mount\_cmd** drive entity id path  
...
* **-mount\_opts** option[:option...]  
  ...
  before ejecting.
* **-session\_string** drive entity id format  
...
* **-report\_el\_torito** mode    
  With mode **plain** print a report about the information found
  ...
  Mode **as\_mkisofs** tries to print the **xorriso -as mkisofs**
...
* **-report\_system\_area** mode  
  ...
  Modes **cmd** and **as\_mkisofs** work like with
  command -report_el_torito. See above.  
  With mode **gpt\_disk\_guid** print the GPT disk GUID of the loaded ISO
  ...
  With mode **gpt\_crc\_of:**disk_path read up to 32 KiB from the disk
  ...
  With mode **make\_guid** print a pseudo-random GUID in RFC 4122 text format
  to result channel.
...
* **-findx** disk_path [-name pattern] [-type t] [-exec action [params]] --  
  ...
  **not\_in\_iso**
  ...
  -compare.  
  **add\_missing** iso_rr_path_start
...
* **-check\_media\_defaults** [option [option ...]] --  
...
* **-check\_md5** severity iso_rr_path [***]  
...
* **-check\_md5\_r** severity iso_rr_path [***]  
...
* **-osirrox** setting[:option:...]  
  ...
  are owned by the effective user who runs **xorriso**. This happens
...
* **-cp\_rax** iso_rr_path [***] disk_path  
  Like -cp_rx but restoring mtime, atime as in ISO image and trying to set
  ownership and group as in ISO image.
* **-paste\_in** iso_rr_path disk_path byte_offset byte_count  
...
* **-concat** mode [target | lim prog [args [...]] lim] iso_rr_path [***]  
  ...
  Mode **pipe** expects as second parameter a delimiter word which shall
...
* **-as** personality option [options] --    
  ...
  **xorriso**.  
  ...
  are interpreted differently than with **xorriso** command -add.
  Directories get
  ...
  is chosen by **xorriso** commands before -as mkisofs or after its
  ...
  They work like the **xorriso** commands with the
  ...
  performed via **xorriso** command -jigdo. See the "Alias:" names there
  for the meaning of the genisoimage options.  
  
  Personalities "**xorrisofs**", "**genisoimage**",
  and "**genisofs**" are aliases for "mkisofs".  
  ...
  Personalites "**xorrecord**", "**wodim**", and "**cdrskin**"
  ...
  From then on, arguments are interpreted as **xorriso** commands.  
  --no_rc as first argument of such a program start
  prevents interpretation of **xorriso** startup files.
...
* **-pacifier** behavior_code  
  ...
  are 0.1 to 60.0.
* **-scdbackup\_tag** list_path record_name  
...
* **-options\_from\_file** fileaddress  
  ...
  should hold one **xorriso** command and all its parameters.
...
* **-sh\_style\_result** "on"|"off"  
  ...
  shell commands.
* **-backslash\_codes** "on"|"off"|mode[:mode]  
...
* **-errfile\_log** mode path|channel    
...
* **-scsi\_log** "on"|"off"  
  ...
  Logging messages get printed to stderr, not to any of the **xorriso**
  output channels.  
...
* **-mark** text  
  ...
  **xorriso** performs a command that was entered to the pager prompt.
* **-msg\_op** opcode parameter_text  
...
* **-named\_pipe\_loop** mode[:mode] disk_path_stdin disk_path_stdout disk_path_stderr  
...
* **-launch\_frontend** program [arguments ...] --  
...
or groups which shall be able to use the drives with **xorriso**.
...
$ xorriso -outdev /dev/sr2 \.br
 -blank as_needed \.br
 -map /home/me/sounds /sounds \.br
...
$ xorriso -outdev /dev/sr2 \.br
 -blank as_needed \.br
 -map /home/me/sounds /sounds \.br
 -map /home/me/pictures /pictures \.br
 -rm_r \.br
   /sounds/indecent \.br
   '/pictures/*private*' \.br
   /pictures/confidential \.br
   -- \.br
 -cd / \.br
...
$ xorriso -dev /dev/sr2 \.br
 -rm_r /sounds -- \.br
 -mv \.br
   /pictures/confidential \.br
   /pictures/restricted \.br
   -- \.br
 -chmod go-rwx /pictures/restricted -- \.br
 -map /home/me/prepared_for_dvd/sounds_dummy /sounds \.br
 -map /home/me/prepared_for_dvd/movies /movies \.br
 -commit -eject all
.SS
**Copy modified ISO image from one medium to another**
...
$ xorriso -indev /dev/sr2 \.br
 -rm_r /sounds -- \.br
 ...  
 -outdev /dev/sr0 -blank as_needed \.br
 -commit -eject all
.SS
**Bring a prepared ISOLINUX tree onto medium and make it bootable**
...
Now **xorriso** can burn an El Torito bootable medium:  
$ xorriso -outdev /dev/sr0 -blank as_needed \.br
   -map /home/me/ISOLINUX_prepared_tree / \.br
   -boot_image isolinux dir=/boot/isolinux
.SS
**Change existing file name tree from ISO-8859-1 to UTF-8**
...
$ xorriso -in_charset ISO-8859-1 -local_charset UTF-8 \.br
   -out_charset UTF-8 -backslash_codes on -dev /dev/sr0 \.br
...
$ xorriso -as mkisofs prepared_for_iso/tree1 | \.br
...
$ xorriso -as mkisofs -M /dev/sr0 -C $m prepared_for_iso/tree2 | \.br
...
-M. If **xorriso** gets started by name "xorrisofs" then it is suitable
for that.  
$ export MKISOFS="xorrisofs"  
$ growisofs -Z /dev/dvd /some/files  
$ growisofs -M /dev/dvd /more/files  
If no "xorrisofs" is available on your system, then you will have to create
a link pointing to the **xorriso** binary and tell growisofs to use it.
E.g. by:  
...
$ xorriso ... \.br
 -report_about UPDATE \.br
 -return_with FAILURE 32 \.br
 -abort_on NEVER \.br
 ...
.SS
**Examples of input timestrings**  
...
$ xorriso \.br
 -abort_on FATAL \.br
 -for_backup -disk_dev_ino on \.br
 -assert_volid 'PROJECTS_MAIL_*' FATAL \.br
 -dev /dev/sr0 \.br
 -volid PROJECTS_MAIL_"$(date '+%Y_%m_%d_%H%M%S')" \.br
 -not_leaf '*.o' -not_leaf '*.swp' \.br
 -update_r /home/thomas/projects /projects \.br
 -update_r /home/thomas/personal_mail /personal_mail \.br
...
the local filesystem, insert these commands immediately before -commit :  
 ...
 -find / -type f -pending_data -exec set_filter --zisofs -- \.br
...
With **mount** option **-o "sbsector="** on GNU/Linux
or **-s** on FreeBSD or NetBSD
...
commands use a common parent directory and clone it after updating is done:  
 -update_r /home/thomas/projects /current/projects \.br
 -update_r /home/thomas/personal_mail /current/personal_mail \.br
 -clone /current /"$(date '+%Y_%m_%d_%H%M%S')" \.br
...
**Better do not use your youngest backup for -update\_r**.
...
$ xorriso -for_backup \.br
 -load volid 'PROJECTS_MAIL_2008_06_19*' \.br
 -indev /dev/sr0 \.br
 -osirrox on:auto_chmod_on \.br
 -chmod u+rwx / -- \.br
 -extract /projects /home/thomas/restored/projects \.br
 -extract /personal_mail /home/thomas/restored/personal_mail \.br
 -rollback_end  
The final command -rollback_end prevents an error message about the altered
image being discarded.
.SS
**Try to retrieve blocks from a damaged medium**  
$ xorriso -abort_on NEVER -indev /dev/sr0 \.br
 -check_media time_limit=1800 report=blocks_files \.br
...
Please describe what you expect **xorriso** to do,
the program arguments or dialog commands by which you tried to achieve it,
the messages of **xorriso**, and the undesirable outcome of your
program run.  
...
Thomas Schmitt &lt;[scdbackup@gmx.net](mailto:scdbackup@gmx.net)&gt;  
...
modified in sync with the technical properties of **xorriso**.
If you make use of the license to derive modified versions of
**xorriso** then you are entitled to modify this text under that
same license.
