...
**Track** which is a contiguous string of readable blocks. 
...
a session. Some of these features can be addressed by program **cdrskin**.  
...
**-multi**. CD-RW and DVD-RW can be blanked in order to be re-usable
from scratch.  
...
**blank=format\_overwrite** and **blank=deformat**.  
...
**--grow\_overwriteable\_iso**. In this case, the need for
...
by a run of **xorrecord --devices** or **xorriso -device\_links**.  
...
which is usually gained by running **xorrecord** via command pfexec.  
...
* **-toc**  
  ...
  compatible to
  cdrecord option -toc, but rather the one of **xorriso** command -toc.
  ...
  If not option **--grow\_overwriteable\_iso** is given or no ISO 9660
...
predictable length, or need to be accompanied by option **tsize=** or
by option **-isosize**.  
...
* **blank=mode**  
  ...
  It is combinable with burning in the same run of **xorrecord**,
...
* **-multi**  
  ...
  the situation which can be retrieved by **xorrecord** option -msinfo.  
...
* **-waiti**  
  ...
  xorrisofs. Example:  
  xorrisofs ... -C 0,12800 -M /dev/sr0 ... | \.br
  ...
  **xorrecord**. Note that this input line will not be consumed by
...
* **padsize=size**  
  ...
  by **xorrecord** if the track size cannot be predicted or if the CD
...
* **fs=size**  
  ...
  In this case, consider to use program **cdrskin** for CD burning.
...
* **-V**  
  ...
  This might be of interest if **xorrecord** and a particular drive
...
* **drive\_scsi\_dev\_family=sr|scd|sg|default**  
  GNU/Linux specific:  
...
* **--grow\_overwriteable\_iso**  
  ...
  I.e. it is possible to use the same **xorrecord** options with both
  kinds of media
...
* **stream\_recording="on"|"off"|number**  
...
directory ./tree1 under the ISO 9660 directory /dir1:  
  ...
  $ xorrecord -v dev=/dev/sr0 speed=12 fs=8m \.br
              -multi --grow_overwriteable_iso \.br
...
Use the numbers with xorrisofs to add ./tree2 to the image as /dir2:  
  $ xorrisofs -M /dev/sr0 -C $m -o image_2.iso \.br
...
Again, xorriso has the patience and dd is a dispensable victim for demons.  
  $ m=$(xorrecord dev=/dev/sr0 -msinfo)  
  $ dd if=/dev/sr0 count=1 &gt;/dev/null 2&gt;&1  
  $ xorrisofs -M /dev/sr0 -C $m  \.br
              -J -graft-points /dir2=./tree2 \.br
    | xorrecord -v dev=/dev/sr0 speed=12 fs=8m \.br
                -waiti -multi --grow_overwriteable_iso \.br
                -eject padsize=300k -  
...
where the run would need no system workarounds and simply look like:  
  $ xorriso -dev /dev/sr0 -joliet on -speed 12 -fs 8m \.br
...
temporary data shortage.  
  $ find . | afio -oZ - | \.br
    xorrecord -v dev=/dev/sr0 speed=12 fs=64m \.br
              -multi padsize=300k -  
...
to exist. The lines are not interpreted as **xorrecord** options but
