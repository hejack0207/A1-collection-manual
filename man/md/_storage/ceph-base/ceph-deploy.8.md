# ceph-deploy(8) - Ceph deployment tool
...
Other options like _--no-ssh-copykey_, _--fsid_,
_--cluster-network_ and _--public-network_ can also be used with
this command.
...
_--public-network_ option with the command as mentioned above.
...
If the user explicitly specifies a custom repo url with _--repo-url_ for
...
defined) and installs them. **cd\_conf** is the object built from **argparse**
...
its dependencies by using _--repo_ option.
...
Other options like _--testing_, _--dev_, _--adjust-repos_,
_--no-adjust-repos_, _--repo_, _--local-mirror_,
_--repo-url_ and _--gpg-url_ can also be used with this command.
...
with _--mkfs_ on remote nodes. All keyring files are concatenated to be
...
resolving the hostname to an IP. If _--address_ is used it will override
...
exist in **monmap** and if neither **public\_addr** nor **public\_network** keys
...
status is checked by running **ceph daemon mon.hostname mon\_status** on remote
...
found.
It then uses **ceph-volume** utility's **lvm create** subcommand to
...
specified after the command. Two options _--install_ and
_--remove_ are used for this purpose.
...
**ceph-deploy** is part of Ceph, a massively scalable, open-source, distributed storage system. Please refer to
the documentation at _https://ceph.com/ceph-deploy/docs_ for more information.
