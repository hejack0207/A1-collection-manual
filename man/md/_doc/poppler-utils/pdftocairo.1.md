...
If the
_PDF-file_
is -\*(rq , the PDF is read from stdin.
If the
_output-file_
is -\*(rq , the output file will be written to stdout. Using stdout is not valid with image formats unless -singlefile is used.
If
_output-file_
...
<a name="windows-printer-options"></a>
