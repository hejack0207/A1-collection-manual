...
The **LIRC\_GET\_REC\_MODE** ioctl (see below) allows probing for the
mode.


<a name="reading-input-with-the-lirc_mode_mode2-drivers"></a>

### Reading input with the LIRC_MODE_MODE2 drivers


In the **LIRC_MODE_MODE2 mode**, the data returned by
**read**(2)
...
<a name="reading-input-with-the"></a>
...
When sending data, only the **LIRC\_MODE\_PULSE**
mode is supported.
...
<a name="always-supported-commands"></a>
...
<a name="optional-commands"></a>

### Optional Commands


Some lirc devices support commands listed below.
Unless otherwise stated, these fail with the error **ENOIOCTLCMD**
or with the error **ENOSYS** if the operation
isn't supported, or with the error **EINVAL** if the operation
failed.

* **LIRC_SET_REC_MODE** (_int_)  
...
* * **LIRC_GET_MIN_TIMEOUT** (_void_)**,** **LIRC_GET_MAX_TIMEOUT** (_void_)  
...
* **LIRC_SET_REC_TIMEOUT_REPORTS** (_int_)  
  Enable
  (_val_
  is 1) or disable
  (_val_
...
* **LIRC_SET_REC_CARRIER** (_int_)  
  Set the receive carrier frequency (Hz).
* **LIRC_SET_REC_CARRIER_RANGE** (_int_)  
...
* **LIRC_SET_MEASURE_CARRIER_MODE** (_int_)  
  Enable
  (_val_
  is 1) or disable
  (_val_
...
* **LIRC_GET_REC_RESOLUTION** (_void_)  
  Return the driver resolution (microseconds).
* **LIRC_GET_MIN_FILTER_PULSE** (_void_)**,** **LIRC_GET_MAX_FILTER_PULSE** (_void_)  
  ...
  **lircd.conf**(5)
  config file.
* **LIRC_GET_MIN_FILTER_SPACE** (_void_)**,** **LIRC_GET_MAX_FILTER_SPACE** (_void_)  
  See
  **LIRC_GET_MIN_FILTER_PULSE**.
* **LIRC_SET_REC_FILTER** (_int_)  
  Pulses/spaces shorter than this (microseconds) are filtered out by
  hardware.
* **LIRC_SET_REC_FILTER_PULSE** (_int_)**,** **LIRC_SET_REC_FILTER_SPACE** (_int_)  
  Pulses/spaces shorter than this (microseconds) are filtered out by
  hardware.
...
* **LIRC_SET_WIDEBAND_RECEIVER** (_int_)  
...
* Note: wide band receiver may be implicitly enabled if you enable
  ...
  will do nothing.
* **LIRC_SETUP_START** (_void_), **LIRC_SETUP_END** (_void_)  
