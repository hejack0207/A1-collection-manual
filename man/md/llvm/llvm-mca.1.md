# llvm-mca(1) - LLVM Machine Code Analyzer
...
**llvm-mca** is a performance analysis tool that uses information
...
(**llvm-mca** detects Intel syntax by the presence of an _.intel\_syntax_
...
By design, the quality of the analysis conducted by **llvm-mca** is
...
If **input** is "**-**" or omitted, **llvm-mca** reads from standard
input. Otherwise, it will read from the specified filename.

If the _-o_ option is omitted, then **llvm-mca** will send its output
...
<a name="using-markers-to-analyze-specific-code-blocks"></a>

# Using Markers to Analyze Specific Code Blocks


**llvm-mca** allows for the optional usage of special code comments to
...
If no user-defined region is specified, then **llvm-mca** assumes a
...
an impact on the code generated. This is because the **\_\_asm** statements are
...
The _Clang options to emit optimization reports_
can also help in detecting missed optimizations.

<a name="how-llvm-mca-works"></a>
...
Field _DispatchWidth_ is the maximum number of micro opcodes that are dispatched
...
Field _Block RThroughput_ is the reciprocal of the block throughput. Block
...
of a single iteration by the _Block RThroughput_.
...
single iteration by the _Block RThroughput_.

Field _uOps Per Cycle_ is bounded from above by the dispatch width. That is
...
_Block RThroughput_) is an indicator of a performance bottleneck caused by the
...
are no loop-carried dependencies, the observed _uOps Per Cycle_ is expected to
...
resources, and the _Resource pressure view_ can help to identify the problematic
resource usage.

The second section of the report is the _instruction info view_. It shows the
...
Field _RThroughput_ is the reciprocal of the instruction throughput. Throughput
...
_-show-encoding_ is specified.

Below is an example of _-show-encoding_ output for the dot-product kernel:
.INDENT 0.0
.INDENT 3.5
...
The _Encoding Size_ column shows the size in bytes of instructions.  The
_Encodings_ column shows the actual instruction encodings (byte sequences in
hex).

The third section is the _Resource pressure view_.  This view reports
...
(named _Average Wait times_) reports useful timing statistics, which should
...
Table _Average Wait times_ helps diagnose performance issues that are caused by
the presence of long latency instructions and potentially long data dependencies
which may limit the ILP. Last row, **&lt;total&gt;**, shows a global average over all
instructions measured. Note that **llvm-mca**, by default, assumes at
least 1cy between the dispatch event and the issue event.

When the performance is limited by data dependencies and/or long latency
instructions, the number of cycles spent while in the _ready_ state is expected
...
The _critical sequence_ is the most expensive sequence of instructions according
...
<a name="extra-statistics-to-further-diagnose-performance-issues"></a>
...
Below is an example of **-all-stats** output generated by  **llvm-mca**
...
If we look at the _Dynamic Dispatch Stall Cycles_ table, we see the counter for
...
Looking at the _Dispatch Logic_ table, we see that the pipeline was only able to
...
The next table, _Schedulers_, presents a histogram displaying a count,
...
The _Scheduler's queue usage_ table shows that the average and maximum number of
...
The next table, _Retire Control Unit_, presents a histogram displaying a count,
...
The last table presented is _Register File statistics_.  Each physical register
...
<a name="instruction-flow"></a>

### Instruction Flow


This section describes the instruction flow through the default pipeline of
**llvm-mca**, as well as the functional units involved in the process.
...
bottlenecks in the frontend are not diagnosed. **llvm-mca** assumes that
instructions have all been decoded and placed into a queue before the simulation
start.  Also, **llvm-mca** does not model branch prediction.

<a name="instruction-dispatch"></a>
...
the processor. **llvm-mca** uses that information to initialize register
...
**-register-file-size**.  A value of zero for this option means _unbounded_. By
...
_MicroOpBufferSize_ in the target scheduling model.

Instructions that are dispatched to the schedulers consume scheduler buffer
entries. **llvm-mca** queries the scheduling model to determine the set
of buffered resources consumed by an instruction.  Buffered resources are
treated like scheduler resources.

<a name="instruction-issue"></a>
...
Instruction latencies are computed by **llvm-mca** with the help of the
scheduling model.
...
<a name="write-back-and-retire-stage"></a>
...
<a name="loadstore-unit-and-memory-consistency-model"></a>
...
(_-noalias=true_) store operations.  Under this assumption, younger loads are
