...
```
.HP \w'busctl&nbsp;'u busctl [OPTIONS...] [COMMAND] [NAME...]
```
...
\m[blue]**Libpcap File Format**\m[]\s-2\u[1]\d\s+2
...
\m[blue]**Type system chapter of the D-Bus specification**\m[]\s-2\u[2]\d\s+2). For simple types, each parameter following the signature should simply be the parameters value formatted as string. Positive boolean values may be formatted as
"true",
"yes",
"on", or
