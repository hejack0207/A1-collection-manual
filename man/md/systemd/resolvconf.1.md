...
```
.HP \w'resolvectl&nbsp;'u resolvectl [OPTIONS...] {COMMAND} [NAME...]
```
...
**-p** _PROTOCOL_, **--protocol=**_PROTOCOL_
...
**-t** _TYPE_, **--type=**_TYPE_, **-c** _CLASS_, **--class=**_CLASS_
...
Resolve
\m[blue]**DNS-SD**\m[]\s-2\u[3]\d\s+2
and
\m[blue]**SRV**\m[]\s-2\u[4]\d\s+2
...
\m[blue]**OPENPGPKEY**\m[]\s-2\u[5]\d\s+2
...
\m[blue]**TLSA**\m[]\s-2\u[6]\d\s+2
resource records. A query will be performed for each of the specified names prefixed with the port and family ("\__port_.\__family_._domain_"). The port number may be specified after a colon (":"), otherwise
**443**
...
Get/set per-interface DNS configuration. These commands may be used to configure various DNS settings for network interfaces that arent managed by
**systemd-networkd.service**(8). (These commands will fail when used on interfaces that are managed by
**systemd-networkd**, please configure their DNS settings directly inside the
.network
...
**default-route**
command expects a boolean paremeter, and configures whether the link may be used as default route for DNS lookups, i.e. if it is suitable for lookups on domains no other link explicitly is configured for. The
**llmnr**,
**mdns**,
**dnssec**
and
...
**dnssec**,
**dnsovertls**,
**nta**. Note that when a network interface disappears all configuration is lost automatically, an explicit reverting is not necessary in that case.

<a name="compatibility-with-resolvconf8"></a>
...
**systemd-resolved.service**(8), similar to how
**dns**
and
**domain**
