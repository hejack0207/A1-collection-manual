...
"resolve". Controls Link-Local Multicast Name Resolution support (\m[blue]**RFC 4795**\m[]\s-2\u[1]\d\s+2) on the local host. If true, enables full LLMNR responder and resolver support. If false, disables both. If set to
...
_MulticastDNS=_
Takes a boolean argument or
"resolve". Controls Multicast DNS support (\m[blue]**RFC 6762**\m[]\s-2\u[2]\d\s+2) on the local host. If true, enables full Multicast DNS responder and resolver support. If false, disables both. If set to
"resolve", only resolution support is enabled, but responding is disabled. Note that
**systemd-networkd.service**(8)
