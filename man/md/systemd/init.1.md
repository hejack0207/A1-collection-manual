...
```
.HP \w'/usr/lib/systemd/systemd&nbsp;'u /usr/lib/systemd/systemd [OPTIONS...] .HP \w'init&nbsp;'u init [OPTIONS...] {COMMAND}
```
...
_Requires=_
and
_Conflicts=_) as well as ordering dependencies (_After=_
and
_Before=_). NB: ordering and requirement dependencies are orthogonal. If only a requirement dependency exists between two units (e.g.
foo.service
requires
...
Application programs and units (via dependencies) may request state changes of units. In systemd, these requests are encapsulated as jobs\*(Aq and maintained in a job queue. Jobs may succeed or can fail, their execution is ordered based on the ordering dependencies of the units they have been scheduled for.
...
systemd will automatically and implicitly load units from disk — if they are not loaded yet — as soon as operations are requested for them. Thus, in many respects, the fact whether a unit is loaded or not is invisible to clients. Use
...
\m[blue]**cgroups.txt**\m[]\s-2\u[1]\d\s+2
...
\m[blue]**Original Design Document**\m[]\s-2\u[2]\d\s+2.

Note that some but not all interfaces provided by systemd are covered by the
\m[blue]**Interface Stability Promise**\m[]\s-2\u[3]\d\s+2.
...
\m[blue]**Container Interface**\m[]\s-2\u[4]\d\s+2
or
\m[blue]**initrd Interface**\m[]\s-2\u[5]\d\s+2
...
\m[blue]**XDG Base Directory specification**\m[]\s-2\u[6]\d\s+2
...
**systemctl start ctrl-alt-del.target --job-mode=replace-irreversibly**. If this signal is received more than 7 times per 2s, an immediate reboot is triggered. Note that pressing
Ctrl+Alt+Del
...
_systemd.show\_status=1_
on the kernel command line.

**SIGRTMIN+21**
Disables display of status messages on the console, as controlled via
_systemd.show\_status=0_
on the kernel command line.

**SIGRTMIN+22**
Sets the service managers log level to
"debug", in a fashion equivalent to
_systemd.log\_level=debug_
on the kernel command line.
...
_systemd.log\_target=console_
(or
_systemd.log\_target=kmsg_
on
...
_$XDG\_CONFIG\_HOME_, _$XDG\_CONFIG\_DIRS_, _$XDG\_DATA\_HOME_, _$XDG\_DATA\_DIRS_
...
_$LISTEN\_PID_, _$LISTEN\_FDS_, _$LISTEN\_FDNAMES_
...
\m[blue]**Known Environment Variables**\m[]\s-2\u[7]\d\s+2.
...
_systemd.crash\_shell_, the system is rebooted after the shell exits.
...
_systemd.log\_target=_, _systemd.log\_level=_, _systemd.log\_location=_, _systemd.log\_color_
...
_systemd.default\_standard\_output=_, _systemd.default\_standard\_error=_
...
\m[blue]**unified cgroup hierarchy**\m[]\s-2\u[9]\d\s+2
...
\m[blue]**legacy cgroup hierarchy**\m[]\s-2\u[10]\d\s+2, a.k.a.&nbsp;cgroups-v1, for other controllers), and forces a full "legacy" mode. When specified with a false argument, enables the use of "hybrid" hierarchy.
...
_systemd.log\_level=debug_. Note that this option is also read by the kernel itself and enables kernel debug output. Passing this option hence turns on the debug output from both the system manager and the kernel.

_emergency_, _rd.emergency_, _-b_
Boot into emergency mode. This is equivalent to
_systemd.unit=emergency.target_
or
_rd.systemd.unit=emergency.target_, respectively, and provided for compatibility reasons and to be easier to type.

_rescue_, _rd.rescue_, _single_, _s_, _S_, _1_
Boot into rescue mode. This is equivalent to
_systemd.unit=rescue.target_
or
_rd.systemd.unit=rescue.target_, respectively, and provided for compatibility reasons and to be easier to type.

_2_, _3_, _4_, _5_
Boot into the specified legacy SysV runlevel. These are equivalent to
_systemd.unit=runlevel2.target_,
_systemd.unit=runlevel3.target_,
_systemd.unit=runlevel4.target_, and
_systemd.unit=runlevel5.target_, respectively, and provided for compatibility reasons and to be easier to type.
...
The
\m[blue]**systemd Homepage**\m[]\s-2\u[11]\d\s+2,
**systemd-system.conf**(5),
**locale.conf**(5),
**systemctl**(1),
**journalctl**(1),
