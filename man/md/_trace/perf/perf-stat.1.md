...
.if n \{.RS 4
.\}
    percore*(Aq is a event qualifier that sums up the event counts for both
...
* ·  
  .\}
  a symbolically formed event like
  _pmu/config=M,config1=N,config2=K/_
...
.\}
    ...
    uncore_*(Aq is also ignored when performing this match.
.if n \{.RE
.\}
...
.if n \{.RS 4
.\}
    Performance counter stats for perf bench sched pipe*(Aq (5 runs):
.if n \{.RE
.\}
...
perf stat --repeat 10 --null --sync --pre _make -s O=defconfig-build/clean_ — make -s -j64 O=defconfig-build/ bzImage
...
_perf stat --time 2000 -e cycles -a_
...
.if n \{.RS 4
.\}
    Performance counter stats for make*(Aq:
.if n \{.RE
.\}
...
Those times are the very same as displayed by the _time_ tool.
