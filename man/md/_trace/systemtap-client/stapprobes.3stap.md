...
```
 ...
 .SAMPLE probe PROBEPOINT [, PROBEPOINT] { [STMT ...] } .ESAMPLE 
 ...
 Probe aliases can be given on their own, or with a suffix. The suffix attaches to the underlying probe point that the alias is expanded to. For example, .SAMPLE syscall.read.return.maxactive(10) .ESAMPLE expands to .SAMPLE kernel.function("sys_read").return.maxactive(10) .ESAMPLE with the component maxactive(10) being recognized as a suffix. 
...
<synopsis>
.SAMPLE kernel.function("foo").return process("/bin/vi").statement(0x2222) end syscall.* syscall.*.return.maxactive(10) syscall.{open,close} sys**open kernel.function("no_such_function") ? module("awol").function("no_such_function") ! signal.*? if (switch) kprobe.function("foo") .ESAMPLE
</synopsis>
...
<synopsis>
.SAMPLE syscall.{write,read} # Expands to syscall.write, syscall.read
</synopsis>

<synopsis>
{kernel,module("nfs")}.function("nfs*")! # Expands to kernel.function("nfs*")!, module("nfs").function("nfs*")! .ESAMPLE
```
...
symbols" for the _specific program_ being instrumented.  For some others,
...
occurs.  (See the _--use-server_ option and the **stap-server(8)** man page for information about the remote compilation facility,
...
<a name="syscall-and-nd_syscall"></a>

### SYSCALL and ND_SYSCALL


The
_syscall.*_ and _nd_syscall.*_
...
* **.return**  
  Places a probe at the moment **after** the return from the named
...
* **.inline**  
  ...
  **.return** is not supported on **.inline** probes.
* **.call**  
...
* **.callee**  
  Places a probe on the callee function given in the **.callee**
  modifier, where the callee must be a function called by the target
  function given in **.function**. The advantage of doing this over
...
* **.callees**  
  Shortcut for **.callee("*")**, which places a probe on all callees of
  the function.
* **.callees**(DEPTH)  
  Recursively places probes on callees. For example, **.callees(2)**
  will probe both callees of the target function, as well as callees of
  those callees. And **.callees(3)** goes one level deeper, etc...
...
* $var  
  ...
  _kernel_string_ or _user_string_
  functions.
* @var("varname")  
...
* @var("varname","module")  
  ...
  _@var("_r_debug","/lib/ld-linux.so.2")_
* @var("varname@src/file.c")  
  ...
  _@var("foo@bar/baz.c")_
  and CUs with file name paths
  _src/sub/module/bar/baz.c_
  and
  _src/bar/baz.c_
...
* @defined($EXPR)  
  ...
  .ESAMPLE
* @probewrite($VAR)  
  see the PROBES section of _stap_(1).
* $EXPR$  
...
<a name="more-on-return-probes"></a>
...
access function parameters.  These values are **snapshots**
...
parameter context variable, a pointer named **addr**, may be
accessed from a
_.return_
probe.
...
only command parameters are allowed in the _-c_ command (i.e. no
...
_NFPROTO_IPV4_
are
_NF_INET_PRE_ROUTING,_
_NF_INET_LOCAL_IN,_
_NF_INET_FORWARD,_
_NF_INET_LOCAL_OUT,_
and
_NF_INET_POST_ROUTING._
...
<a name="kernel-markers-obsolete"></a>
...
* **.return**  
  Places a probe at the moment **before** the return from the named
