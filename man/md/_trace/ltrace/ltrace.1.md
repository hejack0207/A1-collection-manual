...
<synopsis>
[-e filter|-L] [-l|--library=library_pattern] [-x filter] [-S] [-b|--no-signals]
</synopsis>
...
<synopsis>
[-F pathlist] [-A maxelts] [-s strsize] [-C|--demangle] [-a|--align column] [-n|--indent nr] [-o|--output filename]
</synopsis>
...
<synopsis>
[-e filter|-L] [-l|--library=library_pattern] [-x filter] [-S]
</synopsis>
...
function prototypes.  Those are stored in files called prototype
libraries--see ltrace.conf(5) for details on the syntax of these
files.  See the section **PROTOTYPE LIBRARY DISCOVERY** to learn how
**ltrace** finds prototype libraries.
...
* -D, --debug mask  
  Show debugging output of **ltrace** itself.  _mask_ is a number
...
* -F pathlist  
  ...
  searched (see the section **PROTOTYPE LIBRARY DISCOVERY**).  If it refers to
...
* -o, --output filename  
  ...
  stderr.
* -p pid  
  Attach to the process with the process ID _pid_ and begin tracing.
...
* -w, --where nr  
  Show backtrace of _nr_ stack frames for each traced function. This
...
* -x filter  
  ...
  section **FILTER EXPRESSIONS**.  If more than one -x option appears
...
{[+-][_symbol\_pattern_][@_library\_pattern_]}

_Symbol\_pattern_ is used to match symbol names,
_library\_pattern_ to match library SONAMEs.  Both are implicitly
...
Both _symbol\_pattern_ and _library\_pattern_ have to match the
...
Initially, the symbol is _unmarked_.  If it matches a **+**
rule, it becomes _marked_, if it matches a **-** rule, it
becomes _unmarked_ again.  If, after applying all rules, the
symbol is _marked_, it will be traced.
...
$HOME/.ltrace, _X_/ltrace for each _X_ in $XDG_CONFIG_DIRS and
...
Juan Cespedes &lt;[cespedes@debian.org](mailto:cespedes@debian.org)&gt;  
