# fiologparser_hist.py(1) - Calculate statistics from fio histograms

August 18, 2016

```
fiologparser_hist.py [options] [clat_hist_files]...
```
...
* **-i**, **--interval**=int  
  ...
  should be set a minimum of the value for **log\_hist\_msec** as given
  to fio.
* **--noweight**  
...
* **-d**, **--divisor**=int  
  ...
  convert latencies from milliseconds to seconds (**divisor**=1000).
* **--warn**  
  Enables warning messages printed to stderr, useful for debugging.
* **--group_nr**=int  
  Set this to the value of _FIO\_IO\_U\_PLAT\_GROUP\_NR_ as defined in
  stat.h if fio has been recompiled. Defaults to 19, the
...
* **--directions**=str  
  ...
  \'rwtm\' characters with the --directions=rwtm option.
...
described here: _https://en.wikipedia.org/wiki/Percentile#Weighted\_percentile_.
...
The value of *_GROUP_NR in _stat.h_ (and *_BITS) determines how many latency bins
...
you will need to recompile fio with a larger GROUP_NR, e.g. with:
    ...
    sed -i.bak 's/^#define FIO_IO_U_PLAT_GROUP_NR 19n/#define FIO_IO_U_PLAT_GROUP_NR 24/g' stat.h
    make fio
...
**fiologparser_hist.py**
and this manual page were written by Karl Cronburg &lt;[karl.cronburg@gmail.com](mailto:karl.cronburg@gmail.com)&gt;.
