...
```
.HP \w'nmcli&nbsp;'u nmcli [OPTIONS...] {help | general | networking | radio | connection | device | agent | monitor} [COMMAND] [ARGUMENTS...]
```
...
**terminal-colors.d**(5). Please refer to the
COLORS
...
**-f** | **--fields** {_field1_,_field2_... | all | common}
...
**-g** | **--get-values** {_field1_,_field2_... | all | common}
...
.HP \w'**nmcli&nbsp;general**&nbsp;'u
**nmcli general** {**status** | **hostname** | **permissions** | **logging**} [_ARGUMENTS_...]
...
**logging** [**level**&nbsp;_level_] [**domains**&nbsp;_domains_...]
...
<a name="networking-control-commands"></a>

# Networking Control Commands

.HP \w'**nmcli&nbsp;networking**&nbsp;'u
**nmcli networking** {**on** | **off** | **connectivity**} [_ARGUMENTS_...]
...
<a name="radio-transmission-control-commands"></a>

# Radio Transmission Control Commands

.HP \w'**nmcli&nbsp;radio**&nbsp;'u
**nmcli radio** {**all** | **wifi** | **wwan**} [_ARGUMENTS_...]
...
<a name="connection-management-commands"></a>

# Connection Management Commands

.HP \w'**nmcli&nbsp;connection**&nbsp;'u
**nmcli connection** {**show** | **up** | **down** | **modify** | **add** | **edit** | **clone** | **delete** | **monitor** | **reload** | **load** | **import** | **export**} [_ARGUMENTS_...]

NetworkManager stores all network configuration as "connections", which are collections of data (Layer2 details, IP addressing, etc.) that describe how to create or connect to a network. A connection is "active" when a device uses that connections configuration to create or connect to a network. There may be multiple connections that apply to a device, but only one of them can be active on that device at any given time. The additional connections can be used to allow quick switching between different networks and configurations.
...
**show** [**--active**] [**id** | **uuid** | **path** | **apath**] _ID_...
...
the
_ID_
denotes a D-Bus static connection path in the format of /org/freedesktop/NetworkManager/Settings/_num_
or just
_num_.
...
the
_ID_
denotes a D-Bus active connection path in the format of /org/freedesktop/NetworkManager/ActiveConnection/_num_
or just
_num_.
...
**nmcli connection**, the default action is
**nmcli connection show**.

**up** [**id** | **uuid** | **path**] _ID_ [**ifname**&nbsp;_ifname_] [**ap**&nbsp;_BSSID_] [**passwd-file**&nbsp;_file_]
...
Be aware that this command deactivates the specified active connection, but the device on which the connection was active, is still ready to connect and will perform auto-activation by looking for a suitable connection that has the autoconnect\*(Aq flag set. Note that the deactivating connection profile is internally blocked from autoconnecting again. Hence it will not autoconnect until reboot or until the user performs an action that unblocks autoconnect, like modifying the profile or explicitly activating it.
...
**modify** [**--temporary**] [**id** | **uuid** | **path**] _ID_ {_option_&nbsp;_value_&nbsp;|&nbsp;[+|-]_setting_._property_&nbsp;_value_}...
...
**add** [**save**&nbsp;{yes&nbsp;|&nbsp;no}] {_option_&nbsp;_value_&nbsp;|&nbsp;[+|-]_setting_._property_&nbsp;_value_}...
...
**edit** {[**id**&nbsp;|&nbsp;**uuid**&nbsp;|&nbsp;**path**]&nbsp;_ID_ | [**type**&nbsp;_type_]&nbsp;[**con-name**&nbsp;_name_] }
...
**connection show**
above for the description of the
_ID_-specifying keywords. Not providing an
_ID_
...
**clone** [**--temporary**] [**id** | **uuid** | **path**] _ID_ _new\_name_
...
_ID_-specifying keywords.
_new\_name_
is the name of the new cloned connection. The new connection will be the exact copy except the connection.id (_new\_name_) and connection.uuid (generated) properties.

The new connection profile will be saved as persistent unless
**--temporary**
option is specified, in which case the new profile wont exist after NetworkManager restart.

**delete** [**id** | **uuid** | **path**] _ID_...
...
**import** [**--temporary**] **type**&nbsp;_type_ **file**&nbsp;_file_
...
**vpn-type**
option in
**nmcli connection add**. VPN configurations are imported by VPN plugins. Therefore the proper VPN plugin has to be installed so that
**nmcli**
...
**export** [**id** | **uuid** | **path**] _ID_ [_file_]
Export a connection.
...
<a name="device-management-commands"></a>

# Device Management Commands

.HP \w'**nmcli&nbsp;device**&nbsp;'u
**nmcli device** {**status** | **show** | **set** | **connect** | **reapply** | **modify** | **disconnect** | **delete** | **monitor** | **wifi** | **lldp**} [_ARGUMENTS_...]
...
**set** [ifname] _ifname_ [**autoconnect**&nbsp;{yes&nbsp;|&nbsp;no}] [**managed**&nbsp;{yes&nbsp;|&nbsp;no}]
...
**modify** _ifname_ {_option_&nbsp;_value_&nbsp;|&nbsp;[+|-]_setting_._property_&nbsp;_value_}...
...
See
**nm-settings**(5)
for the list of available properties. Please note that some properties cant be changed on an already connected device.
...
**wifi** [**list**&nbsp;[**--rescan**&nbsp;|&nbsp;**auto**&nbsp;|&nbsp;**no**&nbsp;|&nbsp;**yes**]&nbsp;[**ifname**&nbsp;_ifname_]&nbsp;[**bssid**&nbsp;_BSSID_]]
...
**wifi** **connect** _(B)SSID_ [**password**&nbsp;_password_] [**wep-key-type**&nbsp;{key&nbsp;|&nbsp;phrase}] [**ifname**&nbsp;_ifname_] [**bssid**&nbsp;_BSSID_] [**name**&nbsp;_name_] [**private**&nbsp;{yes&nbsp;|&nbsp;no}] [**hidden**&nbsp;{yes&nbsp;|&nbsp;no}]
...
**nmcli con up id ****name**. Note that only open, WEP and WPA-PSK networks are supported if no previous connection exists. It is also assumed that IP configuration is obtained via DHCP.
...
**wifi** **hotspot** [**ifname**&nbsp;_ifname_] [**con-name**&nbsp;_name_] [**ssid**&nbsp;_SSID_] [**band**&nbsp;{a&nbsp;|&nbsp;bg}] [**channel**&nbsp;_channel_] [**password**&nbsp;_password_]
...
**wifi** **rescan** [**ifname**&nbsp;_ifname_] [**ssid**&nbsp;_SSID_...]
...
**--ask**). However, you may find the command useful when you want to run a simple text based polkit agent and you do not have an agent of a desktop environment. Note that running this command makes
**nmcli**
...
connection.type
T}:T{
This alias also accepts values of **bond-slave**,
...
Its equivalent to the +bond.options \*(Aqoption=value\*(Aq syntax.
T}
T{
primary
T}::
...
Conectivity state when a connectivity check didnt run.
...
<a name="internationalization-notes"></a>
...
disconnects a connection on interface em2 and marks the device as unavailable for auto-connecting. As a result, no connection will automatically be activated on the device until the devices \*(Aqautoconnect\*(Aq is set to TRUE or the user manually activates a connection.
...
non-interactively adds an Ethernet connection tied to eth0 interface with automatic IP configuration (DHCP), and disables the connections
autoconnect
flag.
...
modifies autoconnect\*(Aq property in the \*(Aqconnection\*(Aq setting of \*(Aqethernet-2\*(Aq connection.

**nmcli con mod "Home Wi-Fi" wifi.mtu 1350**
modifies mtu\*(Aq property in the \*(Aqwifi\*(Aq setting of \*(AqHome Wi-Fi\*(Aq connection.
