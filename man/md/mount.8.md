...
at the directory
_dir_.
The option **-t type** is optional.  The
**mount**
...
specific to filesystem driver. The exception is **--all**, in this case
already mounted filesystems are ignored (see **--all** below for more details).


<a name="listing-the-mounts"></a>
...
<a name="indicating-the-device-and-filesystem"></a>
...
The command **lsblk --fs** provides overview of filesystems, LABELs and UUIDs
...
_/dev/disk/by-{label,uuid,partuuid,partlabel}_
...
<a name="the-files-etcfstab-etcmtab-and-procmounts"></a>
...
**mount**_ device_**|**_dir _**-o**_ options_
...
specify filesystem, otherwise **mount** may fail. For example it's bad idea
...
has been attached by "bind" operation. The _olddir_ and _newdir_ are
independent and the _olddir_ maybe be umounted.
...
**mount**(8)
since v2.31 ignores the **bind** flag from
_/etc/fstab_
on
**remount operation**
...
<a name="shared-subtree-operations"></a>
...
* **-a**,** --all**  
  ...
  **noauto**
  ...
  already mounted filesystems is cached during **mount --all**. It means
...
* **-N**,** --namespace **ns  
  Perform mount in namespace specified by _ns_.
  _ns_ is either PID of process running in that namespace
...
* **-o**,** --options **opts  
  Use the specified mount options.  The _opts_ argument is
...
* **--options-mode **mode  
  ...
  _mode_ can be one of
  **ignore**, **append**, **prepend** or **replace**.
  For example **append** means that options from fstab are appended to options from command line.
  Default value is **prepend** -- it means command line options are evaluated after fstab options.
  Note that the last option wins if there are conflicting ones.
  
* **--options-source **source  
  Source of default options.
  _source_ is comma separated list of
...
* **--options-source-force**  
  Use options from fstab/mtab even if both _device_ and _dir_ are specified.
...
* **-T**,** --fstab **path  
  Specifies an alternative fstab file.  If _path_ is a directory then the files
...
* **-t**,** --types **fstype  
  ...
  **-t**
  ...
  _/proc/filesystems_
  and
  _/lib/modules/$(uname -r)/kernel/fs_
...
<a name="filesystem-independent-mount-options"></a>
...
* .na  
  ...
  **Warning: the _context_ value might contain commas**,
...
* **remount**  
  ...
  **bind**
  flag has special semantic. See above, the subsection **Bind mounts**.
  
  The remount functionality follows the standard way the mount command works
  with options from fstab.  This means that **mount** does not
...
* **X-***  
  ...
  nor sent to the mount._type_ helpers nor to the
  **mount**(2)
  system call.  The suggested format is **X-_appname**.option_.
* **x-***  
  The same as **X-*** options, but stored permanently in the user space. It
...
<a name="filesystem-specific-mount-options"></a>
...
<a name="mount-options-for-adfs"></a>
...
* **ownmask=**_value_ and **othmask=**_value_  
  ...
  See also
  _/usr/src/linux/Documentation/filesystems/adfs.txt_.
  

<a name="mount-options-for-affs"></a>
...
<a name="mount-options-for-debugfs"></a>
...
<a name="mount-options-for-devpts"></a>
...
* newinstance  
  ...
  **newinstance**
  ...
  _pts/ptmx._
  See
  _Documentation/filesystems/devpts.txt_
...
<a name="mount-options-for-fat"></a>
...
<a name="mount-options-for-hfs"></a>
...
* **uid=**_n_**, gid=**_n_  
  Set the owner and group of all files.
  (Default: the UID and GID of the current process.)
* **dir_umask=**_n_**, file_umask=**_n_**, umask=**_n_  
...
<a name="mount-options-for-hpfs"></a>
...
<a name="mount-options-for-iso9660"></a>
...
<a name="mount-options-for-jfs"></a>
...
<a name="mount-options-for-msdos"></a>
...
<a name="mount-options-for-ncpfs"></a>
...
<a name="mount-options-for-ntfs"></a>
...
<a name="mount-options-for-overlay"></a>
...
<a name="mount-options-for-reiserfs"></a>
...
* **resize=**_number_  
  ...
  _ftp://ftp.namesys.com/pub/reiserfsprogs_.
* **user_xattr**  
...
<a name="mount-options-for-ubifs"></a>
...
<a name="mount-options-for-udf"></a>
...
<a name="mount-options-for-debugging-and-disaster-recovery"></a>
...
<a name="unused-historical-mount-options-that-may-be-encountered-and-should-be-removed"></a>
...
<a name="mount-options-for-ufs"></a>
...
<a name="mount-options-for-umsdos"></a>
...
<a name="mount-options-for-vfat"></a>
...
* **shortname=**_mode_  
  ...
  preferred one for display.  There are four _mode_s:
    * **lower**  
...
<a name="mount-options-for-usbfs"></a>
