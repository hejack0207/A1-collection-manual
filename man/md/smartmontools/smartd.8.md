...
To change this default location, please see the -l\*(Aq
...
every 30 minutes.
See the -i\*(Aq option below for additional details.

**smartd** can be configured at start-up using the configuration
file **/etc/smartmontools/smartd.conf** (Windows: **EXEDIR/smartd.conf**).
...
If **smartd** is started as a **systemd**(1) service and
Type=Notify\*(Aq is specified in the service file, the service manager
is notified after successful startup.
Other state changes are reported via systemd notify STATUS messages.
Notification of successful reloads (after **HUP** signal) is not supported.
To detect this process start-up type, **smartd** checks whether the
environment variable NOTIFY_SOCKET\*(Aq is set.
Note that it is required to set the -n\*(Aq (\*(Aq--nofork\*(Aq)
option in the ExecStart=/usr/sbin/smartd\*(Aq command line
if Type=Notify\*(Aq is used.
...
* **LINUX:**  
  ...
  .Sp
  If directive -d nvme\*(Aq
  
  or no -d\*(Aq directive
...
for _all_ possible SMART errors (corresponding to the -a\*(Aq
...
* **-A PREFIX, --attributelog=PREFIX**  
  ...
  attribute values) to files PREFIX\*(Aq\*(AqMODEL-SERIAL.ata.csv\*(Aq
  or PREFIX\*(Aq\*(AqVENDOR-MODEL-SERIAL.scsi.csv\*(Aq.
  ...
  .Sp
  If the PREFIX has the form /path/dir/\*(Aq (e.g.
  /var/lib/smartd/\*(Aq), then files \*(AqMODEL-SERIAL.ata.csv\*(Aq are
  created in directory /path/dir\*(Aq.
  If the PREFIX has the form /path/name\*(Aq (e.g.
  /var/lib/misc/attrlog-\*(Aq),
  then files nameMODEL-SERIAL.ata.csv\*(Aq are created in directory
  /path/\*(Aq.
...
* **-c FILE, --configfile=FILE**  
  ...
  the default location **/etc/smartmontools/smartd.conf**
  (Windows: **EXEDIR/smartd.conf**).
  If FILE does **not** exist, then **smartd** will print an error
  message and exit with nonzero status.
  Thus, -c /etc/smartmontools/smartd.conf\*(Aq can be used to verify the
  existence of the default configuration file.
  .Sp
  By using -\*(Aq for FILE, the configuration is read from standard input.
  This is useful for commands like:  
  **echo /dev/sdb -m user@home -M test | smartd -c - -q onecheck**  
  to perform quick and simple checks without a configuration file.
  
* **-C, --capabilities**  
  [Linux only] Use libcap-ng to drop unneeded Linux process **capabilities**(7).
...
* **-D, --showdirectives**  
  ...
  appear in the configuration file /etc/smartmontools/smartd.conf, and then exits.
...
* **-i N, --interval=N**  
  Sets the interval between disk checks to _N_ seconds, where
  _N_ is a decimal integer.  The minimum allowed value is ten and
...
* **-l FACILITY, --logfacility=FACILITY**  
  ...
  Here FACILITY is one of _local0_, _local1_, ..., _local7_,
  or _daemon_ [default].  If this command-line option is not used,
  ...
  than the default location, include (for example) -l local3\*(Aq in its
  start up argument list.
  Tell the syslog daemon to log all messages from facility **local3**
  to (for example) /var/log/smartd.log\*(Aq.
  .Sp
...
* **-p NAME, --pidfile=NAME**  
  Writes pidfile _NAME_ containing the **smartd** Process ID
...
* **-q WHEN, --quit=WHEN**  
  ...
  _errors_
  - Exit if there are no devices to monitor, or if any errors are found
  in the configuration file /etc/smartmontools/smartd.conf at startup or whenever it
  is reloaded.
  .Sp
  _nodevstartup_
  ...
  **/etc/smartmontools/smartd.conf** has errors, **smartd** will continue to run,
  ...
  .Sp
  This last option is intended for distribution-writers\*(Aq who want to
  ...
  **smartd** in normal daemon mode.  If **smartd** is unable to
  ...
  .Sp
  This option is intended to test whether the -s REGEX\*(Aq directives in
...
* **-r TYPE, --report=TYPE**  
  ...
  **smartd**
  ...
  comma then the integer with no spaces.  For example, _ataioctl,2_
  The default level is 1, so -r ataioctl,1\*(Aq and
  -r ataioctl\*(Aq are equivalent.
* **-s PREFIX, --savestates=PREFIX**  
  Reads/writes **smartd** state information from/to files
  PREFIX\*(Aq\*(AqMODEL-SERIAL.ata.state\*(Aq or
  PREFIX\*(Aq\*(AqVENDOR-MODEL-SERIAL.scsi.state\*(Aq.
  ...
  .Sp
  If the PREFIX has the form /path/dir/\*(Aq (e.g.
  /var/lib/smartd/\*(Aq), then files \*(AqMODEL-SERIAL.ata.state\*(Aq are
  created in directory /path/dir\*(Aq.
  If the PREFIX has the form /path/name\*(Aq (e.g.
  /var/lib/misc/smartd-\*(Aq),
  then files nameMODEL-SERIAL.ata.state\*(Aq are created in directory
  /path/\*(Aq.
  The path must be absolute, except if debug mode is enabled.
  .Sp
...
Directives.
For example:  
**Device: /dev/sda, SMART Attribute: 194 Temperature_Celsius changed from 94 to 93\*(Aq**  
Note that in this message, the value given is the Normalized\*(Aq not the
Raw\*(Aq Attribute value (the disk temperature in this case is about 22
Celsius).  The
**-R\*(Aq**
and
**-r\*(Aq**
...
**Device: /dev/sda, SMART Attribute: 194 Temperature_Celsius changed from 94 [Raw 22] to 93 [Raw 23]\*(Aq**  
Here the Raw values are the actual disk temperatures in Celsius.  The
way in which the Raw values are printed, and the names under which the
Attributes are reported, is governed by the various
**-v Num,Description\*(Aq**
...
**Device: /dev/sdc, Failed SMART Attribute: 5 Reallocated_Sector_Ct\*(Aq**  
 This loglevel is used for reporting enabled by the
**-H\*(Aq, -f\*(Aq, \*(Aq-l&nbsp;selftest\*(Aq,**
and
**-l&nbsp;error\*(Aq**
...
<a name="log-timestamp-timezone"></a>
...
using either the environment variable **TZ**\*(Aq or using a
...
**smartd** will work around this problem _if_ the time-zone is
set using **/etc/localtime**.  The work-around _fails_ if the
time-zone is set using the **TZ**\*(Aq variable (or a file that it
points to).
.Sp
...
* **0:**
  Daemon startup successful, or **smartd** was killed by a SIGTERM
...
* **5:**
  Config file does not exist (only returned in conjunction with the -c\*(Aq
  option).
* **6:**
...
* **10:**
  ...
  coding or compiler bug.  _Please_ report such failures to
...
* **/etc/smartmontools/smartd_warning.sh**  
  script run on warnings (see -w\*(Aq option above and \*(Aq-M exec\*(Aq
  directive on **smartd.conf**(5) man page).
  
* **/etc/smartmontools/smartd_warning.d/**  
  plugin directory for smartd warning script (see -m\*(Aq directive on
