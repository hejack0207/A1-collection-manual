...
self-tests.
**smartctl** also supports some features not related to SMART.
This version of **smartctl** is compatible with
...
is often derived from the device path but may need help with the -d\*(Aq
...
* **LINUX**:  
  Use the forms **"/dev/sd[a-z]"** for ATA/SATA and SCSI/SAS devices.
  For SCSI Tape Drives and Changers with TapeAlert support use the
  devices **"/dev/nst*"** and **"/dev/sg*"**.  For disks behind
  3ware controllers you may need **"/dev/sd[a-z]"** or
  **"/dev/twe[0-9]"**, **"/dev/twa[0-9]"** or **"/dev/twl[0-9]"**:
  see details below.
  For disks behind HighPoint RocketRAID controllers you may need
  **"/dev/sd[a-z]"**.  For disks behind Areca SATA RAID controllers,
  you need **"/dev/sg[2-9]"** (note that smartmontools interacts with
  ...
  are of the form **"/dev/cciss/c[0-9]d0"**.  For disks accessed via
  ...
  **"/dev/sg[0-9]*"**.
  ...
  See the **-d** option below, as well.
...
if -\*(Aq is specified as the device path, **smartctl** reads and
interprets it's own debug output from standard input.
See -r ataioctl\*(Aq below for details.

**smartctl** guesses the device type if possible.
If necessary, the -d\*(Aq option can be used to override this guess.
...
displayed with a leading **"0x"**, for example: "0xff".
...
* **-i, --info**  
  ...
  in the smartmontools database (see -v\*(Aq options below).  If so, the
  drive model family may also be printed.
  If -n\*(Aq (see below) is specified, the power mode of the drive is
  printed.
...
* **--identify[=[w][nvb]]**  
  ...
  two characters from the set wnvb\*(Aq.
  The character w\*(Aq enables printing of all 256 words.  The character
  n\*(Aq suppresses printing of bits, \*(Aqv\*(Aq enables printing of all
  bits from valid words, b\*(Aq enables printing of all bits.
  For example --identify=n\*(Aq (valid words, no bits) produces the
  shortest output and --identify=wb\*(Aq (all words, all bits) produces
  the longest output.
* **-a, --all**  
  ...
  to  
  -H -i -c -A -l error -l selftest -l selective\*(Aq  
  and for SCSI, this is equivalent to  
  -H -i -A -l error -l selftest\*(Aq.  
...
* **-x, --xall**  
  ...
  -H -i -g all -g wcreorder -c -A -f brief -l xerror,error
  -l xselftest,selftest -l selective -l directory -l scttemp -l scterc
  -l devstat -l defects -l sataphy.  
  and for SCSI, this is equivalent to  
  -H -i -g all -A -l error -l selftest -l background -l sasphy\*(Aq.  
...
* **--scan**  
  ...
  ([ATA] or [SCSI]) info.  May be used in conjunction with -d TYPE\*(Aq
...
* **--scan-open**  
  ...
  to autodetection (see also -d test\*(Aq).
  .Sp
  This option can be used to create a draft **smartd.conf** file.
  All options after --\*(Aq are appended to each output line.
  For example:
  .Vb 1
  smartctl --scan-open -- -a -W 4,45,50 -m admin@work &gt; smartd.conf
  .Ve
  .Sp
  Multiple -d TYPE\*(Aq options may be specified with
  --scan[-open]\*(Aq to combine the scan results of more than one TYPE.
* **-g NAME, --get=NAME**  
  Get non-SMART device settings.  See -s, --set\*(Aq below for further
  info.
  .Sp
...
* **-j, --json[=cgiosuvy]**  
  ...
  consists of one or more characters from the set cgiosuvy\*(Aq:  
  c\*(Aq: Outputs **c**ompact format without extra spaces and newlines.
  By default, output is pretty-printed.
  If used with YAML format, the indentation of arrays is reduced.  
  g\*(Aq: Outputs JSON structure as single assignments to allow the usage
  of **g**rep.
  Each assignment reflects the absolute path of a value.
  The syntax is compatible with **gron**:  
  json.KEY1[INDEX2].KEY3 = VALUE;\*(Aq.  
  o\*(Aq: Includes the full **o**riginal plaintext **o**utput of
  **smartctl** as a JSON array smartctl.output[]\*(Aq.  
  s\*(Aq: Outputs JSON object elements **s**orted by key.
  By default, object elements are ordered as generated internally.  
  v\*(Aq: Enables **v**erbose output of possible unsafe integers.
  If specified, values which may exceed JSON safe integer (53-bit) range are
  always output as a number (with some KEY\*(Aq) and a string
  (KEY_s\*(Aq), regardless of the actual value.
  Values which may exceed 64-bit range are also output as a little endian
  byte array (KEY_le\*(Aq).
  By default, the additional elements are only output if the value actually
  exceeds the range.  
  y\*(Aq: [NEW EXPERIMENTAL SMARTCTL FEATURE] Outputs in YAML format.
  .Sp
  The following two arguments are primarily indented for development:  
  i\*(Aq: Includes lines from the plaintext output which print info already
  **i**mplemented for JSON output.
  The lines appear as strings with key smartctl_NNNN_i\*(Aq.  
  u\*(Aq: Includes lines from the plaintext output which print info still
  **u**nimplemented for JSON output.
  The lines appear as strings with key smartctl_NNNN_u\*(Aq.
* **-q TYPE, --quietmode=TYPE**  
  Specifies that **smartctl** should run in one of the quiet modes
  described here.  The valid arguments to this option are:
  .Sp
  _errorsonly_
  - only print: For the -l error\*(Aq option, if nonzero, the number
  of errors recorded in the SMART error log and the power-on time when
  they occurred; For the -l selftest\*(Aq option, errors recorded in
  the device self-test log; For the -H\*(Aq option, SMART "disk failing"
  status or device Attributes (pre-failure or usage) which failed either now
  or in the past; For the -A\*(Aq option, device Attributes (pre-failure
  or usage) which failed either now or in the past.
  .Sp
  _silent_
  - print no output.  The only way to learn about what was found is to
  use the exit status of **smartctl** (see EXIT STATUS below).
  .Sp
  _noserial_
...
* **-d TYPE, --device=TYPE**  
  ...
  **smartctl -a -d megaraid,0 /dev/bus/0**  
  ...
  For PERC5/6 controllers: **megaraid\_sas\_ioctlN**
  .Sp
  ...
  **smartctl -a -d aacraid,1,0,4 /dev/sdb**
  .Sp
  Option -d sat,auto+...\*(Aq is implicitly enabled to detect SATA disks.
  Use -d scsi+aacraid,H,L,ID\*(Aq to disable it.
  .Sp
  ...
  **cat /proc/scsi/sg/device_hdr /proc/scsi/sg/devices**  
  ...
  .Sp
  Option -d sat,auto+...\*(Aq is implicitly enabled to detect SATA disks.
  Use -d scsi+cciss,N\*(Aq to disable it.
  .Sp
  ...
  **smartctl -a -d intelliprop,1+sat /dev/sda**    (under Linux)  
  ...
  Using this option with other devices may have undesirable side effects.
  .Sp
  ...
  WARNING: The ATA pass-through commands are issued via READ/WRITE commands
  to LBA 33 of the RAID volume.
  Using this option with other devices may overwrite this sector.  
  ...
  zero filled.
  The force\*(Aq flag disables this check.  
  WARNING: Original sector data is not written back if smartctl is aborted
  with a signal.
  .Sp
...
* **-T TYPE, --tolerance=TYPE**  
  ...
  **smartctl** messages such as "Feature X not implemented", followed
  ...
  .Sp
  _verypermissive_
  - equivalent to giving a large number of -T permissive\*(Aq options:
...
* **-r TYPE, --report=TYPE**  
  ...
  _ataioctl,2_
  The default level is 1, so -r ataioctl,1\*(Aq and \*(Aq-r ataioctl\*(Aq
  are equivalent.
  .Sp
  For testing purposes, the output of -r ataioctl,2\*(Aq can later be parsed
  by **smartctl** itself if -\*(Aq is used as device path argument.
...
* **-n POWERMODE[,STATUS], --nocheck=POWERMODE[,STATUS]**  
  ...
  Specifies if **smartctl** should exit before performing any
  checks when the device is in a low-power mode.
  It may be used to prevent a disk from being spun-up by **smartctl**.
  The power mode is ignored by default.
  .Sp
  Note: If this option is used it may also be necessary to specify the device
  type with the -d\*(Aq option.  Otherwise the device may spin up due to
  ...
  For example use -n standby,0\*(Aq to return success if a device is in
  SLEEP or STANDBY mode.
  Use -n standby,3\*(Aq to return a unique exit status in this case.
  .Sp
  The valid arguments to this option are:
  .Sp
  _never_
  - check the device always, but print the power mode if -i\*(Aq is
  specified.
  .Sp
  _sleep[,STATUS]_
...
* **-s VALUE, --smart=VALUE**  
  ...
  this option are _on_ and _off_.
  .Sp
  [ATA]
...
* **-o VALUE, --offlineauto=VALUE**  
  ...
  The valid arguments to this option are _on_ and _off_.
  .Sp
  ...
  **first** category, called "online" testing, has no effect on the
  performance of the device.  It is turned on by the -s on\*(Aq option.
  .Sp
  ...
  The -o on\*(Aq option causes this offline testing to be carried out,
  ...
  the -t offline\*(Aq option below, which causes a one-time offline test
  to be carried out immediately.
  .Sp
  The choice (made by the SFF-8035i and ATA specification authors) of
  the word _testing_ for these first two categories is unfortunate,
  ...
  These are visible with the -A\*(Aq and \*(Aq-l error\*(Aq options
  respectively.
  .Sp
  ...
  Attribute value table produced by the -A\*(Aq option indicates this in
  the UPDATED column.  Attributes of the first type are labeled
  "Offline" and Attributes of the second type are labeled "Always".
  .Sp
  The **third** category of testing (and the _only_ category for
  which the word testing\*(Aq is really an appropriate choice) is "self"
  testing.  This third type of test is only performed (immediately) when
  a command to run it is issued.
  The -t\*(Aq and \*(Aq-X\*(Aq options can be used to carry out and
  abort such self-tests; please see below for further details.
  .Sp
  Any errors detected in the self testing will be shown in the
  SMART self-test log, which can be examined using the -l selftest\*(Aq
  option.
  .Sp
...
* **-S VALUE, --saveauto=VALUE**  
  ...
  Attributes.  The valid arguments to this option are _on_
  and _off_.  Note that this feature is preserved across disk power
  ...
  is power-cycled.  If the GLTSD bit is set then smartctl -a\*(Aq will
  issue a warning.  Use _on_ to clear the GLTSD bit and thus enable
  saving counters to non-volatile storage.  For extreme streaming-video
  type applications you might consider using _off_ to set the GLTSD
  bit.
* **-g NAME, --get=NAME, -s NAME[,VALUE], --set=NAME[,VALUE]**  
  Gets/sets non-SMART device settings.
  Note that the --set\*(Aq option shares its short option \*(Aq-s\*(Aq
  with --smart\*(Aq.
  Valid arguments are:
  .Sp
  _all_
  ...
  -g aam -g apm -g lookahead -g security -g wcache -g rcache -g dsn\*(Aq
  .Sp
  _aam[,N|off]_
  ...
  the fastest (loudest) mode, off\*(Aq disables AAM.  Devices may support
  ...
  attempt to enable APM and set the specified value, off\*(Aq disables APM.
  ...
  the IDLE mode.  A value of 0 or off\*(Aq disables the standby timer.
  ...
  If -s standby,now\*(Aq is also specified, the drive is immediately
  ...
  [SCSI]
  [NEW EXPERIMENTAL SMARTCTL FEATURE]
  Only the set option with standby,off\*(Aq or
  standby,0\*(Aq is accepted and will place the SCSI disk
  ...
  -s standby,[N|off]\*(Aq is also specified.  
  [SCSI]
  [NEW EXPERIMENTAL SMARTCTL FEATURE]
  ...
  .Sp
  _wcache[,on|off]_
  - [SCSI] Gets/sets the Write Cache Enable\*(Aq (WCE) bit (if supported).
  The write cache is usually enabled by default.
  .Sp
  _wcache-sct[,ata|on|off[,p]]_
  ...
  If ,p\*(Aq is specified, the setting is preserved across power cycles.
  .Sp
  _wcreorder[,on|off[,p]]_
  ...
  .Sp
  _rcache[,on|off]_
  - [SCSI only] Gets/sets the Read Cache Disable\*(Aq (RCE) bit.
  Off\*(Aq value disables read cache (if supported).
...
* **-H, --health**  
  ...
  **or**
  ...
  this happens, use the -a\*(Aq option to get more information, and
  **get your data off the disk and to someplace safe as soon as you can.**
  ...
  In this case, **smartctl** prints a warning and checks whether any
  ...
  This means that each alert condition is reported only once by **smartctl**
...
* **-A, --attributes**  
  ...
  **not** mean that your disk is about to fail!  It only has this
  ...
  a dash: -\*(Aq) then this Attribute is OK now (not failing) and has
...
* **-l TYPE, --log=TYPE**  
  ...
  .Vb 5
     **ABRT**:  Command **AB**o**RT**ed
     **AMNF**:  **A**ddress **M**ark **N**ot **F**ound
     **CCTO**:  **C**ommand **C**ompletion **T**imed **O**ut
     **EOM**:   **E**nd **O**f **M**edia
     **ICRC**:  **I**nterface **C**yclic **R**edundancy **C**ode (CRC) error
     **IDNF**:  **ID**entity **N**ot **F**ound
     **ILI**:   (packet command-set specific)
     **MC**:    **M**edia **C**hanged
     **MCR**:   **M**edia **C**hange **R**equest
     **NM**:    **N**o **M**edia
     **obs**:   **obs**olete
     **TK0NF**: **T**rac**K 0 N**ot **F**ound
     **UNC**:   **UNC**orrectable Error in Data
     **WP**:    Media is **W**rite **P**rotected
  .Ve
  ...
  spec are listed like this: **READ LONG (w/ retry) [OBS-4]**,
  indicating that the command became obsolete with or in the ATA-4
  specification.  Similarly, the notation **[RET-**_N_**]** is
  used to indicate that a command was retired in the ATA-_N_
  ...
  **[NS]**, meaning non-standard.
  ...
  shall not include errors attributed to the receipt of faulty commands."
  The definitions of these terms are:  
  **UNC** (**UNC**orrectable): data is uncorrectable.  This refers
  ...
  **IDNF** (**ID N**ot **F**ound): user-accessible address could
  ...
  log (see -l error\*(Aq above), it provides sufficient space to log
  ...
  This number can be changed by the optional parameter NUM.
  .Sp
  If ,error\*(Aq is appended and the Extended Comprehensive SMART error
  ...
  -t\*(Aq option described below.  For each of the most recent
  ...
  can be run using the -t\*(Aq option described below (using the ATA
  test terminology).
  .Sp
  _xselftest[,NUM][,selftest]_
  - [ATA only] prints the Extended SMART self-test log (General Purpose
  Log address 0x07).  Unlike the SMART self-test log (see -l selftest\*(Aq
  ...
  This number can be changed by the optional parameter NUM.
  .Sp
  If ,selftest\*(Aq is appended and the Extended SMART self-test log is not
  supported, the old SMART self-test log is printed.
  .Sp
  _selective_
  - [ATA only] Please see the -t select\*(Aq option below for a
  ...
  self-test has completed (see -t afterselect\*(Aq option) and the time
  ...
  SL directory by -l directory,q\*(Aq or \*(Aq-l directory,s\*(Aq
  respectively.
  .Sp
  _background_
  ...
  Counters (General Purpose Log address 0x11).  If -l sataphy,reset\*(Aq
  ...
  Protocol Specific log page (log page 0x18).  If -l sasphy,reset\*(Aq
  is specified, all counters are reset after reading the values.
  .Sp
  _gplog,ADDR[,FIRST[-LAST|+SIZE]]_
  ...
  listed in the log directory (see -l directory\*(Aq above).
  ...
  LAST can be set to max\*(Aq to specify the last page of the log.
  .Sp
  _smartlog,ADDR[,FIRST[-LAST|+SIZE]]_
  - [ATA only] prints a hex dump of any log accessible via SMART Read
  Log command.  See -l gplog,...\*(Aq above for parameter syntax.
  .Sp
  ...
  .Sp
  The hex dump format is compatible with the xxd -r\*(Aq command.
  This command:
  .Vb 1
  ...
  This has the same effect as -l devstat,7\*(Aq, see above.
  .Sp
  _ssd_
...
* **-v ID,FORMAT[:BYTEORDER][,NAME], --vendorattribute=ID,FORMAT...**  
  ...
  If N\*(Aq is specified as ID, the settings for all Attributes are changed.
  .Sp
  The optional BYTEORDER consists of 1 to 8 characters from the
  set 012345rvwz\*(Aq.
  The characters 0\*(Aq to \*(Aq5\*(Aq select the byte 0 to 5 from the
  48-bit raw value, r\*(Aq selects the reserved byte of the attribute
  data block, v\*(Aq selects the normalized value, \*(Aqw\*(Aq selects
  the worst value and z\*(Aq inserts a zero byte.
  The default BYTEORDER is 543210\*(Aq for all 48-bit formats,
  r543210\*(Aq for the 54-bit formats, and \*(Aq543210wv\*(Aq for the
  64-bit formats.
  For example, -v 5,raw48:012345\*(Aq prints the raw value of
  ...
  The -P showall\*(Aq option reports an error if this is the case.
  .Sp
  _-v help_
  ...
  .Sp
  The following old arguments to -v\*(Aq are also still valid:
  .Sp
  _9,minutes_
  - same as:
  _9,min2hour,Power_On_Minutes._
  .Sp
  _9,seconds_
  - same as:
  _9,sec2hour,Power_On_Seconds._
  .Sp
  _9,halfminutes_
  - same as:
  _9,halfmin2hour,Power_On_Half_Minutes._
  .Sp
  _9,temp_
  - same as:
  _9,tempminmax,Temperature_Celsius._
  .Sp
  _192,emergencyretractcyclect_
  - same as:
  _192,raw48,Emerg_Retract_Cycle_Ct_
  .Sp
  _193,loadunload_
  - same as:
  _193,raw24/raw24._
  ...
  _194,10xCelsius_
  - same as:
  _194,temp10x,Temperature_Celsius_x10._
  .Sp
  _194,unknown_
  - same as:
  _194,raw48,Unknown_Attribute._
  .Sp
  _197,increasing_
  - same as:
  _197,raw48,Total_Pending_Sectors._
  ...
  _198,increasing_
  - same as:
  _198,raw48,Total_Offl_Uncorrectabl._
  Also means that Attribute number 198 (Offline Uncorrectable Sector Count)
  is not reset if uncorrectable sectors are reallocated
  (see **smartd.conf**(5) man page).
  .Sp
  _198,offlinescanuncsectorct_
  - same as:
  _198,raw48,Offline_Scan_UNC_SectCt._
  .Sp
  _200,writeerrorcount_
  - same as:
  _200,raw48,Write_Error_Count._
  .Sp
  _201,detectedtacount_
  - same as:
  _201,raw48,Detected_TA_Count._
  .Sp
  _220,temp_
  - same as:
  _220,tempminmax,Temperature_Celsius._
* **-F TYPE, --firmwarebug=TYPE**  
  ...
  execution status (see options -c\*(Aq or \*(Aq-a\*(Aq above)
  accordingly.
  .Sp
  _xerrorlba_
...
* **-P TYPE, --presets=TYPE**  
  ...
  Attribute interpretation (-v N,something\*(Aq) command-line options or
  explicit -F\*(Aq command-line options..
  .Sp
  _ignore_
  - do not use presets.
  .Sp
  ...
  .Sp
  The -P showall\*(Aq option takes up to two optional arguments to
  ...
  .Vb 1
    smartctl -P showall MODEL\*(Aq
  .Ve
  lists all entries matching MODEL, and the command:
  .Vb 1
    smartctl -P showall MODEL\*(Aq \*(AqFIRMWARE\*(Aq
  .Ve
...
* **-B [+]FILE, --drivedb=[+]FILE**  
  ...
  the built in database by default.  If +\*(Aq is specified, then the new
  entries prepend the built in entries.
  .Sp
  Optional entries are read from the file
  
  **/etc/smartmontools/smart\_drivedb.h**
...
* **-t TEST, --test=TEST**  
  Executes TEST immediately.  The -C\*(Aq option can be used in
  ...
  .Sp
  All -t TEST\*(Aq commands can be given during normal system operation
  unless captive mode (-C\*(Aq option) is used.
  ...
  -l error\*(Aq option.
  .Sp
  If the -c\*(Aq option to **smartctl** shows that the device has the
  "Suspend Offline collection upon new command" capability then you can
  track the progress of the Immediate Offline test using the -c\*(Aq
  option to **smartctl**.  If the -c\*(Aq option show that the device
  ...
  try to track the progress of the test with -c\*(Aq, as it will abort
  the test.
  .Sp
  _offline_
  ...
  captive mode - see the -C\*(Aq option below).  This is a
  ...
  the -l selftest\*(Aq option.  Note that on some disks the progress of
  the self-test can be monitored by watching this log during the self-test;
  with other disks use the -c\*(Aq option to monitor progress.
  .Sp
  _short_
  ...
  system operation (unless run in captive mode - see the -C\*(Aq option
  below).
  .Sp
  _long_
  ...
  -C\*(Aq option below).
  .Sp
  _select,N-M, select,N+SIZE_
  - [ATA only] runs a SMART Selective Self Test, to test a **range**
  ...
  A span at the end of a disk can be specified by N-**max**.
  .Sp
  ...
  The -t\*(Aq option can be given up to five times, to test
  ...
  -l selftest\*(Aq option to smartctl.
  .Sp
  ...
  done in captive mode - see the -C\*(Aq option below).
  .Sp
  ...
  by future uses of -t select,next\*(Aq.
  .Sp
  _select,cont[+SIZE]_
  - [ATA only] performs a redo\*(Aq (above) if the self test status
  reports that the last test was aborted by the host.
  Otherwise it run the next\*(Aq (above) test.
  .Sp
  _afterselect,on_
  ...
  the _select,N-M_ options above.  If the LBAs that have been
  ...
  with one or more of the _select,N-M_ options above.  The value of this
  ...
  option must be use together with one or more of the _select,N-M_
  options above.
  ...
  (e.g. 0x01: -t short\*(Aq, 0x7f: \*(Aq-X\*(Aq, 0x82:
  -C -t long\*(Aq).
  .Sp
  ...
  The subcommand 0x40 (-t vendor,0x40\*(Aq) clears the timed workload
...
* **-C, --captive**  
  ...
  offline or if the \*(Aq-t\*(Aq option is not used.
  .Sp
...
<a name="ata-scsi-command-sets-and-sat"></a>
...
In some cases the -d sat\*(Aq option is needed on the command line.
...
log visible with the -l selftest\*(Aq option after it has completed.
...
with the -A\*(Aq option.  If any device errors occur, they are logged to
the SMART error log, which can be seen with the -l error\*(Aq option.
...
* **Bit 1:**
  ...
  or device is in a low-power mode (see -n\*(Aq option above).
* **Bit 2:**
  Some SMART or other ATA command to the disk failed, or there was a checksum
  error in a SMART data structure (see -b\*(Aq option above).
* **Bit 3:**
...
* **/usr/share/smartmontools/drivedb.h**  
  drive database (see -B\*(Aq option).
  
* **/etc/smartmontools/smart_drivedb.h**  
  optional local drive database (see -B\*(Aq option).
  .Sp
...
volume of the AT Attachment with Packet Interface-7\*(Aq (ATA/ATAPI-7)
