...
```
.IX Header "SYNOPSIS" openssl enc -\f(BIcipher [-help] [-list] [-ciphers] [-in filename] [-out filename] [-pass arg] [-e] [-d] [-a] [-base64] [-A] [-k password] [-kfile filename] [-K key] [-iv \s-1IV\s0] [-S salt] [-salt] [-nosalt] [-z] [-md digest] [-iter count] [-pbkdf2] [-p] [-P] [-bufsize number] [-nopad] [-debug] [-none] [-rand file...] [-writerand file] [-engine id] 
 openssl [cipher] [...]
```
...
* **-nosalt**  
  .IX Item "-nosalt"
  Don't use a salt in the key derivation routines. This option **\s-1SHOULD NOT\s0** be
...
* **-K key**  
  ...
  key given with the **-K** option will be used and the \s-1IV\s0 generated from the
...
**openssl enc -cipher**. The first form doesn't work with
...
The **-salt** option should **\s-1ALWAYS\s0** be used if the key is being derived
...
the **-ciphers** option (that is **openssl enc -ciphers**) produces a
...
The **enc** interface by necessity must begin streaming output (e.g.,
to standard output when **-out** is not used) before the authentication
tag could be validated, leading to the usage of **enc** in pipelines
...
integrity upon reuse of key/iv/nonce, and since **enc** places the
entire burden of key/iv/nonce management upon the user, the risk of
exposing \s-1AEAD\s0 modes is too great to allow.  These key/iv/nonce
management issues also affect other modes currently exposed in **enc**,
