...
The **start** command skips the following steps if
...
* 3.  
  Starts **ovsdb-server**, unless the **--no-ovsdb-server** command
  option is given.
* 4.  
  Initializes a few values inside the database.
* 5.  
  If the **--delete-bridges** option was used, deletes all of the
  bridges from the database.
* 6.  
  If the **--delete-transient-ports** option was used, deletes all ports
  that have **other\_config:transient** set to true.

The **start** command skips the following step if
**ovs-vswitchd** is already running, or if the **--no-ovs-vswitchd**
...
* **--system-id=uuid**  
  ...
  **external-ids:system-id** in the database's **Open\_vSwitch**
...
* When **random** is specified, **ovs-ctl** will generate a random
...
* When not specified, **ovs-ctl** uses values from the optional
  ...
  **FILES**) or it uses the **lsb\_release** program, if present, to
  provide reasonable defaults.

The following options are also likely to be useful:

* --external-id=  
  Sets **external-ids:_name** to value_ in the database's
  **Open\_vSwitch** table.  Specifying this option multiple times adds
...
* **--ovs-user=user[:group]**  
  ...
  **--user** option to the **ovsdb-server** and **ovs-vswitchd**
...
* **--no-monitor**  
  ...
  **ovsdb-server**, requesting that it spawn a process monitor which will
...
* **--no-mlockall**  
  ...
  **ovs-vswitchd**, requesting that it lock all of its virtual
...
* **--ovsdb-server-priority=niceness**  
  .IQ "**--ovs-vswitchd-priority=niceness**"
  Sets the **nice**(1) level used for each daemon.  All of them
  default to **-10**.
* **--ovsdb-server-wrapper=wrapper**  
  .IQ "**--ovs-vswitchd-wrapper=wrapper**"
  Configures the specified daemon to run under _wrapper_, which is
  one of the following:
    * **valgrind**  
      Run the daemon under **valgrind**(1), if it is installed, logging to
      daemon**.valgrind.log.pid** in the log directory.
    * **strace**  
      Run the daemon under **strace**(1), if it is installed, logging to
...
* Each of the wrappers can expose bugs in Open vSwitch that lead to
  ...
  **strace** wrappers greatly slow daemon operations so they should
  ...
  quickly fill small disk partitions.  The **glibc** wrapper is less
...
* **--extra-dbs=file**  
  Adds _file_ as an extra database for **ovsdb-server** to serve
  out.  Multiple space-separated file names may also be specified.
  _file_ should begin with **/**; if it does not, then it will be
  taken as relative to _dbdir_.

<a name="the-stop-command"></a>
...
**--no-ovs-vswitchd** options as that of the **start**
command. 

This command does nothing and finishes successfully if the OVS daemons
aren't running.

<a name="the-restart-command"></a>
...
<a name="the-force-reload-kmod-command"></a>
...
* 3.  
  Stops the Open vSwitch daemons, as if by a call to ovs-ctl
  stop.
* 4.  
...
* 6.  
  Starts OVS back up, as if by a call to **ovs-ctl start**.  This
...
**force-kmod-reload** internally stops and starts OVS, so it
accepts all of the options accepted by the **start** command except
for the **--no-ovs-vswitchd** option.

<a name="the-load-kmod-command"></a>
...
the **start** command. The motivation for providing the **load-kmod**
...
<a name="the-enable-protocol-command"></a>

# The ``Enable\-Protocol'' Command


The **enable-protocol** command checks for rules related to a
specified protocol in the system's **iptables**(8) configuration.  If there
...
* **--sport=sport**  
  ...
  and allowed only with **--protocol=tcp** or
  **--protocol=udp**.

<a name="the-delete-transient-ports-command"></a>

# The ``Delete\-Transient\-Ports'' Command

Deletes all ports that have the **other\_config:transient** value set to true.

<a name="the-help-command"></a>
...
By default, **ovs-ctl** will control the **ovsdb-server**, and
the **ovs-vswitchd** daemons. The following options restrict that control
to exclude one or the other:

* **--no-ovsdb-server**  
  Specifies that the **ovs-ctl** commands **start**, **stop**, and
  **restart** should not modify the running status of **ovsdb-server**.
* **--no-ovs-vswitchd**  
  Specifies that the **ovs-ctl** commands **start**, **stop**, and
  **restart** should not modify the running status of **ovs-vswitchd**.
  It is an error to include this option with the **force-reload-kmod**
  command.
...
* **PATH**  
  **ovs-ctl** does not hardcode the location of any of the programs
  that it runs.  **ovs-ctl** will add the _sbindir_ and
...
* **OVS\_LOGDIR**  
  ...
  respective **configure** option, both for **ovs-ctl** itself and
...
* rundir**/openvswitch/db.sock**  
  ...
  **ovsdb-server** (use **--db-sock** to override this
  location).
...
* sysconfdir**/openvswitch/system-type.conf**  
  .IQ "sysconfdir**/openvswitch/system-version.conf**"
  The **system-type**  and **system-version** values stored in the database's
  **Open\_vSwitch** table when not specified as a command-line option.
...
**xenserver/etc\_init.d\_openvswitch** in the Open vSwitch source
