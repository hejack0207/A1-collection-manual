...
the leafs are already handled by **(1)**. Still, it's perfectly possible to
...
the 2nd period _and_ the gap - if the gap is small enough.
...
* E()  
  ...
  service curve, _but in case of a convex curve, uses its 2nd slope only._
* D()  
...
* V()  
  ...
  function of a virtual time - see **LINKSHARE CRITERION** section for
...
RT criterion _ignores class hierarchy_ and guarantees precise bandwidth and
...
it's constructed using _only_ RT service curve's 2nd slope (in our example
 7Mbit).

The effect of such E() - packets will be sent earlier, and at the same time
D() _will_ be updated - so the current deadline time calculated from it
...
secondary to LS one, and used _only_ if there's a risk of violating precise
...
service curve is an extension to LS one - see **UPPERLIMIT CRITERION**
section).
...
_far_ later than A2's one. Considering the type of decision made by LS
...
* 2)  
  ...
  time [fairly or not]:
    ...
    B  - 3Mbit, then 7Mbit
      ...
      What happens now? B (choice made by R) will _always_ get 7 Mbit as R is
...
happen _only_ in the first segment, then there's little wrong with
...
hardware dependent and can go _far_ beyond the highest 'timer frequency'
setting available.
