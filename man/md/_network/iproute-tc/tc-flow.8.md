# flow filter in tc(8) - flow based traffic control filter

iproute2, 20 Oct 2015
...
<synopsis>
.RS .in +8 .ti -8 tc filter ... flow map key  KEY [ OPS ] [ OPTIONS ]  .RE .TP Hashing mode:
</synopsis>

<synopsis>
.RS .in +8 .ti -8 tc filter ... flow hash keys  KEY_LIST [  perturb secs ] [ OPTIONS ]  .RE
</synopsis>

<synopsis>
.in +8 .ti -8 OPS := [ OPS ] OP
</synopsis>

<synopsis>
.ti -8 OPTIONS := [  divisor NUM ] [  baseclass ID ] [  match EMATCH_TREE ] [  action ACTION_SPEC ]
</synopsis>

<synopsis>
.ti -8 KEY_LIST := [ KEY_LIST ] KEY
</synopsis>

<synopsis>
.ti -8 OP := {  or | and | xor | rshift | addend }  NUM
</synopsis>
...
<synopsis>
.ti -8 KEY := {  src | dst | proto | proto-src | proto-dst | iif |  priority | mark | nfct | nfct-src | nfct-dst |  nfct-proto-src | nfct-proto-dst | rt-classid | sk-uid |  sk-gid | vlan-tag | rxhash }
```
...
* **baseclass**_ ID_  
  ...
  may be
  **root**, **none**
  or a hexadecimal class ID in the form [_X**:**]Y_. _X_ must
  match qdisc's/class's major handle (if omitted, the correct value is chosen
  automatically). If the whole **baseclass** is omitted, _Y_ defaults
  to 1.
* **divisor**_ NUM_  
