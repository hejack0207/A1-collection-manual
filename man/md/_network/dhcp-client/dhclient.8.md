...
On startup, **dhclient** reads the dhclient.conf
...
* **-6**  
  ...
  **-4**.  The **-S -T -P -N** and
  **-D** arguments provide more control over aspects of the DHCPv6
...
* **-4o6**_&nbsp;port_  
  ...
  to ::1 _port_ and _port + 1_. Both clients must
...
* **-x**  
  ...
  Kills existing **dhclient** process as previously recorded in the
...
* **-g**_&nbsp;relay_  
  ...
  Set the giaddr field of all packets to the _relay_ IP address
...
_Options available for DHCPv6 mode:_

* **-S**  
  
  Use Information-request to get only stateless configuration parameters
  (i.e., without address).  This implies **-6**.  It also doesn't
  rewrite the lease database.
  
* **-T**  
  
  Ask for IPv6 temporary addresses, one set per **-T** flag.  This
  implies **-6** and also disables the normal address query.
  See **-N** to restore it.
* **-P**  
...
* **-R**  
  ...
  **-N**, **-T**, or **-P** options.  Normally even if
...
* **-D**_&nbsp;LL&nbsp;or&nbsp;LLT_  
  ...
  DHCPv6 **dhclient** creates an identifier based on the link-layer address
  (DUID-LL) if it is running in stateless mode (with **-S**, not
  ...
  stateful mode (without **-S**, requesting an address).  When DHCPv4
  is configured to use a DUID using **-i** option the default is to use
  a DUID-LLT.  **-D**
  overrides these default, with a value of either _LL_ or _LLT_.
* **-N**  
  
  Restore normal address query for IPv6. This implies **-6**.
  It is used to restore normal operation after using **-T** or **-P**.
  Multiple addresses can be requested with multiple **-N** flags.
* **--dad-wait-time**_&nbsp;seconds_  
...
* **-cf**_&nbsp;config-file_  
  ...
  **/etc/dhcp/dhclient.conf**
  is used.  See **dhclient.conf(5)** for a description of this file.
* **-df**_&nbsp;duid-lease-file_  
...
* **-lf**_&nbsp;lease-file_  
  ...
  **/var/lib/dhclient/dhclient.leases**
  is used.  See **dhclient.leases(5)** for a description of this file.
* **-pf**_&nbsp;pid-file_  
...
* **-sf**_&nbsp;script-file_  
  ...
  **/usr/sbin/dhclient-script**
  is used.  See **dhclient-script(8)** for a description of this file.
...
**/usr/sbin/dhclient-script,**
**/etc/dhcp/dhclient.conf, /var/lib/dhclient/dhclient.leases, /var/run/dhclient.pid,**
**/var/lib/dhclient/dhclient.leases~.**
