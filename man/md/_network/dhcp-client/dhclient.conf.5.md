...
1/2 to 1 1/2 times the _time_ specified.  It defaults to fifteen
seconds.
...
_-R_ option is given on the command line then messages that
...
are called _DHCP Options_.  DHCP Options are defined in
 **dhcp-options(5)**.
 ...
 **[ also ] request [ [ _option-space** . ] option_ ] [**, ... ];**
 ...
 **[ also ] require [ [ _option-space** . ] option_ ] [**, ... ];**
...
declaration as described in **dhcp-options(5)**.  Options that are
...
configure the _fqdn_ option on the client, as follows:
...
The _fqdn.fqdn_ option **MUST** be a fully-qualified domain
name.  You **MUST** define a zone statement for the zone to be
updated.  The _fqdn.encoded_ option may need to be set to
_on_ or _off_, depending on the DHCP server you are using.
...
the **do-forward-updates** statement.  _Flag_ should be **true**
 ...
 **lease {** _lease-declaration_ [ ... lease-declaration ] **}**
...
define one or more _fixed_ leases in the client configuration file
...
The _dhcp-vendor-identifier_ can be used to request a specific
...
using.  The **rebind** statement defines the time at which the dhcp
client should begin to try to contact _any_ dhcp server in order
...
these two formats depending on if the **db-time-format** configuration
parameter has been set to _default_ or _local_.

If it is set to _default_, then _date_ values appear as follows:
...
If the **db-time-format** configuration was set to _local_, then
the _date_ values appear as follows:

 **epoch** _&lt;seconds-since-epoch&gt;_**; #** &lt;day-name&gt; &lt;month-name&gt;
&lt;day-number&gt; &lt;hours&gt;**:**_&lt;minutes&gt;_**:**_&lt;seconds&gt; &lt;year&gt;_

The _seconds-since-epoch_ is as according to the system's local clock (often
referred to as "unix time").  The **#** symbol supplies a comment that
...
If the time is infinite in duration, then the _date_ is **never**
...
used for printing times in leases files.  The _default_ format provides
day-and-time in UTC, whereas _local_ uses a seconds-since-epoch to store
...
The _cidr-ip-address_ configuration type is of the
form _ip-address_[**/_prefixlen**], where ip-address_ is a
...
__reject_ 192.168.0.0**/**16**,** 10.0.0.5**;**_
...
interface named _real-name_, with its own lease and its own
...
where a lease has expired.  See **dhclient-script(8)** for more
information.
 ...
 **hardware** _link-type mac-address_**;**
...
is not recommended.  The _link-type_ corresponds to the interface's
link layer type (example: \'ethernet\'), while the _mac-address_ is
a string of colon-separated hexadecimal values for octets.

 **anycast-mac** _link-type mac-address_**;**
...
client.  The _link-type_ and _mac-address_ parameters are configured
in a similar manner to the **hardware** statement.
