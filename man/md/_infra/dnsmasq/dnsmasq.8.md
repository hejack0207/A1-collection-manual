...
* **-S, --local, --server=[/[&lt;domain&gt;]/[domain/]][&lt;ipaddr&gt;[#&lt;port&gt;]][@&lt;source-ip&gt;|&lt;interface&gt;[#&lt;port&gt;]]**  
  ...
  not suppress reading of /etc/resolv.conf, use **--no-resolv** to do that. If one or more
  ...
  **//**
  ...
  More than one **--server** flag is allowed, with
...
* **--rev-server=&lt;ip-address&gt;/&lt;prefix-len&gt;[,&lt;ipaddr&gt;][#&lt;port&gt;][@&lt;source-ip&gt;|&lt;interface&gt;[#&lt;port&gt;]]**  
  ...
  **--rev-server=1.2.3.0/24,192.168.0.1**
...
* **-A, --address=/&lt;domain&gt;[/&lt;domain&gt;...]/[&lt;ipaddr&gt;]**  
  ...
  both IPv4 and IPv6 addresses for a domain, use repeated **--address** flags.
  To include multiple IP addresses for a single query, use
  **--addn-hosts=&lt;path&gt;** instead.
  Note that /etc/hosts and DHCP leases override this for individual
  names. A common use of this is to redirect the entire doubleclick.net
  domain to some friendly local web server to avoid banner ads. The
  domain specification works in the same was as for **--server**, with
  the additional facility that **/#/** matches any domain. Thus
  **--address=/#/1.2.3.4** will always return **1.2.3.4** for any
  query not answered from **/etc/hosts** or DHCP and not sent to an
  upstream nameserver by a more specific **--server** directive. As for
  **--server**, one or more domains with no address returns a
  no-such-domain answer, so **--address=/example.com/** is equivalent to
  ...
  **--address=/example.com/#** will return NULL addresses for example.com and
  its subdomains. This is partly syntactic sugar for **--address=/example.com/0.0.0.0**
  and **--address=/example.com/::** but is also more efficient than including both
...
* **-F, --dhcp-range=[tag:&lt;tag&gt;[,tag:&lt;tag&gt;],][set:&lt;tag&gt;,]&lt;start-IPv6addr&gt;[,&lt;end-IPv6addr&gt;|constructor:&lt;interface&gt;][,&lt;mode&gt;][,&lt;prefix-len&gt;][,&lt;lease time&gt;]**  
  ...
  **ra-only, slaac, ra-names, ra-stateless, ra-advrouter, off-link.**
...
* **-G, --dhcp-host=[&lt;hwaddr&gt;][,id:&lt;client_id&gt;|*][,set:&lt;tag&gt;][tag:&lt;tag&gt;][,&lt;ipaddr&gt;][,&lt;hostname&gt;][,&lt;lease_time&gt;][,ignore]**  
  ...
  **--cname**
  ...
  instance
  **--dhcp-host=00:20:e0:3b:13:af,ignore**
  This is
  ...
  **--dhcp-host=06-00:20:e0:3b:13:af,1.2.3.4**
  will only match a
...
* **-6 --dhcp-script=&lt;path&gt;**  
  ...
  and lease expiry if **--script-on-renewal** is set).
...
_/etc/dnsmasq.conf,_
if it exists. (On
FreeBSD, the file is 
_/usr/local/etc/dnsmasq.conf_
) (but see the 
**--conf-file**
and
**--conf-dir**
...
**--no-daemon**
mode or when full logging is enabled (**--log-queries**), a complete dump of the
...
_/etc/ppp/resolv.conf_
and
_/etc/dhcpc/resolv.conf_
...
<a name="authoritative-configuration"></a>
...
**--mx-host, --srv-host, --dns-rr, --txt-record, --naptr-record, --caa-record,**
...
_/etc/resolv.conf_
_/var/run/dnsmasq/resolv.conf_
_/etc/ppp/resolv.conf_
_/etc/dhcpc/resolv.conf_
...
_/var/lib/dnsmasq/dnsmasq.leases_

_/var/db/dnsmasq.leases_

_/var/run/dnsmasq.pid_
...
This manual page was written by Simon Kelley &lt;[simon@thekelleys.org](mailto:simon@thekelleys.org).uk&gt;.
