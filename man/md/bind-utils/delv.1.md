...
```
.HP \w'delv&nbsp;'u delv [@server] [[-4] | [-6]] [-a&nbsp;anchor-file] [-b&nbsp;address] [-c&nbsp;class] [-d&nbsp;level] [-i] [-m] [-p&nbsp;port#] [-q&nbsp;name] [-t&nbsp;type] [-x&nbsp;addr] [name] [type] [class] [queryopt...] .HP \w'delv&nbsp;'u delv [-h] .HP \w'delv&nbsp;'u delv [-v] .HP \w'delv&nbsp;'u delv [queryopt...] [query...]
```
...
_address_. This must be a valid address on one of the hosts network interfaces or "0.0.0.0" or "::". An optional source port may be specified by appending "#&lt;port&gt;"
...
_name_. While the query name can be specified without using the
...
_type_, which can be any valid query type supported in BIND 9 except for zone transfer types AXFR and IXFR. As with
