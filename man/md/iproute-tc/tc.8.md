...
```
tc [ OPTIONS ] qdisc [ add | change | replace | link | delete ] dev DEV .B [ parent qdisc-id | root ] [ handle qdisc-id ] [ ingress_block BLOCK_INDEX ] [ egress_block BLOCK_INDEX ] qdisc [ qdisc specific parameters ] 
...
<synopsis>
 ...
 .ti 8 OPTIONS := { [ -force ] -b[atch] [ filename ] | [ -n[etns] name ] | [ -N[umeric] ] | [ -nm | -nam[es] ] | [ { -cf | -c[onf] } [ filename ] ]  [ -t[imestamp] \] | [ -t[short] | [ -o[neline] ] }
</synopsis>

<synopsis>
.ti 8 FORMAT := { -s[tatistics] | -d[etails] | -r[aw] | -i[ec] | -g[raph] | -j[json] | -p[retty] | -col[or] }
```
...
* skb-&gt;priority  
  Userspace programs can encode a _class-id_ in the 'skb-&gt;priority' field using
...
* get  
  Displays a single filter given the interface _DEV_, _qdisc-id_,
  _priority_, _protocol_ and _filter-id_.
...
* **file**  
  If the file option is given, the **tc** does not listen to kernel events, but opens
