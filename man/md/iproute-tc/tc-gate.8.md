...
iproute2, 12 Mar 2020

```
tc " ... " action gate .ti +8 [ base-time BASETIME ] [ clockid CLOCKID ] .ti +8 sched-entry <gate state> <interval 1> [ <internal priority> <max octets> ] .ti +8 sched-entry <gate state> <interval 2> [ <internal priority> <max octets> ] .ti +8 sched-entry <gate state> <interval 3> [ <internal priority> <max octets> ] .ti +8 ...... .ti +8 sched-entry <gate state> <interval N> [ <internal priority> <max octets> ]
```
...
# tc filter add dev eth0 parent ffff: protocol ip &nbsp;          flower skip_hw src_ip 192.168.0.20 &nbsp;          action gate index 2 clockid CLOCK_TAI &nbsp;          base-time 200000000000ns &nbsp;          sched-entry open 200000000ns -1 8000000b &nbsp;          sched-entry close 100000000ns
...
.EX
# tc qdisc add dev eth0 ingress
# tc filter add dev eth0 parent ffff:  protocol ip \	flower ip_proto icmp dst_mac 10:00:80:00:00:00 \	action gate index 12 sched-entry close 200000000ns
...
Po Liu &lt;[Po.Liu@nxp.com](mailto:Po.Liu@nxp.com)&gt;
