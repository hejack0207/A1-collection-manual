...
* **init**  
  Initializes a new PKI (by default in directory **/var/lib/openvswitch/pki**) and populates
...
* **req** _name_  
  Generates a new private key named _name_**-privkey.pem** and
  corresponding certificate request named _name_**-req.pem**.
  ...
  _name_**-req.pem** must be copied to the CA machine for signing
  ...
  **ovs-pki** looks for this file in **/var/lib/openvswitch/pki/dsaparam.pem**, but
  the **-D** or **-\^-dsaparam** option (see below) may be used to
  specify an alternate location.
  
  _name_**-privkey.pem** has sensitive contents that should not be
  exposed.  _name_**-req.pem** may be safely made public.
  
* **sign** _name_ [_type_]  
  Signs the certificate request named _name_**-req.pem** that was
  produced in the previous step, producing a certificate named
  _name_**-cert.pem**.  _type_, either **switch** (default) or
...
* **verify** _name_ [_type_]  
  Verifies that _name_**-cert.pem** is a valid certificate for the
  given _type_ of use, either **switch** (default) or
  **controller**.  If the certificate is valid for this use, it prints
  the message \`\`_name_**-cert.pem**: OK''; otherwise, it prints an
  error message.
  
* **fingerprint** _file_  
  Prints the fingerprint for _file_.  If _file_ is a
...
* **-k** _type_  
  ...
  The _type_ may be **rsa** (the default) or **dsa**.
...
* **-d** _dir_  
  ...
  the command (default: **/var/lib/openvswitch/pki**).  All commands, except **req**,
...
* **-l** _file_  
  .IQ "**-\^-log=file**"
  Sets the log file to _file_.  Default:
