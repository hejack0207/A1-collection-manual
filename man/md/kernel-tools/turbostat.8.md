...
**command**, which is forked and statistics are printed
...
**--enable column** show the specified built-in columns, which are otherwise disabled, by default.  Currently the only built-in counters disabled by default are "usec", "Time_Of_Day_Seconds", "APIC" and "X2APIC".
...
**--out output\_file** turbostat output is written to the specified output_file.
...
<a name="too-much-information-example"></a>
...
<a name="one-shot-command-example"></a>
...
<a name="periodic-interval-example"></a>
...
<a name="system-configuration-information-example"></a>
...
The **max efficiency** frequency, a.k.a. Low Frequency Mode, is the frequency
available at the minimum package voltage.  The **TSC frequency** is the base
...
as **acpi-cpufreq **periodically cleared the APERF and MPERF MSRs
in those kernels.
