# x86_energy_perf_policy(8) - Manage Energy vs. Performance Policy via x86 Model Specific Registers
...
<a name="scope-options"></a>

### SCOPE OPTIONS


**-c, --cpu** Operate on the MSR_IA32_HWP_REQUEST for each CPU in a CPU-list.
The CPU-list may be comma-separated CPU numbers, with dash for range
or the string "all".  Eg. '--cpu 1,4,6-8' or '--cpu all'.
When --cpu is used, **--hwp-use-pkg** is available, which specifies whether the per-cpu
...
**-B, --epb** set EPB per-core or per-package.
See value strings in the table above.

**-d, --debug** debug increases verbosity.  By default
x86_energy_perf_policy is silent for updates,
and verbose for read-only mode.

**-P, --hwp-epp** set HWP.EPP per-core or per-package.
See value strings in the table above.
...
**-D, --hwp-desired** request HWP 'desired' frequency.
...
**-w, --hwp-window** specify integer number of microsec
...
**-U, --hwp-use-pkg** (0 | 1), when used in conjunction with --cpu,
