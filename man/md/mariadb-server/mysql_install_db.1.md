# \fbmysql_install_db\fr(1)

MariaDB 10\&.3, 4 April 2019
...
```
.HP \w'mysql_install_db&nbsp;[options]&nbsp;'u mysql_install_db [options]
```
...
To invoke
**mysql\_install\_db**, use the following syntax:
...
**mysqld**, needs to access the data directory when it runs later, you should either run
**mysql\_install\_db**
...
.if n \{.RS 4
.\}
    shell> bin/mysql_install_db --user=mysql e
...
**--skip-grant-tables**
options (see
Section&nbsp;2.3.2, “Typical configure Options”). If MariaDB was configured with the
...
If using **--srcdir** with out-of-directory builds, you will need to set this 
