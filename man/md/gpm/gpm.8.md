...
* -d **delta**  
  ...
  is longer than **delta**, **accel** is used as a multiplying
...
* -g **number**  
  ...
  **number** must be 1, 2, or 3, and refers to the
...
* -i **interval**  
  Set **interval** to be used as an upper time limit
  for multiple clicks. If the interval between button-up and
  button-down events is less than **limit**, the press is considered
...
* -l **charset**  
  ...
  **charset** argument is a list of characters. - is used to
  specify a range and nbsp; is used to escape the next character
