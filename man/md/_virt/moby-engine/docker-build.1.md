...
**-f**, **--file**=_PATH/Dockerfile_
   Path to the Dockerfile to use. If the path is a relative path and you are
   building from a local directory, then the path must be relative to that
   directory. If you are building from a remote URL pointing to either a
   tarball or a Git repository, then the path must be relative to the root of
   the remote context. In all cases, the file must be within the build context.
   The default is _Dockerfile_.


**--squash**=_true_|_false_
   **Experimental Only**
   Once the image is built, squash the new layers into a new image with a single
   new layer. Squashing does not destroy any existing image, rather it creates a new
   image with the content of the squashed layers. This effectively makes it look
   like all **\fCDockerfile** commands were created with a single layer. The build
...
**--build-arg**=_variable_
   name and value of a **buildarg**.


For example, if you want to pass a value for **\fChttp\\_proxy**, use
   **\fC--build-arg=http\\_proxy="http://some.proxy.url"**


Users pass these values at build-time. Docker uses the **\fCbuildargs** as the
   environment context for command(s) run via the Dockerfile's **\fCRUN** instruction
...
**--force-rm**=_true_|_false_
   Always remove intermediate containers, even after unsuccessful builds. The default is _false_.
...
**--no-cache**=_true_|_false_
   Do not use cache when building the image. The default is _false_.
...
**--pull**=_true_|_false_
   Always attempt to pull a newer version of the image. The default is _false_.


**--compress**=_true_|_false_
    Compress the build context using gzip. The default is _false_.


**-q**, **--quiet**=_true_|_false_
   Suppress the build output and print image ID on success. The default is _false_.


**--rm**=_true_|_false_
   Remove intermediate containers after a successful build. The default is _true_.
...
(**--memory**) flag. The swap **\fCLIMIT** should always be larger than **-m**
(**--memory**) value.


The format of **\fCLIMIT** is **\fC&lt;number&gt;[&lt;unit&gt;]**. Unit can be **\fCb** (bytes),
**\fCk** (kilobytes), **\fCm** (megabytes), or **\fCg** (gigabytes). If you don't specify a
unit, **\fCb** is used. Set LIMIT to **\fC-1** to enable unlimited swap.


**--network**=_bridge_
  Set the networking mode for the RUN instructions during build. Supported standard
  values are: **\fCbridge**, **\fChost**, **\fCnone** and **\fCcontainer:&lt;name|id&gt;**. Any other value
  is taken as a custom network's name or ID which this container should connect to.


**--shm-size**=_SHM-SIZE_
  Size of **\fC/dev/shm**. The format is **\fC&lt;number&gt;&lt;unit&gt;**. **\fCnumber** must be greater than **\fC0**.
  Unit is optional and can be **\fCb** (bytes), **\fCk** (kilobytes), **\fCm** (megabytes), or **\fCg** (gigabytes). If you omit the unit, the system uses bytes.
  If you omit the size entirely, the system uses **\fC64m**.
...
**--cpuset-cpus**=_CPUSET-CPUS_
  CPUs in which to allow execution (0-3, 0,1).


**--cpuset-mems**=_CPUSET-MEMS_
  Memory nodes (MEMs) in which to allow execution (0-3, 0,1). Only effective on
  NUMA systems.


For example, if you have four memory nodes on your system (0-3), use **\fC--cpuset-mems=0,1**
to ensure the processes in your Docker container only use memory from the first
two memory nodes.


**--cgroup-parent**=_CGROUP-PARENT_
  Path to **\fCcgroups** under which the container's **\fCcgroup** are created.


If the path is not absolute, the path is considered relative to the **\fCcgroups** path of the init process.
...
For more information about **\fCulimit** see Setting ulimits in a
container
...
keep them, you must explicitly set **\fC--rm=false**.
...
sub-directory. These files will then be specified with the **\fCCOPY** or **\fCADD**
instructions in the **\fCDockerfile**.


Note: If you include a tar file (a good practice), then Docker will
automatically extract the contents of the tar file specified within the **\fCADD**
...
If you do not provide a version tag then Docker will assign **\fClatest**:
...
When you list the images, the image above will have the tag **\fClatest**.


You can apply multiple tags to an image. For example, you can apply the **\fClatest**
tag to a newly built image and add another tag that references a specific
version.
For example, to tag an image both as **\fCwhenry/fedora-jboss:latest** and
**\fCwhenry/fedora-jboss:v2.1**, use the following:
...
Note: You can set an arbitrary Git repository via the **\fCgit://** scheme.


<a name="building-an-image-using-a-url-to-a-tarballed-context"></a>
...
Windows. The **\fC--isolation=&lt;value&gt;** option sets a container's isolation
...
Specifying the **\fC--isolation** flag without a value is the same as setting **\fC--isolation="default"**.
