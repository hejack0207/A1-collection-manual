...
```
.IX Header "SYNOPSIS" .Vb 1  virt-tar-out -a disk.img /dir files.tar   virt-tar-out -d domain /dir files.tar   virt-tar-out -d domain /dir - | gzip --best > files.tar.gz .Ve
```
...
\f(CW`virt-tar-out\*(C' packs a virtual machine disk image directory into a
tarball.

The first parameter is the absolute path of the virtual machine
directory.  The second parameter is the tar file to write.  Use \f(CW`-\*(C'
...
**guestfish**\|(1) \f(CW`tar-out\*(C' command.  For anything more complex than a
