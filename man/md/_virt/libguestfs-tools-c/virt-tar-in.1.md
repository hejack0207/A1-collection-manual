...
```
.IX Header "SYNOPSIS" .Vb 1  virt-tar-in -a disk.img data.tar /destination   virt-tar-in -d domain data.tar /destination   zcat data.tar.gz | virt-tar-in -d domain - /destination .Ve
```
...
\f(CW`virt-tar-in\*(C' unpacks an uncompressed tarball into a virtual machine
disk image or named libvirt domain.

The first parameter is the tar file.  Use \f(CW`-\*(C' to read the tar file
...
**guestfish**\|(1) \f(CW`tar-in\*(C' command.  For anything more complex than a
