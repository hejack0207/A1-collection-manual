...
_/etc/chrony.conf_. Other locations can be specified on the
...
typically needed. See the **EXAMPLES** section for configuration in
...
<a name="time-sources"></a>

### Time sources


**server** _hostname_ [_option_]...
...
which is specified by the **keyfile** directive.
...
enabled to do so (by using the **online** command in
**chronyc**).
...
the need to run the **offline** command from **chronyc**
when disconnecting the network link. (It will still be necessary to use the
**online** command when the link has been established, to
...
**clientloglimit** is too small), or it might be overwritten
...
**maxdistance** directive, **chronyd** will try to replace the
...
**initstepslew** _step-threshold_ [_hostname_]...
...
configured with the **local** directive, the master can be set up with
...
**makestep** and **server** directives with the **iburst**
...
**refclock** _driver_ _parameter_[:_option_]... [_option_]...
...
**local** directive to allow synchronisation with some unknown but
...
format of the messages is described in the _refclock\_sock.c_ file in the chrony
source code.
...
**leapsectz** directive must be used with this option and the
...
**settime** command in **chronyc**. If no **manual**
...
the **manual** command in **chronyc**. (The idea of the two
...
configured with the **port** directive) to use only one socket for all
NTP packets.
...
or the **dump** command in **chronyc** is issued.
...
in the file _/run/chrony/1.2.3.4.dat_. History of reference clocks is saved
...
individual sources in the **server** and **refclock**
...
**tracking** and
**sourcestats** reports (and the _tracking.log_ and
_statistics.log_ files) may be smaller than the actual offsets.

**ntsdumpdir** _directory_
...
<a name="source-selection"></a>

### Source selection


**authselectmode** _mode_
...
<a name="system-clock"></a>

### System clock


**clockprecision** _precision_
...
**fallbackdrift** _min-interval_ _max-interval_
...
**maxslewrate** value the correction takes 12 seconds.
...
**slew** mode can be combined with the **smoothtime** directive to
...
**maxchange** _offset_ _start_ _ignore_
...
ratio (which can be set by the **corrtimeratio** directive) and
...
**tempcomp** _file_ _interval_ _T0_ _k0_ _k1_ _k2_, **tempcomp** _file_ _interval_ _points-file_
...
_tempcomp.log_ file if enabled by the **log tempcomp** directive.
...
Note, if the **initstepslew** directive is used in the
...
**broadcast** _interval_ _address_ [_port_]
...
using the **clients** command in **chronyc**. This option
...
**ntpsigndsocket** _directory_
This directive specifies the location of the Samba **ntp\_signd** socket when it
...
**allow** command in **chronyc**, can get an MS-SNTP
...
**maxntsconnections** _connections_
...
**ntsrotate** to 0. By default, no hostname or address is provided
to the clients, which means they should use the same server for NTS-KE and NTP.

**ntsrotate** _interval_
...
the **rekey** command is issued in **chronyc**. The file can
...
**ntsntpname** directive to point the clients to the right NTP
server.
...
**allow** directive or the **allow** command in
...
time depends on the memory limit set by the **clientloglimit**
directive.
...
**smoothtime** _max-freq_ _max-wander_ [**leaponly**]
...
corrected by stepping, e.g. by the **makestep** directive or the
**makestep** command in **chronyc**. The process can be
reset without stepping the clock by the smoothtime
reset command.
...
option is useful in a combination with the **leapsecmode slew**
...
can be also activated manually by the smoothtime
...
threshold. The **smoothing** command can be used to
monitor the process.
...
<a name="command-and-monitoring-access"></a>

### Command and monitoring access


**bindcmdaddress** _address_
...
path of the socket is _/run/chrony/chronyd.sock_. The socket can be
...
**bindcmdaddress** directive to not listen only on the
loopback interface to actually allow remote access.

**cmddeny** [**all**] [_subnet_]
...
**cmdratelimit** [_option_]...
This directive enables response rate limiting for command packets. It is
similar to the **ratelimit** directive, except responses to
...
if the RTC keeps local time or UTC. It overrides the **rtconutc**
directive.

The compiled-in default value is '_/etc/adjtime_'.
...
trim the RTC as if the **trimrtc** command in **chronyc**
...
facilities (the **rtcfile** directive and the **-s** command-line
...
Note that this setting is overridden by the **hwclockfile** file
and is not relevant for the **rtcsync** directive.
...
On macOS, **chronyd** will perform the RTC copy every 60 minutes
...
**mailonchange** _email_ _threshold_
...
reloaded by the **reload sources** command in
...
**hwtimestamp** _interface_ [_option_]...
...
be enabled by the **xleave** option in the **server** or the
**peer** directive.

This directive is supported on Linux 3.19 and newer. The NIC must support HW
timestamping, which can be verified with the **ethtool -T** command. The list of
capabilities should include _SOF\_TIMESTAMPING\_RAW\_HARDWARE_,
_SOF\_TIMESTAMPING\_TX\_HARDWARE_, and _SOF\_TIMESTAMPING\_RX\_HARDWARE_. Receive
filter _HWTSTAMP\_FILTER\_ALL_, or _HWTSTAMP\_FILTER\_NTP\_ALL_, is necessary for
...
indicated in the _measurements.log_ file if enabled by the log
measurements directive, and the **ntpdata** report in
**chronyc**.

This directive can be used multiple times to enable HW timestamping on multiple
interfaces. If the specified interface is <i>\*</i>, **chronyd** will try to enable HW
...
The **keygen** command of **chronyc** can be used to
...
file when the **rekey** command is issued by **chronyc**).

**lock\_all**
The **lock\_all** directive will lock chronyd into RAM so that it will never be
...
_/run/chrony/chronyd.pid_. The **pidfile** directive allows the name to be changed,
e.g.:
...
**sched\_priority** _priority_
On Linux, FreeBSD, NetBSD, and Solaris, the **sched\_priority** directive will
...
On systems other than macOS, this directive uses the **pthread\_setschedparam()**
...
use this unless you really need it. The **pthread\_setschedparam(3)** man page has
more details.

On macOS, this directive uses the **thread\_policy\_set()** kernel call to
...
The compiled-in default value is _chrony_.
...
<a name="ntp-client-with-permanent-connection-to-ntp-servers"></a>
...
**driftfile**, **makestep** and **rtcsync**
might be particularly useful. Also, the **iburst** option of the
**server** directive is useful to speed up the initial
...
might change over time), it is better to specify them with the **pool**
...
<a name="ntp-client-with-infrequent-connection-to-ntp-servers"></a>
...
start and finish sampling the servers, the **online** and
**offline** commands of **chronyc** need to be used.
...
_/usr/bin/chronyc_, the script _/etc/ppp/ip-up_ would include:
...
in the form of the **manual** directive and the
**settime** command in the **chronyc** program.
...
The **smoothtime** directive is useful when the clocks of the
clients need to stay close together when the local time is adjusted by the
**settime** command. The smoothing process needs to be
activated by the **smoothtime activate** command when
...
enable the **HPET\_EMULATE\_RTC** option in your kernel configuration. Otherwise,
...
saved to a file (if the **rtcfile** directive has been specified).
These pieces of information are also saved if the **dump**
and **writertc** commands respectively are issued
through **chronyc**.
...
**ratelimit** directive. The system timezone database, if it is
...
**-r** option with the **dumpdir** directive shortens the time in which
