...
**lvcreate --type raid0**
[**--stripes** _Number_ **--stripesize** _Size_]
_VG_
[_PVs_]
...
_PVs_ specifies the devices to use.  If not specified, lvm will choose
_Number_ devices, one for each stripe based on the number of PVs
...
_PVs_ specifies the devices to use.  If not specified, lvm will choose
_Number_ devices, one for each image.
...
_Number_ stripes requires _Number_+1 devices.  _Number_ must
be 2 or more.
...
_Number_ stripes requires _Number_+2 devices.  _Number_ must be
3 or more.
...
[**--mirrors** _NumberMirrors_]  
[**--stripes** _NumberStripes_ **--stripesize** _Size_]  
_VG_
[_PVs_]
...
**--maxrecoveryrate**
_Size_[k|UNIT]  
Sets the maximum recovery rate for a RAID LV.  _Size_ is specified as
an amount per second for each device in the array.  If no suffix is given,
then KiB/sec/device is used.  Setting the recovery rate to **0**
means it will be unbounded.

.HP
**--minrecoveryrate**
_Size_[k|UNIT]  
Sets the minimum recovery rate for a RAID LV.  _Size_ is specified as
an amount per second for each device in the array.  If no suffix is given,
then KiB/sec/device is used.  Setting the recovery rate to **0**
means it will be unbounded.
...
<a name="scrubbing-limitations"></a>
...
holds parity.  Each rmeta SubLV holds RAID metadata.
    ...
    # lvs -a -o name,segtype,devices
      lvr4            raid4  lvr4_rimage_0(0),&nbsp;                        lvr4_rimage_1(0),&nbsp;                        lvr4_rimage_2(0)
...
Each rmeta SubLV holds RAID metadata.
    ...
    # lvs -a -o name,segtype,devices
      lvr5            raid5  lvr5_rimage_0(0),&nbsp;                        lvr5_rimage_1(0),&nbsp;                        lvr5_rimage_2(0)
...
Each rmeta SubLV holds RAID metadata.
    ...
    # lvs -a -o name,segtype,devices
      lvr6            raid6  lvr6_rimage_0(0),&nbsp;                        lvr6_rimage_1(0),&nbsp;                        lvr6_rimage_2(0),&nbsp;                        lvr6_rimage_3(0),&nbsp;                        lvr6_rimage_4(0),&nbsp;                        lvr6_rimage_5(0)
...
Each rmeta SubLV holds RAID metadata.
    ...
    # lvs -a -o name,segtype,devices
      lvr10            raid10 lvr10_rimage_0(0),&nbsp;                         lvr10_rimage_1(0),&nbsp;                         lvr10_rimage_2(0),&nbsp;                         lvr10_rimage_3(0)
...
<a name="activating-an-lv-with-missing-devices"></a>
...
(see **Synchronization**).  Failure to clear a transient failure results
...
**dmeventd**(8)
monitoring process (see **Monitoring**).
...
PV name has the suffix **:n**, the write mostly attribute is cleared,
...
Warning: though it is possible to create **striped** LVs  with up to 128 stripes,
a maximum of 64 stripes can be converted to **raid0**, 63 to **raid4/5** and
62 to **raid6** because of the added parity SubLVs.
A **striped** LV with a maximum of 32 stripes can be converted to **raid10**.
...
Converting an LV from **striped** (with 4 stripes) to **raid6\_n\_6**.
...
for out-of-place reshaping.  The result is:
    ...
    # lvs -a -o lv_name,segtype,seg_pe_ranges,dataoffset
      ...
      lv            raid6_nr lv_rimage_0:0-32 &nbsp;                        lv_rimage_1:0-32 &nbsp;                        lv_rimage_2:0-32 &nbsp;                        lv_rimage_3:0-32
...
(i.e. image SubLVs) from a RaidLV will shrink its size.
    ...
    # lvconvert --stripes 4 vg/lv
      ...
      WARNING: to remove freed stripes after the conversion has finished,&nbsp; you have to run "lvconvert --stripes 4 vg/lv"
      Logical volume vg/lv successfully converted.
...
We start with the linear LV.
    ...
    # lvs -a -o name,size,segtype,syncpercent,datastripes,&nbsp;           stripesize,reshapelenle,devices vg
...
Then convert it to a 2-way raid1.
    ...
    # lvs -a -o name,size,segtype,datastripes,&nbsp;           stripesize,reshapelenle,devices vg
      LV            LSize   Type   #DStr Stripe RSize Devices
      lv            128.00m raid1      2     0        lv_rimage_0(0),&nbsp;                                                 lv_rimage_1(0)
...
Once the conversion has finished we can can convert to striped.
    ...
    # lvs -a -o name,size,segtype,datastripes,&nbsp;           stripesize,reshapelenle,devices vg
      LV   LSize   Type    #DStr Stripe  RSize Devices
      lv   640.00m striped     5 128.00k       /dev/sda(33),&nbsp;                                          /dev/sdhx(33),&nbsp;                                          /dev/sdhw(33),&nbsp;                                          /dev/sdhv(33),&nbsp;                                          /dev/sdhu(33)
      lv   640.00m striped     5 128.00k       /dev/sda(0),&nbsp;                                          /dev/sdhx(1),&nbsp;                                          /dev/sdhw(1),&nbsp;                                          /dev/sdhv(1),&nbsp;                                          /dev/sdhu(1)
...
actually converting to 1 stripe.  The **--force** option is needed to
...
added to LVM version 2.02.87.  The capabilities of the LVM **raid1**
type have surpassed the old **mirror** type.  raid1 is now recommended
