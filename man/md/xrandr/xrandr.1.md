...
<a name="randr-version-15-options"></a>
...
* --setmonitor _name_ _geometry_ _outputs_  
...
* --setprovideroutputsource _provider_ _source_  
  Set _source_ as the source of display output images for _provider_.
  This is only possible if _source_ and _provider_ have the Source
  Output and **Sink Output** capabilities, respectively.
  If _source_ is **0x0**, then _provider_ is disconnected from its
  current output source.
* --setprovideroffloadsink _provider_ _sink_  
  Set _provider_ as a render offload device for _sink_.
  This is only possible if _provider_ and _sink_ have the Source
  Offload and **Sink Offload** capabilities, respectively.
  If _sink_ is **0x0**, then _provider_ is disconnected from its current
  render offload sink.


<a name="randr-version-13-options"></a>
...
* --panning _width_x_height_[+_x_+_y_[/_track\_width_x_track\_height_+_track\_x_+_track\_y_[/_border\_left_/_border\_top_/_border\_right_/_border\_bottom_]]]  
  ...
  size with _--fb_ simultaneously.
* --transform _a_,_b_,_c_,_d_,_e_,_f_,_g_,_h_,_i_  
  ...
  The mathematical form corresponds to:
      ...
      of the transformed pixel are:
          ...
          with  w' = (gx + hy + i)  .
      Typically, _a_ and
      _e_ corresponds to the scaling on the X and Y axes, _c_ and _f_
      corresponds to the translation on those axes, and _g_, _h_, and _i_
      ...
      passing a matrix, one can pass the string _none_, in which case the default
...
* --scale _x_[x_y_]  
  ...
  If the _y_ value is omitted, the _x_ value will be used for both dimensions.
  ...
  This option is actually a shortcut version of the _--transform_ option.
* --scale-from _w_x_h_  
...
<a name="randr-version-12-options"></a>
...
* --dpi _from-output_  
  ...
  configurations.
* --newmode _name_ _mode_  
  New modelines can be added to the server and then associated with outputs.
  This option does the former. The _mode_ is specified using the ModeLine
  syntax for xorg.conf: clock hdisp hsyncstart hsyncend htotal vdisp vsyncstart
  vsyncend vtotal _flags_. _flags_ can be zero or more of +HSync,
...
* --addmode _output_ _name_  
  Add a mode to the set of valid modes for an output.
* --delmode _output_ _name_  
...
* --mode _mode_  
  This selects a mode. Use either the name or the XID for _mode_
* --preferred  
...
* --rotate _rotation_  
  ...
  rotation.
* --left-of, --right-of, --above, --below, --same-as _another-output_  
...
<a name="randr-version-11-options"></a>
