...
```
Interactive Usage: 
 atop [-g|-m|-d|-n|-u|-p|-s|-c|-v|-o|-y|-Y] [-C|-M|-D|-N|-A] [-afFG1xR] [-L linelen] [-Plabel[,label]...] [ interval [ samples ]] 
 Writing and reading raw logfiles: 
 atop -w rawfile [-a] [-S] [ interval [ samples ]]
atop -r [ rawfile ] [-b  [YYYYMMDD]hhmm ] [-e [YYYYMMDD]hhmm ] [-g|-m|-d|-n|-u|-p|-s|-c|-v|-o|-y|-Y] [-C|-M|-D|-N|-A] [-fFG1xR] [-L linelen] [-Plabel[,label]...]
```
...
<a name="gpu-statistics-gathering"></a>
...
on the target system. The Python code of the daemon is compatible with
...
**atop**
especially when lots of new counters have to be maintained.
...
<a name="output-description-system-level"></a>
...
* **SWP**  
  ...
  the amount of free swap space (\`free') and the size of the swap cache
  (\`swcac').  
...
<a name="output-description-process-level"></a>
...
(disk, only if "storage accounting" is active) and "PRN" (network, only if
...
* **MEM**  
  ...
  total size of huge pages (huge pages),
  size of free huge pages (huge pages), and
...
* **NET**  
  ...
  number of input errors (TCP), and
...
* **PRG**  
  ...
  Docker container id (CID), and indication if the task is newly started
  during this interval ('N').
* **PRC**  
  For every process one line is shown.  
  Subsequent fields:
  ...
  runqueue delay in nanoseconds for this thread or for all threads (in case of 
  process), and wait channel of this thread (between brackets).
* **PRE**  
  For every process one line is shown.  
  Subsequent fields:
