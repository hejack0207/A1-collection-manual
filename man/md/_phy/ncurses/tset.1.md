...
Having retrieved these settings, **tset** remembers which
...
error output device in the _/etc/ttys_ file.
(On System-V-like UNIXes and systems using that convention,
_getty_ does this job by setting
**TERM** according to the type passed to it by _/etc/inittab_.)
...
.bP
if the \`**-w**\*('' option is enabled, **tset** may update
...
* If the window size cannot be obtained from the operating system,
  ...
  .bP
  if the \`**-c**\*('' option is enabled,
  the backspace, interrupt and line kill characters
  (among many other things) are set
  .bP
  unless the \`**-I**\*('' option is enabled,
  the terminal
  and tab _initialization_ strings are sent to the standard error output,
...
<a name="reset-reinitialization"></a>
...
Also, rather than using the terminal _initialization_ strings,
it uses the terminal _reset_ strings.
...
* **-Q**  
  ...
  Normally **tset** displays the values for control characters which
...
* **-w**  
  Resize the window to match the size deduced via **setupterm**(3X).
...
_/etc/ttys_ file or the **TERM** environmental variable is often
...
and that **csh** users insert a backslash character (\`\e\*('') before
...
feature when **tset** was invoked as **reset**.
Rather than simply copying the existing program,
in this merged version, **tset** used the termcap database
...
Also, the interaction between the - option and the _terminal_
...
* TERMCAP  
  ...
  **tset** removes the variable from the environment before looking
