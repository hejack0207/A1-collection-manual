...
_file_ for **termcap** descriptions.
For each
one found, an equivalent **terminfo** description is written to standard
output.
Termcap **tc** capabilities are translated directly to terminfo
**use** capabilities.

If no _file_ is given, then the environment variable **TERMCAP** is used
for the filename or entry.
If **TERMCAP** is a full pathname to a file, only
the terminal whose name is specified in the environment variable **TERM** is
extracted from that file.
If the environment variable **TERMCAP** is not
...
<a name="translations-from-nonstandard-capabilities"></a>
...
be composed into an **acsc** string.
...
Additionally, the AIX _box1_ capability will be automatically translated to
an **acsc** string.
...
This utility is actually a link to **tic**(1M), running in _-I_ mode.
