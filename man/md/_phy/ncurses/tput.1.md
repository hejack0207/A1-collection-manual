...
and the _capname_ associated with each, see **terminfo**(5).
...
special; they are defined by the **tput** program.
The others are the names of _capabilities_ from the terminal database
(see **terminfo**(5) for a list).
Although **init** and **reset** resemble capability names,
...
* **init**  
  ...
  terminal exists (see **-T**_type_, above), the following will
  occur:
    * (1)  
...
* **reset**  
  This is similar to **init**, with two differences:
    ...
    * (2)  
      Instead of putting out _initialization_ strings, the terminal's
      _reset_ strings will be output if present
      (**rs1**, **rs2**, **rs3**, **rf**).
      If the _reset_ strings are not present, but _initialization_
      strings are, the _initialization_ strings will be output.
* Otherwise, **reset** acts identically to **init**.
* **longname**  
  If the terminal database is present and an entry for the
  user's terminal exists (see **-T**_type_ above), then the long name
...
i.e., only **reset\_1string**, **reset\_2string** and **reset\_file**
...
With the changes made for ncurses 6.1, the _reset_ feature of the
...
does not support _NAWS_: negotiations about window size).
.bP
finally, it inspects the environment variables **LINES** and **COLUMNS**
which may override the terminal size.

If the **-T** option is given
tput ignores the environment variables by calling **use\_tioctl(TRUE)**,
...
exit code **1** will never appear.
Exit codes **2**, **3**, and
**4** retain their usual interpretation.
If the **-S** option is not used,
the exit code depends on the type of _capname_:

* _boolean_
  a value of **0** is set for TRUE and **1** for FALSE.
* _string_
  a value of **0** is set if the
  _capname_ is defined for this terminal _type_ (the value of
  _capname_ is returned on standard output);
  a value of **1** is set if _capname_
  is not defined for this terminal _type_
  (nothing is written to standard output).
* _integer_
  a value of **0** is always set,
  whether or not _capname_ is defined for this terminal _type_.
  To determine if _capname_ is defined for this terminal _type_,
  the user must test the value written to standard output.
  A value of **-1**
  means that _capname_ is not defined for this terminal _type_.
* _other_
...
=
**0**	T{
(_capname_ is a numeric variable that is not specified in the
...
accepted some parameters named for _terminfo capabilities_
(**clear**, **init**, **longname** and **reset**).
However (because he had only termcap available),
it accepted _termcap names_ for other capabilities.
...
AT&T or BSD: Ross Ridge's _mytinfo_ package, published on
_comp.sources.unix_ in December 1992.
Ridge's program made more sophisticated use of the terminal capabilities
than the BSD program.
Eric Raymond used that **tput** program
(and other parts of _mytinfo_) in ncurses in June 1995.
...
two important areas:
.bP
**tput** _capname_ writes to the standard output.
...
* Until changes made after ncurses 6.0,
  ...
  **tput** will return an error.
  .bP
  AT&T **tput** guesses the type of its _capname_ operands by seeing if
  all of the characters are numeric, or not.
* Most implementations which provide support for _capname_ operands
  use the _tparm_ function to expand parameters in it.
...
* This implementation uses a table to determine the parameter types for
  the standard _capname_ operands, and an internal library
  function to analyze nonstandard _capname_ operands.

This implementation (unlike others) can accept both _termcap_
and _terminfo_ names for the _capname_ feature,
if
_termcap_ support is compiled in.
However, the predefined _termcap_ and _terminfo_ names have two
ambiguities in this case (and the _terminfo_ name is assumed):
.bP
The _termcap_ name **dl** corresponds to
the _terminfo_ name **dl1** (delete one line).  
The _terminfo_ name **dl** corresponds to
the _termcap_ name **DL** (delete a given number of lines).
.bP
The _termcap_ name **ed** corresponds to
the _terminfo_ name **rmdc** (end delete mode).  
The _terminfo_ name **ed** corresponds to
the _termcap_ name **cd** (clear to end of screen).
...
.bP
In this implementation, **clear** is part of the _capname_ support.
...
provide support for _capname_ operands.
.bP
...
Because (apparently) _all_ of the certified Unix systems
support the full set of capability names, the reasoning for documenting
only a few may not be apparent.
.bP
X/Open Curses Issue 7 documents **tput** differently, with _capname_
...
a **tput** utility which does not provide the _capname_ feature.
...
which may be the same as _unknown capability_.
