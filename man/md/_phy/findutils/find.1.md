...
known (the left hand side is false for _and_ operations, true for
_or_), at which point
**find**
...
* -maxdepth _levels_  
  Descend at most _levels_ (a non-negative integer) levels of
  directories below the starting-points.
  **-maxdepth 0**
   means only apply the tests and actions to the starting-points themselves.
  
* -mindepth _levels_  
  Do not apply any tests or actions at levels less than _levels_ (a
...
* -amin _n_  
  File was last accessed _n_ minutes ago.
  
* -anewer _file_  
  File was last accessed more recently than _file_ was modified.  If
  _file_ is a symbolic link and the
  **-H**
  option or the
  **-L**
...
* -atime _n_  
  File was last accessed _n_*24 hours ago.
...
* -cmin _n_  
  File's status was last changed _n_ minutes ago.
  
* -cnewer _file_  
  File's status was last changed more recently than _file_ was
  modified.  If _file_ is a symbolic link and the
  **-H**
  option or the
  **-L**
...
* -ctime _n_  
  File's status was last changed _n_*24 hours ago.
...
* -fstype _type_  
  File is on a filesystem of type _type_.  The valid filesystem
...
* -gid _n_  
  File's numeric group ID is _n_.
...
* -inum _n_  
  File has inode number _n_.  It is normally easier to use the
  **-samefile**
  test instead.
...
* -lname _pattern_  
  ...
  _pattern_.  The metacharacters do not treat \`/' or \`.' specially.
  If the
  **-L**
  option or the
  **-follow**
...
* -mmin _n_  
  File's data was last modified _n_ minutes ago.
  
* -mtime _n_  
  File's data was last modified _n_*24 hours ago.
  See the comments for
  **-atime**
...
* -name _pattern_  
  ...
  matches shell pattern _pattern_.  Because the leading directories
...
* -newer _file_  
  File was modified more recently than _file_.  If _file_ is a
...
* -newerXY _reference_  
  Succeeds if timestamp _X_ of the file being considered is newer
  than timestamp _Y_ of the file _reference_.   The letters
  _X_ and _Y_ can be any of the following letters:
...
* -path _pattern_  
  File name matches shell pattern _pattern_.  The metacharacters do
...
* -perm _mode_  
  File's permission bits are exactly _mode_ (octal or symbolic).
...
* -perm -_mode_  
  All of the permission bits _mode_ are set for the file.
...
* -perm /_mode_  
  Any of the permission bits _mode_ are set for the file.  Symbolic
...
* -regex _pattern_  
  File name matches regular expression _pattern_.  This is a match
...
* -samefile _name_  
  File refers to the same inode as _name_.   When
  **-L**
  is in effect, this can include symbolic links.
  
* -size _n_[cwbkMG]  
  File uses _n_ units of space, rounding up.  The following suffixes
  can be used:
    * \`b'  
...
* -type _c_  
  File is of type _c_:
    ...
    * D  
      door (Solaris)
* -uid _n_  
  File's numeric user ID is _n_.
  
* -used _n_  
  File was last accessed _n_ days after its status was last changed.
...
* -xtype _c_  
  ...
  **-P**
  ...
  link to a file of type _c_; if the
  **-L**
  option has been given, true
  if _c_ is \`l'.  In other words, for symbolic links,
  **-xtype**
...
* -exec _command_ ;  
  Execute _command_; true if 0 status is returned.  All following
  arguments to
  **find**
...
* -fprint _file_  
  True; print the full file name into file _file_.  If _file_
...
* -printf _format_  
  True; print _format_ on the standard output, interpreting \`\e'
...
* %A_k_  
  File's last access time in the format specified by _k_, which is
  either \`@' or a directive for the C \`strftime' function.  The possible
  values for _k_ are listed below; some of them might not be
...
* %c  
  ...
  function.
* %C_k_  
  File's last status change time in the format specified by _k_,
...
* %t  
  ...
  function.
* %T_k_  
  File's last modification time in the format specified by _k_,
  which is the same as for %A.
* %u  
...
* ! _expr_  
  True if _expr_ is false.  This character will also usually need
...
* _expr1 expr2_  
  ...
  implied "and"; _expr2_ is not evaluated if _expr1_ is false.
  
* _expr1_ -a _expr2_  
  Same as _expr1 expr2_.
  
* _expr1_ -and _expr2_  
  Same as _expr1 expr2_, but not POSIX compliant.
...
* _expr1_ -or _expr2_  
  Same as _expr1_
  **-o**
  _expr2_, but not POSIX compliant.
  
* _expr1_ , _expr2_  
  List; both _expr1_ and _expr2_ are always evaluated.  The
  value of _expr1_ is discarded; the value of the list is the value
  of _expr2_.  The comma operator can be useful for searching for
...
could have been used in that case also.
    ...
    find / e( -perm -4000 -fprintf /root/suid.txt '%#m %u %pen' e) , e
     e( -size +100M -fprintf /root/big.txt  '%-10s %pen' e)
...
24 hours ago.
    ...
    find /sbin /usr/sbin -executable e! -readable -print
...
respectively).
    ...
    cd /source-dir
    find . -name .snapshot -prune -o e( e! -name "*~" -print0 e)|
    cpio -pmd0   /dest-dir
...
what is going on.
    ...
    find repo/ -exec test -d {}/.svn e; -or e
    -exec test -d {}/.git e; -or -exec test  -d {}/CVS e; e
    -print -prune
...
_bug-findutils-request@gnu.org_.
