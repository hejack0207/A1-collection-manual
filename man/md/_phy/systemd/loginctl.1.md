...
```
.HP \w'loginctl&nbsp;'u loginctl [OPTIONS...] {COMMAND} [NAME...]
```
...
**user-status**
and
**session-status**, controls the number of journal lines to show, counting from the most recent ones. Takes a positive integer argument. Defaults to 10.

**-o**, **--output=**
When used with
**user-status**
and
**session-status**, controls the formatting of the journal entries that are shown. For the available choices, see
...
**lock-session** [_ID_...], **unlock-session** [_ID_...]
...
**enable-linger** [_USER_...], **disable-linger** [_USER_...]
...
**seat-status** [_NAME_...]
Show terse runtime status information about one or more seats. Takes one or more seat names as parameters. If no seat names are passed the status of the callers session\*(Aqs seat is shown. This function is intended to generate human-readable output. If you are looking for computer-parsable output, use
**show-seat**
instead.
