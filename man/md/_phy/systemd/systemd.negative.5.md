...
\m[blue]**RFC 4035, Section 4.4**\m[]\s-2\u[1]\d\s+2
...
\m[blue]**RFC 1035, Section 5**\m[]\s-2\u[2]\d\s+2. One DS or DNSKEY resource record may be listed per line. Empty lines and lines starting with a semicolon (";") are ignored and considered comments. A DS resource record is specified like in the following example:
...
\m[blue]**RFC 4034, Section 5**\m[]\s-2\u[3]\d\s+2
...
\m[blue]**RFC 4034, Section 2**\m[]\s-2\u[4]\d\s+2
for details about the precise syntax and meaning of these fields.
...
\m[blue]**RFC 5011**\m[]\s-2\u[5]\d\s+2
...
The current DNSSEC trust anchor for the Internets root domain is available at the
\m[blue]**IANA Trust Anchor and Keys**\m[]\s-2\u[6]\d\s+2
page.
...
\m[blue]**RFC 7646**\m[]\s-2\u[7]\d\s+2
