...
**ClientIdentifier=duid**. IAID and DUID allows a DHCP server to uniquely identify the machine and the interface requesting a DHCP IP. To configure IAID and ClientIdentifier, see
**systemd.network**(5).
...
\m[blue]**RFC 3315**\m[]\s-2\u[1]\d\s+2
...
**machine-id**(5). This is the default if
_DUIDType=_
is not specified.
...
\m[blue]**RFC 3315**\m[]\s-2\u[2]\d\s+2,
\m[blue]**RFC 6355**\m[]\s-2\u[3]\d\s+2. To configure IAID, see
**systemd.network**(5).
