...
```
.HP \w'timedatectl&nbsp;[OPTIONS...]&nbsp;{COMMAND}&nbsp;'u timedatectl [OPTIONS...] {COMMAND}
```
...
**systemd-timesyncd.service**(8), show all properties regardless of whether they are set or not.

**-p**, **--property=**
When showing properties of
**systemd-timesyncd.service**(8), limit display to certain properties as specified as argument. If not specified, all set properties are shown. The argument should be a property name, such as
...
**status**, but in machine readable form. This command is intended to be used whenever computer-parsable output is required. Use
**status**
...
**list-timezones**. If the RTC is configured to be in the local time, this will also update the RTC time. This call will alter the
/etc/localtime
symlink. See
**localtime**(5)
...
**show-timesync**
Show the same information as
**timesync-status**, but in machine readable form. This command is intended to be used whenever computer-parsable output is required. Use
**timesync-status**
if you are looking for formatted human-readable output.
