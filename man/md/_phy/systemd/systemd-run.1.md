...
```
.HP \w'systemd-run&nbsp;'u systemd-run [OPTIONS...] COMMAND&nbsp;[ARGS...]  .HP \w'systemd-run&nbsp;'u systemd-run [OPTIONS...] [PATH&nbsp;OPTIONS...] {COMMAND} [ARGS...] .HP \w'systemd-run&nbsp;'u systemd-run [OPTIONS...] [SOCKET&nbsp;OPTIONS...] {COMMAND} [ARGS...] .HP \w'systemd-run&nbsp;'u systemd-run [OPTIONS...] [TIMER&nbsp;OPTIONS...] {COMMAND} [ARGS...]
```
...
**systemctl list-units**. Execution in this case is synchronous, and will return only when the command finishes. This mode is enabled via the
**--scope**
switch (see below).
...
**systemd-run**, this means the service might not be able to re-open the passed file descriptors, due to normal file descriptor access restrictions. If the invoked process is a shell script that uses the
...
_OnCalendar=_
in
**systemd.timer**(5). This option is a shortcut for
...
**--quiet**. This option may not be combined with
**--no-block**,
**--scope**
