...
* **--date** timestamp  
  ...
  .TE
* **-d**,** --device **device  
  Use the specified _device_ instead of **rtc0** as realtime clock.
...
* **-t**,** --time **time_t  
  Set the wakeup time to the absolute time _time\_t_.  _time\_t_
  is the time in seconds since 1970-01-01, 00:00 UTC.  Use the
  **date**(1)
  tool to convert between human-readable time and _time\_t_.
* **-u**,** --utc**  
...
The program was written by David Brownell &lt;[dbrownell@users.sourceforge](mailto:dbrownell@users.sourceforge).net&gt;
and improved by Bernhard Walle &lt;[bwalle@suse.de](mailto:bwalle@suse.de)&gt;.
