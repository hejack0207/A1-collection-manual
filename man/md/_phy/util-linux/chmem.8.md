...
* Specify _BLOCKRANGE_ in the form &lt;first&gt;-&lt;last&gt; or &lt;block&gt; as shown in the
  output of the **lsmem** command. &lt;first&gt; is the number of the first memory block
...
* Specify _ZONE_ as the name of a memory zone, as shown in the output of the
  **lsmem -o +ZONES** command. The output shows one or more valid memory zones
  ...
  changed by specifying the **--zone** option with another valid zone.
  ...
  Use the **--help** option to see all available zones.

_SIZE_ and _RANGE_ must be aligned to the Linux memory block size, as
...
* **-b**, **--blocks**  
  Use a _BLOCKRANGE_ parameter instead of _RANGE_ or _SIZE_ for the
...
* **-z**, **--zone**  
  Select the memory _ZONE_ where to set the specified _RANGE_, _SIZE_,
  or _BLOCKRANGE_ of memory online or offline. By default, memory will be set
...
The **chmem** command is part of the util-linux package and is available from
