...
Sets a [variable](../Variables.htm) to itself times the given value. Synonymous with: `Var *= Value`.

**Deprecated:** This command is not recommended for use in new scripts. Use `Var := Var * Value` or `Var *= Value` instead.

```
<span class="func">EnvMult</span>, Var, Value
```
...
[EnvAdd](EnvAdd.htm), [EnvSub](EnvSub.htm), [EnvDiv](EnvDiv.htm), [SetFormat](SetFormat.htm), [Expressions](../Variables.htm#Expressions), [If var is [not] type](IfIs.htm), [SetEnv](SetEnv.htm), [bitwise operations (Transform)](Transform.htm)
