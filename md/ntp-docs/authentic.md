...
- [Microsoft Windows Authentication](#windows)
- [Public Key Cryptography](#pub)

* * *

#### Introduction

This page describes the various cryptographic authentication
...
paper [NTP\
Security Analysis](http://www.eecis.udel.edu/~mills/security.html).

The NTPv3 specification (RFC-1305) defined an authentication scheme
properly described as _symmetric key cryptography_. It used
...
If the OpenSSL cryptographic library is installed prior to building
...
from [http://www.openssl.org](http://www.openssl.org) and
can be installed using the procedures outlined in
the [Building and Installing the Distribution](build.html)
page. Once installed, the configure and build process automatically
detects the library and links the library routines required.

In addition to the symmetric key algorithms, this distribution
...
used when the distribution is built.
...
the [ntp-keygen](keygen.html) utility program in
the NTP software distribution.
...
the key or autokey option of the server
configuration command, as described in
the [Server Options](confopt.html) page.
The [ntp-keygen](keygen.html) page describes the files
...
from [www.ntp.org](http://www.ntp.org).
...
overridden by a disable auth command. In the current climate
of targeted broadcast or "letterbomb" attacks, defeating
this requirement would be decidedly dangerous. In any case,
the notrust flag, described on
the [Access Control Options](authopt.html) page, can be
used to disable access to all but correctly authenticated clients.
...
server returns a special message called a _crypto-NAK_. Since
the crypto-NAK is protected by the loopback test, an intruder cannot
disrupt the protocol by sending a bogus crypto-NAK.

Keys and related information are specified in a keys file, which must
...
the [ntp-keygen](keygen.html) program, but it can
...
OpenSSL library must be MD5, which designates the MD5 message
...
optional /subnetbits suffix, which limits the acceptance of
...
password, such as 2late4Me for key ID 10. Note that two or
more keys files can be combined in any order as long as the key IDs
are distinct.

When ntpd is started, it reads the keys file specified by
the keys command and installs the keys in the key cache.
...
keys and then activating a key remotely using ntpq
or ntpdc. The requestkey command selects the key ID
used as the password for the ntpdc utility, while
the controlkey command selects the key ID used as the
password for the ntpq utility.

#### Microsoft Windows Authentication

In addition to the above means, ntpd now supports Microsoft
...
It is enabled using the mssntp flag of the restrict
command described on the [Access Control\
Options](accopt.html#restrict) page. Note: Potential users should
...
no clients other than MS-SNTP.

#### Public Key Cryptography

See the [Autokey Public-Key Authentication](autokey.html)
page.
