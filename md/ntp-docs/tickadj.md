...
-a _tickadj_Set the kernel variable tickadj to the value _tickadj specified._-ASet the kernel variable tickadj to an internally computed "optimal" value.-t _tick_Set the kernel variable tick to the value _tick_ specified.-sSet the kernel variable dosynctodr to zero, which disables the hardware time-of-year clock, a prerequisite for running the ntpd daemon under SunOS 4.x.-qNormally, tickadj is quite verbose about what it is doing. The -q flag tells it to shut up about everything except errors.
