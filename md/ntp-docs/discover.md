...
![gif](pic/alice51.gif) [from _Alice's Adventures in Wonderland_, Lewis Carroll](http://www.eecis.udel.edu/%7emills/pictures.html)
...
- [Association Management](#assoc)
- [Broadcast/Multicast Scheme](#bcst)
- [Manycast Scheme](#mcst)
- [Server Pool Scheme](#pool)
...
Following is a summary of each scheme. Note that reference to option applies to the commands described on the [Configuration Options](confopt.html) page. See that page for applicability and defaults.
...
Project](http://www.pool.ntp.org/en/use.html) offers instructions on using the pool with the server command, which is suboptimal but works with older versions of ntpd predating the pool command. With recent ntpd, consider replacing the
