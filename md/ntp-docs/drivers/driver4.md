...
Driver ID: WWVB\_SPEC

Serial Port: /dev/wwvb _u_; 9600 baud, 8-bits, no parity

Features: Optional PPS signal processing, tty\_clk
...
The DIPswitches on these clocks should be set to 24-hour display, AUTO DST off, data format 0 or 2 (see below) and baud rate 9600. If this clock is used as the source for the IRIG Audio Decoder (refclock\_irig.c in this distribution), set the DIPswitches for AM IRIG output and IRIG format 1 (IRIG B with signature control).
...
time1 _time_Specifies the PPS time offset calibration factor, in seconds and fraction, with default 0.0.time2 _time_Specifies the serial time offset calibration factor, in seconds and fraction, with default 0.0.stratum _number_Specifies the driver stratum, in decimal from 0 to 15, with default 0.refid _string_Specifies the driver reference identifier, an ASCII string from one to four characters, with default WWVB.flag1 0 \| 1Disable PPS signal processing if 0 (default); enable PPS signal processing if 1.flag2 0 \| 1If PPS signal processing is enabled, capture the pulse on the rising edge if 0 (default); capture on the falling edge if 1.flag3 0 \| 1If PPS signal processing is enabled, use the ntpd clock discipline if 0 (default); use the kernel discipline if 1.flag4 0 \| 1Enable verbose clockstats recording if set.
