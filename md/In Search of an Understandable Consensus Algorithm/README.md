...
proach was unusual in that our primary goal was _under-

standability_

: could we define a consensus algorithm for
...
approach where the majorities of
...
_replicated state machines_

[37]. In this approach, state ma-
...
They ensure _safety_(never returning an incorrect re-
...
log entry. We refer to this subset as _single-decree Paxos_.

Paxos then combines multiple instances of this protocol to

facilitate a series of decisions such as a log ( _multi-Paxos_).
...
important goal—and most difficult challenge—was _un-

derstandability_

. It must be possible for a large audience to
...
guished _leader_, then giving the leader complete responsi-
...
![background image](https://github.com/hejack2007/A1-collection-manual/blob/master/md/In Search of an Understandable Consensus Algorithm/In Search of an Understandable Consensus Algorithm004.png)
...
![background image](https://github.com/hejack2007/A1-collection-manual/blob/master/md/In Search of an Understandable Consensus Algorithm/In Search of an Understandable Consensus Algorithm005.png)
...
, _follower_, or _candidate_. In normal operation there
...
Raft divides time into _terms_ of arbitrary length, as

shown in Figure 5. Terms are numbered with consecutive

integers. Each term begins with an _election_, in which one
...
server stores a _current term_ number, which increases
...
![background image](https://github.com/hejack2007/A1-collection-manual/blob/master/md/In Search of an Understandable Consensus Algorithm/In Search of an Understandable Consensus Algorithm006.png)
...
called the _election timeout_, then it assumes there is no vi-
...
machine. An entry is considered _committed_ if it is safe for that
...
![background image](https://github.com/hejack2007/A1-collection-manual/blob/master/md/In Search of an Understandable Consensus Algorithm/In Search of an Understandable Consensus Algorithm007.png)

tifying its position in the log.

The leader decides when it is safe to apply a log en-

try to the state machines; such an entry is called _commit-
...
RPCs. The leader maintains a _nextIndex_ for each follower,
...
![background image](https://github.com/hejack2007/A1-collection-manual/blob/master/md/In Search of an Understandable Consensus Algorithm/In Search of an Understandable Consensus Algorithm008.png)
...
![background image](https://github.com/hejack2007/A1-collection-manual/blob/master/md/In Search of an Understandable Consensus Algorithm/In Search of an Understandable Consensus Algorithm009.png)
...
from leaderT _before_ voting for leaderU; otherwise it
...
![background image](https://github.com/hejack2007/A1-collection-manual/blob/master/md/In Search of an Understandable Consensus Algorithm/In Search of an Understandable Consensus Algorithm010.png)
...
ing _timing requirement_:

_broadcastTime_ ≪ _electionTimeout_ ≪ _MTBF_

In this inequality _broadcastTime_ is the average time it

takes a server to send RPCs in parallel to every server

in the cluster and receive their responses; _electionTime-

out_

is the election timeout described in Section 5.2; and

_MTBF_

is the average time between failures for a single
...
with a majority of the old configuration ( _C_ old) and another

with a majority of the new configuration ( _C_ new).
...
Up until now we have assumed that the cluster _config-_

_uration_

(the set of servers participating in the consensus
...
first switches to a transitional configuration we call _joint

consensus_

; once the joint consensus has been committed,
...
![background image](https://github.com/hejack2007/A1-collection-manual/blob/master/md/In Search of an Understandable Consensus Algorithm/In Search of an Understandable Consensus Algorithm011.png)
...
ration entry. The leader first creates the _C_ old,new configuration

entry in its log and commits it to _C_ old,new (a majority of _C_ old

and a majority of _C_ new). Then it creates the _C_ new entry and

commits it to a majority of _C_ new. There is no point in time in

which _C_ old and _C_ new can both make decisions independently.
...
quires separate majorities from _both_ the old and new
...
receives a request to change the configuration from _C_ old

to _C_ new, it stores the configuration for joint consensus

( _C_ old,new in the figure) as a log entry and replicates that
...
leader will use the rules of _C_ old,new to determine when the

log entry for _C_ old,new is committed. If the leader crashes,

a new leader may be chosen under either _C_ old or _C_ old,new,

depending on whether the winning candidate has received

_C_ old,new. In any case, _C_ new cannot make unilateral deci-

sions during this period.

Once _C_ old,new has been committed, neither _C_ old nor _C_ new

can make decisions without approval of the other, and the

Leader Completeness Property ensures that only servers

with the _C_ old,new log entry can be elected as leader. It is

now safe for the leader to create a log entry describing

_C_ new and replicate it to the cluster. Again, this configura-

tion will take effect on each server as soon as it is seen.

When the new configuration has been committed under

the rules of _C_ new, the old configuration is irrelevant and

servers not in the new configuration can be shut down. As

shown in Figure 11, there is no time when _C_ old and _C_ new
...
_C_ new log entry. This means that there will be a period of

time (while it is committing _C_ new) when the leader is man-

aging a cluster that does not include itself; it replicates log

entries but does not count itself in majorities. The leader

transition occurs when _C_ new is committed because this is

the first point when the new configuration can operate in-

dependently (it will always be possible to choose a leader

from _C_ new). Before this point, it may be the case that only

a server from _C_ old can be elected leader.

The third issue is that removed servers (those not in

_C_ new) can disrupt the cluster. These servers will not re-
...
to a _snapshot_ on stable storage, then the entire log up to

11

![background image](https://github.com/hejack2007/A1-collection-manual/blob/master/md/In Search of an Understandable Consensus Algorithm/In Search of an Understandable Consensus Algorithm012.png)

Figure 12: A server replaces the committed entries in its log

(indexes 1 through 5) with a new snapshot, which stores just

the current state (variables _x_ and _y_ in this example). The snap-
...
in the snapshot: the _last included index_ is the index of the

last entry in the log that the snapshot replaces (the last en-

try the state machine had applied), and the _last included

term_

is the term of this entry. These are preserved to sup-
...
![background image](https://github.com/hejack2007/A1-collection-manual/blob/master/md/In Search of an Understandable Consensus Algorithm/In Search of an Understandable Consensus Algorithm013.png)
...
mit a blank _no-op_ entry into the log at the start of its
...
![background image](https://github.com/hejack2007/A1-collection-manual/blob/master/md/In Search of an Understandable Consensus Algorithm/In Search of an Understandable Consensus Algorithm014.png)
...
A paired _t_-test states that, with 95% confidence, the true
...
![background image](https://github.com/hejack2007/A1-collection-manual/blob/master/md/In Search of an Understandable Consensus Algorithm/In Search of an Understandable Consensus Algorithm015.png)
...
![background image](https://github.com/hejack2007/A1-collection-manual/blob/master/md/In Search of an Understandable Consensus Algorithm/In Search of an Understandable Consensus Algorithm016.png)
...
![background image](https://github.com/hejack2007/A1-collection-manual/blob/master/md/In Search of an Understandable Consensus Algorithm/In Search of an Understandable Consensus Algorithm017.png)
...
In _Proc. NSDI’11, USENIX Conference on Networked
...
coupled distributed systems. In _Proc. OSDI’06, Sympo-

sium on Operating Systems Design and Implementation_(2006), USENIX, pp. 335–350.

[3] CAMARGOS, L. J., SCHMIDT, R. M., AND PEDONE, F.

Multicoordinated Paxos. In _Proc. PODC’07, ACM Sym-

posium on Principles of Distributed Computing_
...
_PODC’07, ACM Symposium on Principles of Distributed
...
_USENIX Symposium on Operating Systems Design and

Implementation_

(2006), USENIX, pp. 205–218.
...
globally-distributed database. In _Proc. OSDI’12, USENIX

Conference on Operating Systems Design and Implemen-

tation_

(2012), USENIX, pp. 251–264.

[7] COUSINEAU, D., DOLIGEZ, D., LAMPORT, L., MERZ,

S., RICKETTS, D., AND VANZETTO, H. TLA

+ proofs.

In _Proc. FM’12, Symposium on Formal Methods_(2012),

D. Giannakopoulou and D. M´ery, Eds., vol. 7436 of _Lec-

ture Notes in Computer Science_

, Springer, pp. 147–154.

[8] GHEMAWAT, S., GOBIOFF, H., AND LEUNG, S.-T. The

Google file system. In _Proc. SOSP’03, ACM Symposium

on Operating Systems Principles_
...
In _Proceedings of the 12th ACM Ssymposium on Operating
...
correctness condition for concurrent objects. _ACM Trans-

actions on Programming Languages and Systems 12_
...
systems. In _Proc ATC’10, USENIX Annual Technical Con-

ference_

(2010), USENIX, pp. 145–158.

[12] JUNQUEIRA, F. P., REED, B. C., AND SERAFINI, M.

Zab: High-performance broadcast for primary-backup sys-

tems. In _Proc. DSN’11, IEEE/IFIP Int’l Conf. on Depend-

able Systems & Networks_
...
a distributed system. _Commununications of the ACM 21_, 7

(July 1978), 558–565.

[15] LAMPORT, L. The part-time parliament. _ACM Transac-_

_tions on Computer Systems 16_

, 2 (May 1998), 133–169.

[16] LAMPORT, L. Paxos made simple. _ACM SIGACT News_

_32_

, 4 (Dec. 2001), 18–25.

[17] LAMPORT, L. _Specifying Systems, The TLA+ Language_

_and Tools for Hardware and Software Engineers_
...
[19] LAMPORT, L. Fast paxos. _Distributed Computing 19_, 2

(2006), 79–103.

[20] LAMPSON, B. W. How to build a highly available system

using consensus. In _Distributed Algorithms_, O. Baboaglu
...
_PODC’01, ACM Symposium on Principles of Distributed
...
![background image](https://github.com/hejack2007/A1-collection-manual/blob/master/md/In Search of an Understandable Consensus Algorithm/In Search of an Understandable Consensus Algorithm018.png)

[24] LORCH, J. R., ADYA, A., BOLOSKY, W. J., CHAIKEN,

R., DOUCEUR, J. R., AND HOWELL, J. The SMART

way to migrate replicated stateful services. In _Proc. Eu-
...
In _Proc. OSDI’08, USENIX Conference on_
...
_Proc. SOSP’13, ACM Symposium on Operating System
...
highly-available distributed systems. In _Proc. PODC’88,

ACM Symposium on Principles of Distributed Computing_(1988), ACM, pp. 8–17.

[30] O’NEIL, P., CHENG, E., GAWLICK, D., AND ONEIL, E.

The log-structured merge-tree (LSM-tree). _Acta Informat-

ica 33_

, 4 (1996), 351–385.

[31] ONGARO, D. _Consensus: Bridging Theory and Practice_.
...
E., AND STUTSMAN, R. The case for RAMCloud. _Com-

munications of the ACM 54_
...
and implementation of a log-structured file system. _ACM

Trans. Comput. Syst. 10_

(February 1992), 26–52.

[37] SCHNEIDER, F. B. Implementing fault-tolerant services

using the state machine approach: a tutorial. _ACM Com-
...
In _Proc. MSST’10, Symposium on Mass Storage Sys-

tems and Technologies_
