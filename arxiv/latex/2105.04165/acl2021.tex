...
\def\aclpaperid{97} %  Enter the acl Paper ID here
...
% \algrenewcommand{\alglinenumber}[1]{\color{white}\footnotesize#1:}
...
\author{Pan Lu$^{1*}$, Ran Gong$^{1*}$, Shibiao Jiang$^{2*}$, Liang Qiu$^1$, Siyuan Huang$^1$, \\ \textbf{Xiaodan Liang$^3$, Song-Chun Zhu$^{1,4,5,6}$}\\
  ...
  $^{4}$Beijing Institute of General Artificial Intelligence, $^{5}$Peking University, $^{6}$Tsinghua University\\
...
% The code and data are available at \url{https://lupantech.github.io/inter-gps}.
% \footnote{We will make the dataset and code publicly available in the camera ready version.}
...
% \xd{shall we first explain the benefit of structured and interpretable representation over NN models?} For example\xd{delete?}, the structural representation such as formal language\xd{give examples to illustrate what formal language is, quite strange here to suddenly say structured representation. Any references about formal language?} for the problem text and diagram is necessary for machines to understand abstract semantics of the problem. Algorithms also need to make symbolic reasoning and use external knowledge to propagate implicit geometric attributes and relations. 
...
% An early work, using computer vision and natural language processing techniques to solve geometry problems with problem understanding, is attempted by \cite{seo2015solving}. 
...
% Also, a theorem predictor and a low-order first search strategy are employed to provide human-like theorem application steps.
...
A geometry problem $P$ is defined as a tuple $(t,d,\mathbf{c})$, in which $t$ is the input text, $d$ is the diagram image and $\mathbf{c}=\{c_1, c_2, c_3, c_4\}$ is the multiple-choice candidate set in the format of numerical values. Given the text $t$ and diagram $d$, an algorithm is required to predict the correct answer $c_i \in \mathbf{c}$. We formally describe the problem in the geometric domain language $\Omega$, a set of literals composed of predicates and arguments. Basic terms used in the geometry problem solver are defined as follows.

\textbf{Definition 1.} A \textit{predicate} is a geometric shape entity, geometric relation, or arithmetic function.

\textbf{Definition 2.} A \textit{literal} is an application of one \textit{predicate} to a set of arguments like variables or constants. A set of \textit{literals} makes up the semantic description from the problem text and diagrams in the formal language space $\Omega$.
...
% \linespread{0.9}\selectfont % line space
%\scriptsize
\footnotesize
% \small 
\begin{tabular}{l|l}
...
% \renewcommand\arraystretch{0.9}  % vertical space
%\footnotesize
\small 
\begin{tabular}{l|ccccccc}
	...
	\hline
	GeoShader \cite{alvin2017synthesis} & 102 & / & 4 & 1 & 1 & 6-10  & \{$+$, $-$, $\times$, $\div$, $\Box^2$, $\sqrt\Box$\} \\
	GEOS \cite{seo2015solving} & 186 & 4,343 & 4 & 3 & 1 & 6-10  & \{$+$, $-$, $\times$, $\div$, $\Box^2$, $\sqrt\Box$\} \\ 
	GEOS++ \cite{sachan2017textbooks} & 1,406 & / & 4 & 3 & 1 & 6-10 & \{$+$, $-$, $\times$, $\div$, $\Box^2$, $\sqrt\Box$\} \\
	GEOS-OS \cite{sachan2017learning} & 2,235 & / & 4 & 3 & 1 & 6-10 & \{$+$, $-$, $\times$, $\div$, $\Box^2$, $\sqrt\Box$\} \\
	\textbf{Geometry3K} (ours) & 3,002 & 36,736 & 6 & 4 & 3 & 6-12 & \footnotesize \{$+$, $-$, $\times$, $\div$, $\Box^2$, $\sqrt\Box$, $\sin$, $\cos$, $\tan$\} \\
	\hline
\end{tabular}
\vspace{0mm}
...
% Each problem text is annotated in the format of LaTeX. 
...
% We also harvest 17 theorems to provide axiomatic knowledge for the symbolic geometry solver. 
...
% \linespread{0.8}\selectfont % line space
\small 
...
Given the word sequence of the problem text $T$, the text parser needs to translate it into a set of literals $L_t$, a sequence composed of predicates and variables. Recently, deep neural networks have achieved promising performances in sequence-to-sequence (Seq2Seq) learning tasks like machine translation \cite{sutskever2014sequence,vaswani2017attention,devlin2018bert}. However, semantic parsers using Seq2Seq learning methods are not feasible to generate satisfactory literals in the Geometry3K dataset for two reasons. Firstly, the limited scale of geometry datasets weakens these highly data-driven methods. Secondly, neural semantic parsers tend to bring noises in generated results while geometry solvers with symbolic reasoning are sensitive to such deviations. 
...
% An example of parsing the problem text into literals is illustrated in Table \ref{appex:parsing} at the Appendix. 

% Results are discussed in the Section \ref{sec:discussion}. 

% \xd{move this paragraph before our implementation, and then introduce the benefit of our rule-based parsing}
...
% Different from all the previous works, our dataset has a low overlap between diagram information and text information. Only a small fraction of the problems can be solved without the diagram. Therefore accurate diagram information is important. Our diagram parser has the following steps:
% \begin{itemize}
%     \item Extract diagram primitives through \cite{seo2014diagram}
%     \item Extract diagram symbols and text regions through RetinaNet\cite{lin2017focal}
%     \item OCR through MathPix
%     \item Groud diagram symbols and primitives through a constrained optimization
% \end{itemize}
% \textbf{Primitive extraction}. We follow \cite{seo2014diagram} to extract basic primitives like points, lines, and circles. It is a two-stage process. In the first stage, we use Hough transform to extract a set of lines and circles. In the second stage, an optimization is performed to minimize the number of primitives selected to cover as many pixels as possible. \\
% \textbf{Symbol extraction}. Different from all prior works, our dataset contains a rich set of symbols. To detect symbols robustly, we adopt a feature-based approach using RetinaNet\cite{lin2017focal} as it can match symbols with the presence of noise, illumination changes, and background changes\cite{zhang2018unreasonable}. \\
% \textbf{Symbol grounding}. After obtaining a set of symbols and primitives, we need to pair each symbol with its associated primitives. \cite{seo2015solving} adapted a greedy approach where they only use distance as the single metric, namely each symbol is assigned to the closest primitive. However, this approach scales poorly in our dataset as our dataset has a more diverse set of symbols and symbols and primitives often are crowded in a small region. Therefore we formulated it as a constrained mix integer optimization problem where the constraints are mined from text parser and symbols. 
% \begin{equation*}
% \begin{aligned}
%     &\min \sum_{s} dist(s_i, p_j) \mathbbm{1}\{ s_i \text{ assigns to } p_j \} \\
%     &\text{s.t. } (s_i, p_j) \in \text{feasiable}
% \end{aligned} 
% \end{equation*}
% where $dist$ function measures the distance between symbol $s_i$ and primitive $p_j$. Feasibility set is symbol specific. For example, if a perpendicular symbol is detected, line segment primitives are only allowed to be associated to it if $\frac{v_{1}}{|v_{1}|} \cdot \frac{v_{2}}{|v_{2}|} \in (0-\epsilon_1, 0+\epsilon_1)$. If a parallel symbol is detected,  line segment primitives are only allowed to be associated to it if $\frac{v_{1}}{|v_{1}|} - \frac{v_{2}}{|v_{2}|} \in (0 -\epsilon_2,  0+\epsilon_2)$ where $v_1$ and $v_2$ are end points of each line segment in the image space. Currently, $\epsilon_1, \epsilon_2$ are set heuristically; however, it is possible to learn these parameters in a dataset. The above optimization problem can be solved through dynamic programming.
...
\begin{equation}
\begin{split}
    \mathcal{R}_{t} \gets k_i \land \mathcal{R}_{t-1}, k_i \in \mathcal{KB}.
\end{split}
\end{equation}
...
\begin{equation}
\begin{split}
    g^* \gets \text{\scshape SolveEquation}(\mathcal{R}_t, g).
\end{split}
\end{equation}
...
% Recent studies \cite{ alemi2016deepmath,loos2017deep,balunovic2018learning, lederman2018learning} also suggest that neural guided search can speed up the search process.

There are no annotated theorem application sequences for data in Geometry3K due to tremendous worker labor. 
% Thus, we numerate randomly generated sequences from the theorem set.
...
Given the problem formal description $L = \{l_1,...,l_m\}$, the theorem predictor aims to reconstruct the pseudo-optimal theorem  sequence $T = \{t_1,...,t_n\}$ token by token. We formulate the generation task as a sequence-to-sequence (Seq2Seq) problem and use a transformer-based model \cite{lewis2020bart} to generate theorem sequence tokens.  Specifically, the transformer decoder predicts the next theorem order $t_i$ given $T = \{t_1,...,t_i\}$. The Seq2Seq model is trained to optimize the negative log-likelihood loss:
\begin{equation}
\begin{split}
    \mathcal{L}_{\mathrm{TP}} = 
    -\sum_{i=1}^{n} 
    \log p_{\mathrm{TP}}\left(t_{i} \mid t_{1}, \ldots, t_{i-1}\right),
\end{split}
\end{equation}
where $p_{\mathrm{TP}}$ is the parametrized conditional  distribution in the theorem predictor model.
...
\hspace*{0.02in} {\bf Input} 
 Literals $\mathcal{L}$, goal $g$, knowledge bases \hspace*{0.0in}  $\mathcal{KB}_1$, $\mathcal{KB}_2$ \\
\hspace*{0.02in} {\bf Output}
Numeric goal value $g^*$ and theorem application $\mathcal{S}$
\begin{algorithmic}[1]
    \Function{Search}{$\mathcal{L}$, $g$, $\mathcal{KB}_1$, $\mathcal{KB}_2$}
        \State Initialize relation set $\mathcal{R}_0$ with $\mathcal{L}$, $g^* = \emptyset$, $\mathcal{S} =  \emptyset$
        \State $\mathcal{KB}_p \gets \text{\scshape TheoPredictor}(\mathcal{L})$ \Comment{Predicted}
        \For{$k_i \in \mathcal{KB}_p$} 
            \State $\mathcal{R}_{t} \gets k_i \land \mathcal{R}_{t-1}$
            \State $\mathcal{S}$.\text{\scshape append}($k_i$)
        \EndFor
        \State $g^* \gets \text{\scshape SolveEquation}(\mathcal{R}_t, g)$
        \If {$g^* \neq \emptyset$}
            \State \Return $g^*$ and $S$
        \EndIf
        \While{$g^* = \emptyset$ and $\mathcal{R}_t$ is updated}
        \For{$k_i \in \mathcal{KB}_1$} \Comment{Lower-order}
            \State $\mathcal{R}_{t} \gets k_i \land \mathcal{R}_{t-1}$
            \State $\mathcal{S}$.\text{\scshape append}($k_i$)
            \State $g^* \gets \text{\scshape SolveEquation}(\mathcal{R}_t, g)$
            \If {$g^* \neq \emptyset$}
            ...
            \EndIf
        \EndFor
        \For{$k_i \in \mathcal{KB}_2$} \Comment{Higher-order}
            \State $\mathcal{R}_{t} \gets k_i \land \mathcal{R}_{t-1}$
            \State $\mathcal{S}$.\text{\scshape append}($k_i$)
            \State $g^* \gets \text{\scshape SolveEquation}(\mathcal{R}_t, g)$
            \If {$g^* \neq \emptyset$}
        ...
        % \State \textbf{return} $g^*$ and $\mathcal{S}$
    \EndFunction
\end{algorithmic}
\end{algorithm}
...
After the application of the theorem sequence predicted by the theorem predictor, it is likely that Inter-GPS still could not find the problem goal. Generally, humans incline to use simple theorems first when solving math problems to reduce complex calculations. If simple theorems are not tangible, they will turn to more complex theorems. On account of that, we apply an efficient search strategy with heuristics driven by subject knowledge. We categorize theorems into two groups: \textbf{lower-order} theorem set $\mathcal{KB}_1$ and \textbf{higher-order} theorem set $\mathcal{KB}_2$. The lower-order set $\mathcal{KB}_1$ (e.g, \textit{Triangle Angle-Sum Theorem}, \textit{Congruent Triangle Theorem}) only involves in two simple operations of addition and subtraction, while $\mathcal{KB}_2$ (e.g, \textit{Law of 
Sines}) requires complex calculations. In each following search step after using predicted theorems, we first enumerate theorems in the lower-order set $\mathcal{KB}_1$ to update the relation set $\mathcal{R}$:
\begin{equation}
\begin{split}
    \mathcal{R}_{t} \gets k_i \land \mathcal{R}_{t-1}, k_i \in \mathcal{KB}_1.
\end{split}
\end{equation}
If lower-order theorems fail to update $\mathcal{R}$ anymore, higher-order theorems are considered to update $\mathcal{R}$:
\begin{equation}
\begin{split}
    \mathcal{R}_{t} \gets k_i \land \mathcal{R}_{t-1}, k_i \in \mathcal{KB}_2.
\end{split}
\end{equation}

% This system of equations is rather large, and redundant. Therefore, we use the following criterion to eliminate equations:\begin{itemize}
%     \item We eliminate equations that do not contain any symbols.
%     \item We eliminate trivia equations like x = x.
%     \item If we have n equations and there are n unknowns already, we replace the higher-order equations with lower order equations. For example, if we have three equations: $x + y = 3, x \times y = 2, x - y = 1$, we will replace the second equation with the third equation.  
% \end{itemize}

The search process stops once we find the problem goal $g$ or the search steps reach the maximum steps allowed. The whole search algorithm for Inter-GPS is presented in Algorithm \ref{alg:solver}.
...
% 	Q+I (Formal) &34.6 &34.6 &34.3 &37.7 &33.3 &33.3 &34.0 &35.7 &34.3 &40.7 \\
...
% if the difference between the found solution and the ground truth is smaller than a threshold of 0.1. 
...
\caption{Evaluation results on the GEOS  dataset.}% \vspace{0mm}
\label{table:result2}
\end{table}

% \subsection{Neural Solver Baseline}\label{sec:neural_solver}\xd{may need to explain the connection of neural solver with symbolic solver, why do we introduce in the method part. If it is only a baseline, why not place it in the experiment}
% Recently, neural networks have accomplished promising success in various math question answering tasks like visual question answer \citep{antol2015vqa, johnson2017clevr,hu2017learning} and math word problems \citep{wang2017deep, wang2019template}. Motivated by that, we also design a deep neural solver for geometry problem solving.  We denote the formal language from the problem text as $L^t$ and that from the diagram as $L^d$.
% By concatenating each choice candidate $c_i$ to the end of formal description, we obtain an output sequence $L_i$,
% \begin{equation}
% \begin{split}
%     L_i = (L^t, L^d, c_i),\ i=1,...,4. 
% \end{split}
% \end{equation}
% The sequence of problem words $L_i$ is then encoded by a BERT model \cite{devlin2018bert}. Following other methods that use BERT as sequence encoders, the first token $\mathtt{[CLS]}$ is considered as the embedding of the entire problem, noted as $\textbf{h}^{L_i}$.

% Taking the encoded unified problem formal descriptions as inputs, a classifier is used to predict the confidence score of choice candidates, and the outputs are the probabilities of the four choices:
% \begin{equation}
% \begin{split}
%     &\textbf{h}^{L} = \left[\textbf{h}^{L_1}, \textbf{h}^{L_2}, \textbf{h}^{L_3}, \textbf{h}^{L_4}\right],\\
%     &\text{Prob}(\mathbf{c}) = \textbf{W}_2  (\text{ReLU}(\textbf{W}_1\textbf{h}^{L})),
% \end{split}
% \end{equation}
% where $\textbf{W}_1, \textbf{W}_2$ are learnable matrices, ReLU is a non-linear activation function.
...
% All hyper-parameters are tuned on the validation-split data using the grid search strategy. For neural network models, all the embedding dimensions are set as 128. Since there can be numerous literals for a single geometry problem, we only select the top 30 literals\xd{select for what? before search?}. 
...
\begin{tabular}{l|c|c}
	\hline
	 ...
	 \hline
	 Predict & 77.5 $\pm$ 0.1 & \textbf{6.5} $\pm$ 0.1 \\
	 \hline
	 Predict+Low-first (final) & \textbf{78.3} $\pm$ 0.1 & 7.1 $\pm$ 0.1 \\
	\hline
\end{tabular}
%\vspace{0mm}
...
\begin{tabular}{l|c|c|c}
	\hline
	 ...
	 \hline
	 Text (GT) & 25.4 $\pm$ 0.0  & 58.0 $\pm$ 1.7 & \textbf{78.3} $\pm$ 0.1 \\
	\hline
\end{tabular}
% \vspace{0mm}
...
% \textit{Q+I (Formal)} with the input of formal description gains an improvement of 2.3\% over \textit{Q+I}. It shows that structural representations like formal language benefit the solvers to learn more meaningful semantics.


% res_table:nn_solvers
\begin{table}[ht]
\centering 
\setlength{\tabcolsep}{3pt}
% \linespread{0.8}\selectfont % line space
